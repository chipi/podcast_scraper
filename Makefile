...
ci-ui-validation:
	...
	@if [ -z "$(CORPUS)" ]; then \
		...
		echo "Example: make ci-ui-validation CORPUS=$$HOME/.test_outputs/manual/my-corpus"; \
		exit 2; \
	fi
	@echo "=== ci-ui-validation against $(CORPUS) ==="
	@echo "Requires ``make serve`` running in another terminal."
	cd web/gi-kg-viewer && \
		CORPUS_PATH=$(CORPUS) \
