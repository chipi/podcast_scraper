# Autoresearch — Track A (paragraph summaries, Gemini)

Human-maintained instructions for the coding agent. **Do not** let the agent edit this file during a run.

This is the **Gemini paragraph summary** research line. For OpenAI, see `program_summary.md`.

## Goal

Improve allowlisted **paragraph** summarization prompts (`.j2`) for the Gemini experiment config
`data/eval/configs/autoresearch_prompt_gemini_smoke_paragraph_v1.yaml`, measured by
...
- Env key required: `GEMINI_API_KEY` (loaded from `.env`)
...
1. `src/podcast_scraper/prompts/gemini/summarization/long_v1.j2` — **user prompt** (primary)
2. `src/podcast_scraper/prompts/gemini/summarization/system_v1.j2` — **system prompt** (secondary;
...
- `autoresearch/prompt_tuning/program_summary_gemini.md` (this file)
...
3. From repo root:
   ...
   make autoresearch-score \
     CONFIG=data/eval/configs/autoresearch_prompt_gemini_smoke_paragraph_v1.yaml \
...
4. If score improves >1% vs last kept commit:
   ...
   `git commit -m "[autoresearch-gemini] exp-<N>: <hypothesis>"`.
5. Else: `git checkout HEAD -- <edited file(s)>`.
6. Append a row to `autoresearch/prompt_tuning/results_summary_gemini.tsv` (see header).
...
- On early stop or hard stop: write a summary to
  `autoresearch/prompt_tuning/summary_gemini_<YYYY-MM-DD>.md` covering:
...
- `GEMINI_API_KEY` — summarization calls (Gemini provider)
...
probability of transferring to Gemini:
...
After these, explore Gemini-specific directions:

- **Gemini's instruction-following:** Gemini Flash follows structured bullet lists very
  reliably — the current format should work well; test if reordering instructions matters.
- **System prompt tightening:** Gemini may benefit from a more explicit output-format
  contract in the system prompt (e.g. "Output only paragraphs, no headers or bullets").
