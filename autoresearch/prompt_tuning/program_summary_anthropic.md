# Autoresearch — Track A (paragraph summaries, Anthropic)

Human-maintained instructions for the coding agent. **Do not** let the agent edit this file during a run.

This is the **Anthropic paragraph summary** research line. For OpenAI, see `program_summary.md`.

## Goal

Improve allowlisted **paragraph** summarization prompts (`.j2`) for the Anthropic experiment config
`data/eval/configs/autoresearch_prompt_anthropic_smoke_paragraph_v1.yaml`, measured by
...
- Env key required: `ANTHROPIC_API_KEY` (loaded from `.env`)
...
1. `src/podcast_scraper/prompts/anthropic/summarization/long_v1.j2` — **user prompt** (primary)
2. `src/podcast_scraper/prompts/anthropic/summarization/system_v1.j2` — **system prompt** (secondary;
...
- `autoresearch/prompt_tuning/program_summary_anthropic.md` (this file)
...
3. From repo root:
   ...
   make autoresearch-score \
     CONFIG=data/eval/configs/autoresearch_prompt_anthropic_smoke_paragraph_v1.yaml \
...
4. If score improves >1% vs last kept commit:
   ...
   `git commit -m "[autoresearch-anthropic] exp-<N>: <hypothesis>"`.
5. Else: `git checkout HEAD -- <edited file(s)>`.
6. Append a row to `autoresearch/prompt_tuning/results_summary_anthropic.tsv` (see header).
...
- On early stop or hard stop: write a summary to
  `autoresearch/prompt_tuning/summary_anthropic_<YYYY-MM-DD>.md` covering:
...
- `ANTHROPIC_API_KEY` — summarization calls (Anthropic provider)
...
probability of transferring to Claude:
...
After these, explore Anthropic-specific directions:

- **Claude's natural style:** Claude tends toward well-structured, nuanced prose — the system
  prompt may need less guidance than OpenAI's.
- **System prompt persona:** try "You summarize podcast episodes for a reader who needs the
  key ideas without listening to the full episode." after ≥5 user prompt experiments.
- **Coverage instruction:** "Cover the episode's main arguments and conclusions in order
  of presentation" — test if sequential structure helps ROUGE.
