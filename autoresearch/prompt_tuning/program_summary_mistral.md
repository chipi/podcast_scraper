# Autoresearch — Track A (paragraph summaries, Mistral)

Human-maintained instructions for the coding agent. **Do not** let the agent edit this file during a run.

This is the **Mistral paragraph summary** research line. For OpenAI, see `program_summary.md`.

## Goal

Improve allowlisted **paragraph** summarization prompts (`.j2`) for the Mistral experiment config
`data/eval/configs/autoresearch_prompt_mistral_smoke_paragraph_v1.yaml`, measured by
...
- Env key required: `MISTRAL_API_KEY` (loaded from `.env`)
...
1. `src/podcast_scraper/prompts/mistral/summarization/long_v1.j2` — **user prompt** (primary)
2. `src/podcast_scraper/prompts/mistral/summarization/system_v1.j2` — **system prompt** (secondary;
...
- `autoresearch/prompt_tuning/program_summary_mistral.md` (this file)
...
3. From repo root:
   ...
   make autoresearch-score \
     CONFIG=data/eval/configs/autoresearch_prompt_mistral_smoke_paragraph_v1.yaml \
...
4. If score improves >1% vs last kept commit:
   ...
   `git commit -m "[autoresearch-mistral] exp-<N>: <hypothesis>"`.
5. Else: `git checkout HEAD -- <edited file(s)>`.
6. Append a row to `autoresearch/prompt_tuning/results_summary_mistral.tsv` (see header).
...
- On early stop or hard stop: write a summary to
  `autoresearch/prompt_tuning/summary_mistral_<YYYY-MM-DD>.md` covering:
...
- `MISTRAL_API_KEY` — summarization calls (Mistral provider)
...
probability of transferring to Mistral:
...
After these, explore Mistral-specific directions:

- **Mistral-small is efficient and literal:** it follows instructions closely but may
  produce shorter, less detailed paragraphs — "Write 3–4 well-developed paragraphs" may
  improve depth.
- **Tighter constraints:** Mistral can sometimes add headers or bullet points despite prose
  instructions; "Output only paragraphs, no headers, bullets, or numbered lists" as a
  negative constraint may help.
- **System prompt:** after ≥5 user experiments, test a more prescriptive system prompt that
  emphasizes factual fidelity and prose format.
