...
- `generate_gi: true` with `gi_insight_source: provider`
- `generate_kg: true` with **`kg_extraction_source: provider`** and **`kg_max_topics: 10`**
- `vector_search: true` with `vector_backend: faiss`

GI uses **`gi_insight_source: provider`** (API **`generate_insights`**). KG uses
**`kg_extraction_source: provider`** (API **`extract_kg_graph`**) with **`kg_max_topics: 10`**.
