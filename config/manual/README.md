...
| `manual_multi_feed_planet_money_journal_openai_monitor.yaml` | OpenAI | On (`summary_bullets`) | Same multi-feed OpenAI stack as the first row with **`monitor: true`** (RFC-065: live RSS/CPU/stage + **`.pipeline_status.json`** per feed; **`log_file`** keeps stderr clean). Optional commented **`memray`** lines for **`.[monitor]`** installs. |
| `manual_multi_feed_corpus_rss_registry_openai.yaml` | OpenAI | On (`summary_bullets`) | **11 feeds** from [`docs/wip/CORPUS-RSS-FEEDS.md`](../../docs/wip/CORPUS-RSS-FEEDS.md) (ingest-priority order): Hard Fork, No Priors, The Journal (WSJ), Invest Like the Best, Odd Lots, Planet Money, Latent Space, The Daily, NVIDIA AI Podcast, FT Unhedged, Capital Allocators. GI/KG + **`vector_search`**; **`output_dir`:** `output_manual_corpus_rss_registry`. Lower **`max_episodes`** before wide runs; see registry notes (NPR geo, URL verification). |
