...
**Standard block (GI + KG + FAISS):** Multi-feed presets that turn on intelligence layers match
`manual_multi_feed_corpus_rss_registry_openai_gemini.yaml`: `generate_gi`,
**`gi_insight_source: provider`**, **`gi_max_insights: 12`**, `generate_kg`,
**`kg_extraction_source: provider`**, **`kg_max_topics: 10`**, `vector_search`,
`vector_backend: faiss`. The main pipeline does **not** run RFC-075 topic clustering; after a run
with `vector_search`, optionally run
`python -m podcast_scraper.cli topic-clusters --output-dir <corpus_parent>` (see
[RFC-075](../../docs/rfc/RFC-075-corpus-topic-clustering.md)).

**Git:** Most `*.yaml` here stays **gitignored** (local-only presets). **Versioned** in Git:
this `README.md` and **`manual_e2e_mock.yaml`** (E2E mock HTTP feeds + OpenAI + Gemini full
pipeline). See `.gitignore` under `config/manual` for the exact `!` exception.
...
| --- | --- | --- | --- |
| `manual_e2e_mock.yaml` | OpenAI Whisper + Gemini | On (`provider`, `gi_max_insights: 12`) + **`vector_search`** | **Tracked.** Eight mock feeds on **`127.0.0.1:18765`** (p01–p05 + p07–p09; p06 omitted). Run **`make serve-e2e-mock`** first. Option set aligned with **`manual_multi_feed_corpus_rss_registry_openai_gemini.yaml`**. Needs **`OPENAI_API_KEY`** and **`GEMINI_API_KEY`**. |
| `manual_multi_feed_planet_money_journal_openai_gemini.yaml` | OpenAI Whisper + Gemini | On (`provider`, `gi_max_insights: 12`) | **Multi-feed (GitHub #440):** Planet Money + The Journal under one `output_dir` (`feeds/rss_*/*` per feed). **`openai` + `whisper-1`** for transcription; **`gemini`** + **`gemini-2.5-flash-lite`** for speaker + summary. With `vector_search`, parent **`search/`** + **`corpus_manifest.json`** / **`corpus_run_summary.json`** (#505/#506). Requires **`OPENAI_API_KEY`** and **`GEMINI_API_KEY`**; use **`corpus-status`** for offline inspection. **GitHub #562:** **`screenplay: true`** with non-Whisper **`transcription_provider`** is coerced to **`false`** at validation (one INFO); keep YAML **`screenplay: false`** for clarity when using OpenAI audio transcription. |
| `manual_multi_feed_planet_money_journal_openai_gemini_append.yaml` | OpenAI Whisper + Gemini | On (`provider`, `gi_max_insights: 12`) | Same feeds as above with **`append: true`** (GitHub #444): stable `run_append_*` per feed; re-run the same CLI to skip complete episodes. Header includes **handoff text** for LLM ops channels. |
| `manual_multi_feed_planet_money_journal_openai_gemini_monitor.yaml` | OpenAI Whisper + Gemini | On (`provider`, `gi_max_insights: 12`) | Same multi-feed hybrid stack as the first row with **`monitor: true`** (RFC-065: live RSS/CPU/stage + **`.pipeline_status.json`** per feed; **`log_file`** keeps stderr clean). Optional commented **`memray`** lines for **`.[monitor]`** installs. |
| `manual_multi_feed_corpus_rss_registry_openai_gemini.yaml` | OpenAI Whisper + Gemini | On (`provider`, `gi_max_insights: 12`) | **11 feeds** from [`docs/wip/CORPUS-RSS-FEEDS.md`](../../docs/wip/CORPUS-RSS-FEEDS.md) (ingest-priority order): Hard Fork, No Priors, The Journal (WSJ), Invest Like the Best, Odd Lots, Planet Money, Latent Space, The Daily, NVIDIA AI Podcast, FT Unhedged, Capital Allocators. GI/KG + **`vector_search`**; **`output_dir`:** `.test_outputs/manual/corpus_rss_registry_openai_gemini` (override with **`--output-dir`**). Lower **`max_episodes`** before wide runs; see registry notes (NPR geo, URL verification). |
| `manual_multi_feed_planet_money_journal_deepseek.yaml` | OpenAI Whisper + DeepSeek | On (`provider`, `gi_max_insights: 12`) | **Multi-feed:** Planet Money + The Journal; **`deepseek-chat`** for speaker + summary + **`generate_insights`**. Requires **`OPENAI_API_KEY`** (Whisper) and **`DEEPSEEK_API_KEY`**. |
| `manual_multi_feed_planet_money_journal_deepseek_append.yaml` | OpenAI Whisper + DeepSeek | On (`provider`, `gi_max_insights: 12`) | Same DeepSeek stack with **`append: true`** and handoff header for **`run_append_*`** resume checks. |
