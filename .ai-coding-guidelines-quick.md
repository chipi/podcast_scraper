...
2. ❌ NEVER push without running `make ci` first
3. ❌ NEVER push without explicit user approval
4. ✅ Show changes → run `make ci` → fix failures → ask approval → THEN push
...
### Testing After Changes (MANDATORY)

1. ✅ Run `make test-ci-fast` first (fast tests, ~6-10 min)
2. ✅ If changes touch ML/slow areas, run slow tests
3. ✅ Run `make ci` before pushing (full validation)
