...
Usage:
    ...
    SKIP_WHISPER=1 python scripts/cache/preload_ml_models.py
...
def preload_transformers_models(model_names: Optional[List[str]] = None) -> None:  # noqa: C901
    ...
    for model_name in model_names:
        ...
        model_cache_name = resolved_model.replace("/", "--")
        ...
        try:
            ...
            ), f"Model {resolved_model} loaded but is None"

            # Verify tokenizer works
            tokens = tokenizer.encode("Test text", return_tensors="pt")
            assert tokens is not None, f"Tokenizer {resolved_model} doesn't encode text"

            # Verify model structure
            assert (
                hasattr(model, "config") and model.config is not None
            ), f"Model {resolved_model} missing config"
            ...
            # Explicit path logging for debugging
            # Use resolved_model for cache path (not alias)
            model_cache_name = resolved_model.replace("/", "--")
            ...
            try:
                ...
                cache_path = (
                    transformers_cache_base / f"models--{resolved_model.replace('/', '--')}"
                )
...
def main() -> None:
    ...
    if args.production:
        ...
        # MiniLM embedding is corpus-wide core (vector search default #897/#899,
        # GI about-edges, the corpus graph, and the search aux/two-tier/hybrid
        # integration tests all need it offline). Always preload it, independent
        # of SKIP_GIL — same as the --production path above.
        from podcast_scraper import config_constants as _cc
