...
| **`pipeline/`** | One-shot pipeline runner — `INSTALL_EXTRAS=ml` (default, ships local Whisper / spaCy / transformers / LanceDB) or `INSTALL_EXTRAS=llm` (cloud APIs only) | `make stack-test-build` (ml) and `make stack-test-build-cloud` (llm) |
