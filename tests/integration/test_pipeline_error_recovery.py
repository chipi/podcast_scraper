#!/usr/bin/env python3
"""Integration tests for pipeline error recovery and edge cases (Stage 8).
...
<rss xmlns:podcast="https://podcastindex.org/namespace/1.0" version="2.0">
  <channel>
    ...
    <item>
      ...
      <podcast:transcript url="http://127.0.0.1:{self.server.server_address[1]}/transcript-error-404.vtt" type="{TEST_TRANSCRIPT_TYPE_VTT}" />
    </item>
    <item>
      <title>Episode 3: Success</title>
      <podcast:transcript url="http://127.0.0.1:{self.server.server_address[1]}/transcript3.vtt" type="{TEST_TRANSCRIPT_TYPE_VTT}" />
    </item>
  </channel>
</rss>"""
            self._send_response(200, rss_xml.encode("utf-8"), "application/xml")
...
class TestPipelineErrorRecovery:
    ...
    def test_pipeline_handles_malformed_rss_feed(self):
        ...
        # XML parser might be lenient, so we check that it either raises or completes with 0 episodes
        try:
            count, summary = workflow.run_pipeline(cfg)
