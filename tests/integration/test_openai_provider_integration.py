#!/usr/bin/env python3
"""OpenAI provider integration tests (Stage 10).

These tests verify that OpenAI providers work correctly in integration workflows:
...
<rss xmlns:podcast="https://podcastindex.org/namespace/1.0" version="2.0">
  <channel>
    ...
    <item>
      ...
      <description>This is a test episode with Joe Rogan and Elon Musk mentioned for OpenAI speaker detection.</description>
    ...
    def test_openai_all_providers_in_pipeline(self, openai_http_server):
        ...
        try:
            ...
            with (
                ...
                # Mock Whisper transcription and file writing to ensure transcript file exists
                # Note: When using OpenAI transcription, we don't use Whisper, so this won't be called
                # But we still mock it in case the pipeline falls back to Whisper
                ...
                # Summarization may not be called if transcript file doesn't exist or there's an error
                # But if it was called, verify it worked
                ...
                # Transcript files may not exist if OpenAI transcription fails or is mocked incorrectly
                # The key is that the pipeline completes
