"""Integration tests for Phi-3 Mini speaker detection (Issue #396).

These tests verify that Phi-3 Mini model-specific prompts are loaded correctly
and that speaker detection works with the optimized prompts (simplified for lightweight model).
"""
...
@pytest.mark.llm
@pytest.mark.ollama
class TestPhi3MiniSpeakerDetection(unittest.TestCase):
    """Test Phi-3 Mini speaker detection with model-specific prompts."""
    ...
    def test_provider_initialization_with_phi3_model(self, mock_openai_class, mock_httpx):
        """Test that Ollama speaker detector initializes correctly with Phi-3 Mini model."""
        ...
        self.assertEqual(detector.speaker_model, "phi3:mini")

    @patch("podcast_scraper.providers.ollama.ollama_provider.httpx")
    @patch("podcast_scraper.providers.ollama.ollama_provider.OpenAI")
    @patch("podcast_scraper.prompts.store.render_prompt")
    def test_detect_speakers_with_phi3_prompts(
        self, mock_render_prompt, mock_openai_class, mock_httpx
    ):
        """Test successful speaker detection using Phi-3 Mini model-specific prompts."""
        # Mock health check and model validation
        mock_health_response = Mock()
        mock_health_response.raise_for_status = Mock()
        mock_models_response = Mock()
