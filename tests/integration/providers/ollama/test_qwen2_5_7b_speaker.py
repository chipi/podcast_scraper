"""Integration tests for Qwen 2.5 7B speaker detection (Issue #430).

These tests verify that Qwen 2.5 7B model-specific prompts are loaded correctly
and that speaker detection works with the optimized prompts (emphasizing structured JSON).
"""
...
@pytest.mark.llm
@pytest.mark.ollama
class TestQwen257BSpeakerDetection(unittest.TestCase):
    """Test Qwen 2.5 7B speaker detection with model-specific prompts."""
    ...
    def test_provider_initialization_with_qwen_model(self, mock_openai_class, mock_httpx):
        """Test that Ollama speaker detector initializes correctly with Qwen 2.5 7B model."""
        ...
        self.assertEqual(detector.speaker_model, "qwen2.5:7b")
    ...
    def test_detect_speakers_with_qwen_prompts(
        ...
        """Test successful speaker detection using Qwen 2.5 7B model-specific prompts."""
        # Mock health check and model validation
        mock_health_response = Mock()
        mock_health_response.raise_for_status = Mock()
        mock_models_response = Mock()
