"""Integration tests for Llama 3.1 8B summarization (Issue #394).

These tests verify that Llama 3.1 8B model-specific prompts are loaded correctly
and that summarization works with the optimized prompts.

Note: File name uses underscores (test_llama3_1_8b_summary.py) because Python
...
@pytest.mark.llm
@pytest.mark.ollama
class TestLlama31_8BSummarization(unittest.TestCase):
    """Test Llama 3.1 8B summarization with model-specific prompts."""
    ...
    def test_provider_initialization_with_llama_model(self, mock_openai_class, mock_httpx):
        """Test that Ollama summarization provider initializes correctly with Llama 3.1 8B model."""
        ...
        self.assertEqual(provider.summary_model, "llama3.1:8b")

    @patch("podcast_scraper.providers.ollama.ollama_provider.httpx")
    @patch("podcast_scraper.providers.ollama.ollama_provider.OpenAI")
    @patch("podcast_scraper.prompts.store.render_prompt")
    @patch("podcast_scraper.prompts.store.get_prompt_metadata")
    def test_summarize_with_llama_prompts(
        self, mock_get_metadata, mock_render_prompt, mock_openai_class, mock_httpx
    ):
        """Test successful summarization using Llama 3.1 8B model-specific prompts."""
        # Mock health check and model validation
        ...
        mock_get_metadata.return_value = {"name": "ollama/llama3.1_8b/summarization/system_v1"}

        # Mock OpenAI client
        mock_client = Mock()
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "This is a summary from Llama 3.1 8B."
        ...
        self.assertEqual(result["summary"], "This is a summary from Llama 3.1 8B.")
