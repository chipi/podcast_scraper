"""Integration tests for Gemma 2 9B speaker detection (Issue #397).

These tests verify that Gemma 2 9B model-specific prompts are loaded correctly
and that speaker detection works with the optimized prompts (balanced quality/speed).
"""
...
@pytest.mark.llm
@pytest.mark.ollama
class TestGemma29BSpeakerDetection(unittest.TestCase):
    """Test Gemma 2 9B speaker detection with model-specific prompts."""
    ...
    def test_provider_initialization_with_gemma_model(self, mock_openai_class, mock_httpx):
        """Test that Ollama speaker detector initializes correctly with Gemma 2 9B model."""
        ...
        self.assertEqual(detector.speaker_model, "gemma2:9b")

    @patch("podcast_scraper.providers.ollama.ollama_provider.httpx")
    @patch("podcast_scraper.providers.ollama.ollama_provider.OpenAI")
    @patch("podcast_scraper.prompts.store.render_prompt")
    def test_detect_speakers_with_gemma_prompts(
        self, mock_render_prompt, mock_openai_class, mock_httpx
    ):
        """Test successful speaker detection using Gemma 2 9B model-specific prompts."""
        # Mock health check and model validation
        mock_health_response = Mock()
        mock_health_response.raise_for_status = Mock()
        mock_models_response = Mock()
