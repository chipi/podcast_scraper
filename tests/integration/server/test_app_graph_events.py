...
from podcast_scraper.server.app_user_store import get_or_create_user, set_role, User

pytestmark = [pytest.mark.integration]


def _client(root: Path) -> tuple[TestClient, User]:
    """Authed client + the seeded user (RFC-120: graph-events now requires auth)."""
...
def test_graph_events_anonymous_logged_under_anon(tmp_path: Path) -> None:
    # RFC-120 made this route require auth; events now log under the authed user's id.
    client, user = _client(tmp_path)
    resp = client.post(
    ...
    events = app_graph_telemetry.read_events(tmp_path / "appdata", user.user_id)
...
def test_graph_events_empty_is_noop_204(tmp_path: Path) -> None:
    ...
    assert resp.status_code == 204
    assert app_graph_telemetry.read_events(tmp_path / "appdata", user.user_id) == []


def test_graph_events_summary_requires_admin(tmp_path: Path) -> None:
    # A non-admin authed user → the admin gate rejects with 403.
    client, _user = _client(tmp_path)
