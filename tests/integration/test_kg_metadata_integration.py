...
# The mock must carry a non-None __spec__ so that later `importlib.util.find_spec("spacy")`
# (called by transformers at import-time) does not raise ValueError.
if "spacy" not in sys.modules:
    _spacy_mock = MagicMock()
    _spacy_mock.__spec__ = "mocked"
...
@pytest.mark.integration
def test_generate_episode_metadata_kg_stub_source(
    tmp_path,
    monkeypatch,
) -> None:
    """Stub KG path: kg.json written on disk; extract_kg_graph NOT called; metrics updated."""
    out = tmp_path / "run"
    out.mkdir(parents=True)
    (out / "transcripts").mkdir()
    ep = create_test_episode(idx=2, title="Stub Episode", title_safe="Stub_Episode")
    tname = filesystem.build_whisper_output_name(2, "Stub_Episode", None)
    trans_path = out / "transcripts" / tname
    trans_path.write_text("Discussing markets and technology.", encoding="utf-8")
    ...
    prov = MagicMock()
    prov.summary_model = "mock-stub-model"
    pm = metrics_mod.Metrics()
    ...
    kg_paths = list((out / "metadata").rglob("*.kg.json"))
    assert len(kg_paths) == 1, f"expected one kg.json, got {kg_paths}"
    prov.extract_kg_graph.assert_not_called()
    assert pm.kg_artifacts_generated == 1
    assert pm.kg_extractions_stub == 1
