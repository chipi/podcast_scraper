#!/usr/bin/env python3
"""Concurrent execution integration tests (Stage 9).
...
class ConcurrentHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
      ...
      <enclosure url="http://127.0.0.1:{self.server.server_address[1]}/episode{i}.mp3" type="audio/mpeg" />
      <podcast:transcript url="http://127.0.0.1:{self.server.server_address[1]}/transcript{i}.vtt" type="{TEST_TRANSCRIPT_TYPE_VTT}" />
    </item>"""
                )
