"""Fake Gemini SDK client for E2E and integration testing.
...
Usage:
    In E2E or integration tests, monkeypatch the Gemini SDK to use this fake client:

    ```python
    from tests.fixtures.mock_server.gemini_mock_client import create_fake_gemini_client

    # In conftest.py or test setup
    FakeGenerativeModel = create_fake_gemini_client(base_url)
...
class FakeGeminiUsageMetadata:
    """Fake usage metadata that mimics Gemini SDK usage_metadata structure."""

    def __init__(self, prompt_token_count: int = 0, candidates_token_count: int = 0):
        """Initialize fake usage metadata.

        Args:
            prompt_token_count: Number of input tokens (default: 0)
            candidates_token_count: Number of output tokens (default: 0)
        """
...
class FakeGeminiResponse:
    ...
    def __init__(self, text: str, input_tokens: int = 0, output_tokens: int = 0):
        ...
        """
        self.text = text
        self.usage_metadata = FakeGeminiUsageMetadata(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            base_url: Base URL for E2E mock server
                (e.g., "http://127.0.0.1:18765/v1beta")
        ...
        self.base_url = base_url or "http://127.0.0.1:18765/v1beta"
            ...
            if "candidates" in data and len(data["candidates"]) > 0:
                ...
                if "content" in candidate and "parts" in candidate["content"]:
                    ...
                    if parts and "text" in parts[0]:
                        text = parts[0]["text"]
                        # Extract usage metadata if available (for metrics tracking)
                        usage = data.get("usageMetadata", {})
                        input_tokens = usage.get("promptTokenCount", 0)
                        output_tokens = usage.get("candidatesTokenCount", 0)
                        return FakeGeminiResponse(text, input_tokens, output_tokens)

            # Fallback: return empty response
            logger.warning("Unexpected response format from E2E server: %s", data)
            return FakeGeminiResponse("", 0, 0)
