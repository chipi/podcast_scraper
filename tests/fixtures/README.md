# Offline Podcast Fixtures (RSS + Transcripts + Audio)

> ## ⚠️ Which version — use **v3** (v1 and v2 are DEPRECATED)
>
> `transcripts/` and `audio/` are **versioned** (`v1/`, `v2/`, `v3/`). The current set is named
> in [`FIXTURES_VERSION`](./FIXTURES_VERSION) — today that is **`v3`**. **v1 and v2 are old and
> no longer used** (they each carry a `DEPRECATED` README). New work and all eval runs read
> `tests/fixtures/{transcripts,audio}/v3/`. **Always check `FIXTURES_VERSION` first** — do not
> assume a bare `transcripts/*.txt` path is current (that reflex analyses the wrong, dead set).
>
> Each **v3** episode has a co-located **ground-truth sidecar** `<name>.groundtruth.json`
> (speaker count, `has_commercial`, `type`, `failure_modes`) — the source of truth for the
> speaker / diarization eval. See [`transcripts/v3/README.md`](./transcripts/v3/README.md).

This directory contains **synthetic podcast fixtures** used for **offline,
deterministic end-to-end testing**.

These fixtures are intentionally designed to:

- avoid hitting real RSS feeds or audio hosting services
- mimic realistic podcast workloads (long audio, transcripts, metadata)
- be safe to commit to git
- be repeatable and regenerable if needed

These fixtures are **test infrastructure**, not editorial content.

---

## Folder layout

tests/fixtures/
├─ README.md              # this file (overview & navigation)
├─ FIXTURES_SPEC.md       # authoritative spec + metadata (source of truth)
├─ VERIFICATION.md        # how to verify fixtures after generation
├─ rss/                   # RSS2 + iTunes XML feeds
├─ transcripts/           # synthetic transcripts (input to audio generation)
├─ scripts/               # audio generation scripts
├─ audio/                 # generated MP3 files
└─ mock_server/           # mock SDK clients for E2E/integration testing

---

## Podcasts and RSS feeds

There are **9 synthetic podcasts** with varying episode counts:

### The nine shows (v3)

Every show now carries **four or more** episodes. The "3 episodes each" split
between standard and long-form shows was **v1**; v3 levelled the shows up and,
in doing so, lost the long-form content entirely — see "What v3 does not cover".

| show | subject | episodes | words (min–max) |
| --- | --- | --- | --- |
| `p01` | Singletrack Sessions — mountain biking | e01–e04 | 244 – 1,309 |
| `p02` | Practical Systems — software engineering | e01–e05 | 199 – 1,143 |
| `p03` | Below the Surface — scuba diving | e01–e04 | 245 – 927 |
| `p04` | Frame & Light — photography | e01–e04 | 870 – 984 |
| `p05` | Long Horizon Notes — investing | e01–e05 | 189 – 1,041 |
| `p06` | The Drift — edge cases | e01–e06 | 44 – 983 |
| `p07` | The Long View: Sustainability | e01–e04 | 239 – 977 |
| `p08` | Public Hour — public radio | e01–e04 | 545 – 792 |
| `p09` | Cross-Show — cross-podcast guests | e01–e04 | 589 – 629 |

**40 episodes across 9 shows.** `FIXTURES_SPEC.md` explains why you will also see
46, 38 and 36 quoted, and which of them means what.

### What v3 does not cover

Worth knowing before you design a test around these fixtures:

- **Nothing longer than 1,309 words (~8 minutes).** v1 had episodes of 14,471 and
  19,251 words; v3 has none above 1,309. Chunk-boundary behaviour, context-window
  pressure and cost-at-scale cannot be exercised here.
- **`long_context_chunk_boundary` is tagged on one episode, `p07_e01`, at 977
  words.** The tag is present; the condition it names is not.
- **42 of 46 transcripts have exactly two speakers.** Panels are thin (3 episodes
  with three speakers) and there is exactly one single-speaker episode.

---

## Naming and mapping conventions (critical)

All assets use a **shared deterministic ID**:
pXX_eYY

Where:

- `pXX` = podcast ID (`p01` .. `p09`)
- `eYY` = episode ID (`e01` .. `e06`)

This ID is used consistently across:

- RSS `<guid>`
- transcript filename
- audio filename

Example:
p02_e03
├─ RSS guid: p02_e03
├─ Transcript: transcripts/v3/p02_e03.txt
└─ Audio: audio/v3/p02_e03.mp3

---

## Episodes overview

**Two sets of feeds exist, and they advertise different things.** Read the column
you actually need — most confusion about "how many episodes are there" starts here.

| show | episodes on disk | `pNN_corpus.xml` advertises | legacy `rss/pNN_*.xml` advertises |
| --- | --- | --- | --- |
| p01 | e01–e04 | 4 | 3 (`p01_episode_selection.xml`) |
| p02 | e01–e05 | 5 | 3 (`p02_software.xml`) |
| p03 | e01–e04 | 4 | 3 (`p03_scuba.xml`) |
| p04 | e01–e04 | 4 | 3 (`p04_photo.xml`) |
| p05 | e01–e05 | 5 | 3 (`p05_investing.xml`) |
| p06 | e01–e06 | 6 | 6 (`p06_edge_cases.xml`) |
| p07 | e01–e04 | 4 | **1** (`p07_sustainability.xml`) |
| p08 | e01–e04 | 4 | **1** (`p08_solar.xml`) |
| p09 | e01–e04 | 4 | 3 (`p09_biohacking.xml`) |
| | **40** | **40** | **26** |

- **`pNN_corpus.xml`** (served with `--corpus`) is generated by
  `scripts/build_corpus_feeds.py` and lists every episode that has both a
  transcript and audio. It matches the disk exactly. **Use these** unless you
  specifically need the legacy shapes.
- **`rss/pNN_*.xml`** are the original hand-written feeds, deliberately frozen as
  an e2e test surface with fixed shapes (`FIXTURES_SPEC.md` §7). p07 and p08
  advertise one episode each, so a pipeline run against them produces one episode
  per show — correct for those tests, wrong as a description of the corpus.

## Files that are not episodes

Six files in `transcripts/v3/` look like episodes and are not. They are excluded
from every episode count, and they exist for good reasons:

| file(s) | what it is | why it exists |
| --- | --- | --- |
| `p01_e01_fast` | the first ~60s of `p01_e01` | cuts E2E-fast from ~180-240s to ~30-45s; counting it would count `p01_e01` twice |
| `p01_multi_e01`–`e05` | five ~40-word stubs | multi-feed connectivity fixture — they belong to no show's storyline and exercise feed handling, not content |

`46 files − these 6 = 40 episodes.` See `FIXTURES_SPEC.md` for the full
reconciliation of 46 / 40 / 38 / 36.

---

## Transcripts

Location:
tests/fixtures/transcripts/

Properties:

- plain text (`.txt`)
- fully **synthetic**
- conversational format
- includes speaker labels (e.g. `Maya:`, `Ethan:`)
- may include timestamps like `[00:00]`
- may include stage directions like `*Short break.*`

Target spoken duration:

- `e01`, `e02`: ~10–12 minutes (medium)
- `e03`: ~30–35 minutes (long)
- Long-form podcasts (p07-p09): ~20 minutes to 2+ hours

Actual duration depends on TTS voice and rate.

---

## Audio files

Location:
audio/

Files:
audio/pXX_eYY.mp3

Properties:

- generated locally from transcripts
- mono MP3
- realistic duration and file size
- suitable for streaming, partial reads, and timeout testing

### Fast Test Fixtures

For fast test execution, minimal fixtures are available:

- **Fast RSS Feed**: `rss/p01_fast.xml` - Single episode with 1-minute duration
- **Fast Transcript**: `transcripts/p01_e01_fast.txt` - First ~1 minute of p01_e01 transcript
- **Fast Audio**: `audio/p01_e01_fast.mp3` - 60-second audio file (469 KB vs 5.8 MB for full episode)

**Purpose**: Reduce E2E-fast test execution time from ~180-240 seconds to ~30-45 seconds by
using shorter audio files.

**Usage**: Automatically used by E2E-fast tests when `_allowed_podcasts` is set (fast mode
detection). Integration-fast tests use `/feed-no-transcript.xml` which serves the fast audio file.

**Generation**: Fast audio was created by extracting the first 60 seconds of `p01_e01.mp3` using ffmpeg.

### Multi-Episode Test Fixtures

For testing multi-episode processing logic, multi-episode test fixtures are available:

- **Multi-Episode RSS Feed**: `rss/p01_multi.xml` - 5 episodes with 10-15 second duration each
- **Multi-Episode Transcripts**: `transcripts/p01_multi_e01.txt` through `p01_multi_e05.txt` -
  Short transcripts (2-3 sentences each)

- **Multi-Episode Audio**: `audio/p01_multi_e01.mp3` through `p01_multi_e05.mp3` - Very short
  audio files (~50-100 KB each)

**Purpose**: Test episode iteration/looping logic, concurrent processing, job queues, and error
handling across multiple episodes without the overhead of long audio files. Total processing time:
~2-3 minutes for 5 episodes.

**Usage**: Used by E2E tests for multi-episode processing. These tests can process multiple
episodes (5 episodes) to validate multi-episode processing logic.

**Test Strategy**:

- **Fast Feed** (`p01_fast.xml`): 1 episode, 1 minute - for regular E2E tests (code quality)
- **Multi-Episode Feed** (`p01_multi.xml`): 5 episodes, 10-15 seconds each - for multi-episode processing tests
- **Episode selection E2E** (`p01_episode_selection.xml`): 3 short episodes, newest-first in the RSS document, each with a `podcast:transcript` URL (Path 1). Reuses `p01_multi_e05` / `e03` / `e01` audio and transcript files. On the E2E mock server this feed is exposed as **`podcast1_episode_selection`** (`e2e_server.urls.feed("podcast1_episode_selection")`). Used by `tests/e2e/test_episode_selection_e2e.py` and integration checks for GitHub #521 (`episode_order`, `--since` / `--until`, `episode_offset`).
- **Original Mock Data** (`p01_mtb.xml`, etc.): Full episodes - for nightly data quality tests
- **Long-Form Fixtures** (`p07`, `p08`, `p09`): Extra-long episodes (1-2+ hours) - for threshold boundary testing and long-context model evaluation (related to issue #283)

### Long-Form Test Fixtures (Threshold Boundary Testing)

For testing summarization threshold boundaries and long-context model behavior:

- **p07 - Sustainability**: 1 episode, **14,471 words**, ~1h 45m
  - Purpose: Test 3-4k combined summary token threshold (issue #283)
  - Expected: ~22 chunks → ~3,300 combined summary tokens

- **p08 - Solar Energy**: 1 episode, **19,251 words**, ~2h 21m
  - Purpose: Test >4k combined summary token threshold
  - Expected: ~30 chunks → ~4,500 combined summary tokens

- **p09 - Biohacking**: 3 episodes, **~3,000 words each**, ~20 min each
  - Purpose: Test medium-to-long episode processing
  - Expected: ~4-5 chunks per episode → ~600-750 combined tokens each

**Use cases:**

- Validate LED model vs BART/PEGASUS threshold behavior
- Test hierarchical reduce vs extractive fallback decision logic
- Verify compression ratio warnings are model-specific
- Ensure consistent quality across varying episode lengths

---

## Ground-truth sidecars — the fixture reality check

Every v3 episode has a sidecar `transcripts/v3/<name>.groundtruth.json` (generated by
`scripts/make_groundtruth.py`). **The sidecar is the full per-episode spec** — the single place
that says exactly what is inside an episode, so evals know what to score against:

- `speakers`, `num_human_speakers`, `type` (monologue/interview/panel)
- `has_commercial` / `num_ad_voices`, `expected_diarized_voices` (humans + ad = what a correct
  diarizer should DETECT)
- `failure_modes` (from the transcript's `#fixture-v3:` annotation)
- **`voice_map`** — every speaker (incl. `Ad`) → the `say` voice it is rendered with, straight from
  the one-voice-per-person map (see `FIXTURES_SPEC.md`). So the sidecar records exactly *who sounds
  like what*.
- **`cameo`** — when `cameo` is tagged: `{speaker, voice, turns}` of the brief 3rd voice.
- **`transcript_sha256` / `audio_sha256`** — reality-check hashes of the transcript and the audio.

**Rule (non-negotiable): whenever a transcript, the voice map, or an audio file changes, regenerate
the sidecars** (`python tests/fixtures/scripts/make_groundtruth.py`). They carry the new hashes.
`make_groundtruth.py --check` recomputes both hashes from disk and **fails if they drift** — so audio
or a transcript can never change without the sidecar being brought back in sync. If a hash changes
unexpectedly, open the sidecar and reconcile before doing anything else. `say` TTS is
non-deterministic, so regenerating audio always changes `audio_sha256`; that is expected — just
regenerate the sidecars in the same step.

---

## Curated diarization eval subset — always start from these 8

When evolving diarization / speaker work, **always run this curated 8-episode set first** before the
full corpus. It was hand-picked so 8 episodes pack every axis that matters — monologue / interview /
panel × ad × brief-voice × ASR garble × cross-show guest × non-NER host. Do not lose this curation;
it is the fast, high-signal gate (the current baseline is **3.1 = 8/8**).

| fixture | why it's in the set (the unique thing) |
| --- | --- |
| `p06_e05` | **monologue** — the single-speaker (1 voice) baseline |
| `p01_e03` | **plain interview + ad** — the vanilla 3-voice case (Maya + Noah Bryer + Ad) |
| `p05_e04` | **panel** guardrail — 3 real humans, must stay == 3 (Nora + Daniel Cho + Scott Bessent) |
| `p08_e04` | **zero_host_ner** — non-NER host "A. correspondent" + Renee + ad; caught the period-name undercount bug |
| `p09_e01` | **cross-show cast** — Sam + Dr. Elena Fischer (Elena also appears in p07) |
| `p09_e02` | **multi_accent + asr_garble + recurring_guest** — Sam (en-US) + Skanda (en-IN) + ad |
| `p09_e03` | **asr_garble_severe** — the hard same-gender merge case; the sensitivity anchor (3.1 merged it until the one-voice-per-person remap fixed it) |
| `p01_multi_e05` | **brief-voice / short clip** — a 4-word guest turn in a ~10s clip; the sub-second-voice stress case |

Plus the two **designed cameo** fixtures (`p02_e05` caller, `p05_e05` archival clip): a genuine
~3s brief 3rd voice. Both 3.1 and community-1 merge it — they document the model-independent
brief-voice detection floor (see `#1170`).

The set's expected counts + per-fixture properties are the `expected_diarized_voices` /
`voice_map` / `cameo` fields in each sidecar above.

---

## Audio generation

Audio is generated **offline** using macOS built-in TTS.

### Engine

- Platform: macOS
- TTS engine: `say`
- No online services used

### Voices

- **Host voice:** Samantha
- **Guest voice:** Daniel

### Parameters

- Speech rate: `145`
- MP3 bitrate: `64k`
- Channels: mono

### Voice assignment logic

- Host name is inferred from filename prefix:
  - `p01_*` → Maya
  - `p02_*` → Ethan
  - `p03_*` → Rina
  - `p04_*` → Leo
  - `p05_*` → Nora
  - `p06_*` → TBD (edge cases)
  - `p07_*` → Alex (The Long View - Sustainability)
  - `p08_*` → Alex (The Long View - Solar Energy)
  - `p09_*` → Alex (The Long View - Biohacking)
- When a transcript line speaker matches the host name → host voice
- All other speakers → guest voice
- Timestamps, headings, and stage directions are removed from speech

---

## Generation scripts

Scripts:

- `fixtures/scripts/transcripts_to_mp3.py` — main generator
- `fixtures/scripts/generate_audio.sh` — wrapper with defaults

Generate all audio (from repo root):

```bash
chmod +x tests/fixtures/scripts/generate_audio.sh
./tests/fixtures/scripts/generate_audio.sh
```
