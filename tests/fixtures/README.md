...
## Ground-truth sidecars — the fixture reality check

Every v3 episode has a sidecar `transcripts/v3/<name>.groundtruth.json` (generated by
`scripts/make_groundtruth.py`). **The sidecar is the full per-episode spec** — the single place
that says exactly what is inside an episode, so evals know what to score against:

- `speakers`, `num_human_speakers`, `type` (monologue/interview/panel)
- `has_commercial` / `num_ad_voices`, `expected_diarized_voices` (humans + ad = what a correct
  diarizer should DETECT)
- `failure_modes` (from the transcript's `#fixture-v3:` annotation)
- **`voice_map`** — every speaker (incl. `Ad`) → the `say` voice it is rendered with, straight from
  the one-voice-per-person map (see `FIXTURES_SPEC.md`). So the sidecar records exactly *who sounds
  like what*.
- **`cameo`** — when `cameo` is tagged: `{speaker, voice, turns}` of the brief 3rd voice.
- **`transcript_sha256` / `audio_sha256`** — reality-check hashes of the transcript and the audio.

**Rule (non-negotiable): whenever a transcript, the voice map, or an audio file changes, regenerate
the sidecars** (`python tests/fixtures/scripts/make_groundtruth.py`). They carry the new hashes.
`make_groundtruth.py --check` recomputes both hashes from disk and **fails if they drift** — so audio
or a transcript can never change without the sidecar being brought back in sync. If a hash changes
unexpectedly, open the sidecar and reconcile before doing anything else. `say` TTS is
non-deterministic, so regenerating audio always changes `audio_sha256`; that is expected — just
regenerate the sidecars in the same step.

---

## Curated diarization eval subset — always start from these 8

When evolving diarization / speaker work, **always run this curated 8-episode set first** before the
full corpus. It was hand-picked so 8 episodes pack every axis that matters — monologue / interview /
panel × ad × brief-voice × ASR garble × cross-show guest × non-NER host. Do not lose this curation;
it is the fast, high-signal gate (the current baseline is **3.1 = 8/8**).

| fixture | why it's in the set (the unique thing) |
| --- | --- |
| `p06_e05` | **monologue** — the single-speaker (1 voice) baseline |
| `p01_e03` | **plain interview + ad** — the vanilla 3-voice case (Maya + Noah Bryer + Ad) |
| `p05_e04` | **panel** guardrail — 3 real humans, must stay == 3 (Nora + Daniel Cho + Scott Bessent) |
| `p08_e04` | **zero_host_ner** — non-NER host "A. correspondent" + Renee + ad; caught the period-name undercount bug |
| `p09_e01` | **cross-show cast** — Sam + Dr. Elena Fischer (Elena also appears in p07) |
| `p09_e02` | **multi_accent + asr_garble + recurring_guest** — Sam (en-US) + Skanda (en-IN) + ad |
| `p09_e03` | **asr_garble_severe** — the hard same-gender merge case; the sensitivity anchor (3.1 merged it until the one-voice-per-person remap fixed it) |
| `p01_multi_e05` | **brief-voice / short clip** — a 4-word guest turn in a ~10s clip; the sub-second-voice stress case |

Plus the two **designed cameo** fixtures (`p02_e05` caller, `p05_e05` archival clip): a genuine
~3s brief 3rd voice. Both 3.1 and community-1 merge it — they document the model-independent
brief-voice detection floor (see `#1170`).

The set's expected counts + per-fixture properties are the `expected_diarized_voices` /
`voice_map` / `cameo` fields in each sidecar above.
