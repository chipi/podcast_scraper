...
answers depending on which layer is asking. Anyone quoting one without saying
which will eventually be contradicted by a build:

| count | what it means | how to reproduce it |
| --- | --- | --- |
| **46** | `.txt` files in `transcripts/v3/` | `ls transcripts/v3/*.txt \| wc -l` |
| **40** | real episodes: canonical `pNN_eNN` with both transcript and audio; what `build_corpus_feeds.py` advertises | 46 − `p01_e01_fast` − 5 × `p01_multi_e*` |
| **38** | generated by `scripts/build_v3_fixtures.py` — the p01–p09 core | 40 − `p06_e05` − `p06_e06`, which are standalone |
| **36** | episodes in the BUILT app-validation corpus | 40 − the 4 the per-feed cap drops (below) |

The 36 is not a property of the fixtures — it is the output of a flag.
`build_app_validation_corpus.py` and `build_synthetic_validation_corpus.py` both
default `--max-episodes-per-feed 4`, and three shows exceed that: `p02` and `p05`
have 5, `p06` has 6. So `p02_e05`, `p05_e05`, `p06_e05` and `p06_e06` have
transcript, audio and ground truth, are advertised by the corpus feeds, and are
never built. §15 records that as a defect in the layer disagreement; it is listed
here so the number itself stops being a surprise.

**Raise the cap and the count changes.** Any test, baseline or doc that hardcodes
