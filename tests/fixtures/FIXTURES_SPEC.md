...
- **v3** (**current default**) — the diarization/speaker + enrichment corpus.
  ...
  Each `.txt` also has a `.vtt` twin, generated by
  `tests/fixtures/scripts/transcripts_to_vtt.py`, carrying the speaker of every
  turn as a WebVTT `<v Speaker>` voice span. The feeds advertise the `.vtt` —
  including the generated `pNN_corpus.xml`, which did NOT until 2026-09-24.
  `build_corpus_feeds.py` emitted no `<podcast:transcript>` at all, so only three
  hand-authored p01 feeds carried one, and a pipeline run over the corpus feeds
  with `transcribe_missing: false` produced NOTHING while exiting 0 — 40 episodes,
  40 lines of `no transcript for: <title>`, `summary:0`.
  This is what lets an episode taking the transcript-download fast path produce a
  **placed** roster: a transcript that names its own turns is a diarization the
  pipeline does not have to compute, so stack-test exercises speaker attribution
  without ever running Whisper or pyannote. Plain `.txt` remains served and
  supported — an unlabelled transcript behaves exactly as before.

  Two things the `.vtt` twins deliberately do NOT do. Cue times are **synthesised**
  from the `[MM:SS]` block markers and text length, so they are plausible and
  ordered but not measured against the audio. And sponsor reads (`Ad:` in the
  `.txt`) keep their text but carry **no** voice span: `groundtruth.json` counts
  the ad as a voice (`num_ad_voices`) and never as a `speakers` entry, so
  emitting `<v Ad>` would author a person nobody is. Ad *detection* is unaffected
  — it reads the text, and fires on all 30 ad-bearing fixtures from the VTT-derived
  transcript.
...
feed-level metadata, so nothing ever checked that the feeds listed the corpus's episodes:
    ...
    corpus:         9 shows x 4 episodes = 36      <- the state ON 2026-08-16
...
(#1386) rejected any summary containing `"braking earlier"`.

**That fragment rule is gone (2026-08-16).** It could not tell a correct summary of
a braking episode from a copied one, so `p01_e02` lost its summary on every attempt
and the committed corpus carried a stand-in for it. The guard now measures
near-verbatim reuse of a whole example SENTENCE — contiguous lifted run ≥ 5 words
covering ≥ 50% of the line — and keeps the fragments only as a cheap pre-filter.
Under that rule `p01_e02` has a real summary again, which is why the corpus no
longer has a "known exception". Verified against
`_looks_copied_from_example`: the episode's summary returns `None`, the verbatim
style example returns coverage `1.0`.
...
- **FIXED 2026-09-24.** `_RUN_TAG` was `run_20260101_000000` (underscore) but `corpus_scope._RUN_TS_RE` expects the dash
...
### Three episode counts, all correct

The corpus grew by accretion — shows started at four episodes each and picked up
extras for specific tests — so "how many episodes are there" has more than one
right answer depending on which layer is asking:

| count | what it means | how to reproduce it |
| --- | --- | --- |
| **46** | `.txt` files in `transcripts/v3/` | `ls transcripts/v3/*.txt \| wc -l` |
| **40** | real episodes: canonical `pNN_eNN` with both transcript and audio — what the corpus feeds advertise AND what the built corpus contains | 46 − `p01_e01_fast` − 5 × `p01_multi_e*` |
| **38** | generated by `scripts/build_v3_fixtures.py` — the p01–p09 core | 40 − `p06_e05` − `p06_e06`, which are hand-written |

**There used to be a fourth: 36.** `build_app_validation_corpus.py` and
`build_synthetic_validation_corpus.py` both defaulted `--max-episodes-per-feed 4`,
which was a no-op while every show had four episodes and then silently started
excluding when `p02`/`p05` reached five and `p06` six. Four episodes with
transcript, audio and ground truth sat in no build — including the corpus's only
single-speaker episode and its only code-switching one.

The default is gone (the flag remains, for deliberately building something
smaller), so the built corpus is now the corpus: **40 either way.** A test or doc
still asserting 36 is asserting a flag value that no longer exists.
...
| episode | words | what it is for |
| --- | --- | --- |
| `p02_e05` | 199 | **cameo** — listener call-in; a very short item inside a show whose others are ~1,000w |
| `p05_e05` | 189 | **cameo** — archival clip; same shape, different show |
| `p06_e05` | 44 | **stage directions** (`*Theme music plays.*`) and the **only single-speaker episode in the corpus** — `tests/e2e/test_full_pipeline_e2e.py:347` records that no feed serves it on its own |
| `p06_e06` | 46 | **code-switching** — the only mixed-language episode ("Hola, ¿cómo estás?"), for transcription robustness |
| `p01_multi_e01`–`e05` | 38–43 | multi-feed connectivity fixtures, NOT episodes; excluded from every count above except 46 |
| `p01_e01_fast` | — | first ~60s of `p01_e01`, for E2E-fast; a variant of an episode, not one |

Those four are the corpus's only sub-200-word coverage. Drop them and the 36 that
remain span 239w (`p07_e04`) to 1,309w (`p01_e01`) — still short, but never short
enough for a summary to come out longer than its input.

Length is the weaker reason to keep them. The stronger one is that `p06_e05` and
`p06_e06` are the **sole carriers** of single-speaker, stage-direction and
code-switching coverage. All four are over the per-feed cap, so those modes are
covered on disk and exercised by no build — the state where a corpus reports
...
- e05 — *Stage Directions* (Maya, solo) — the corpus's **only single-speaker episode**, and its only
  test of stage-direction parsing. Hand-written; `build_v3_fixtures.py` does not produce it.
- e06 — *Mixed Languages* (Maya, Liam) — the corpus's **only code-switching episode**. Hand-written.

`p06` is the only show with six episodes, so it is also the show the default
`--max-episodes-per-feed 4` truncates hardest: e05 and e06 are advertised by the
corpus feed and never built.
