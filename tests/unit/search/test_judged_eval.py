"""Unit tests for the judged hybrid-vs-baseline eval harness (RFC-057 / C)."""
...
def test_build_template_unions_candidates_with_per_backend_ranks():
    baseline = {"q": [("a", "text-a"), ("b", "text-b")]}
    hybrid = {"q": [("b", "text-b"), ("c", "text-c")]}
    records = build_judgment_template(
        ["q"],
        baseline_ranks=lambda q: baseline[q],
    ...
    assert rec.baseline_ranking == ["a", "b"] and rec.hybrid_ranking == ["b", "c"]
    cand = {c["doc_id"]: c for c in rec.candidates}
    assert set(cand) == {"a", "b", "c"}  # unioned
    assert cand["a"]["baseline_rank"] == 1 and cand["a"]["hybrid_rank"] is None
    assert cand["b"]["baseline_rank"] == 2 and cand["b"]["hybrid_rank"] == 1


def test_score_rewards_ranking_relevant_doc_higher():
    # Same relevant doc "good" (grade 2); hybrid ranks it #1, baseline ranks it #3.
    rec = JudgmentRecord(
        query="q",
        intent="semantic",
        baseline_ranking=["x", "y", "good"],
        hybrid_ranking=["good", "x", "y"],
        relevance={"good": 2, "x": 0, "y": 0},
    )
    scores = score_from_judgments([rec], k=10)
    assert scores["hybrid"]["ndcg"] > scores["baseline"]["ndcg"]
