...
from podcast_scraper import cache_manager
...
class TestGetWhisperCacheInfo(unittest.TestCase):
    ...
    @patch("podcast_scraper.cache_manager.get_whisper_cache_dir")
...
class TestGetTransformersCacheInfo(unittest.TestCase):
    ...
    @patch("podcast_scraper.cache_manager.get_transformers_cache_dir")
...
class TestGetSpacyCacheInfo(unittest.TestCase):
    ...
    @patch("podcast_scraper.cache_manager.get_spacy_cache_dir")
...
class TestCacheManagerEdgeCases(unittest.TestCase):
    ...
    @patch("podcast_scraper.cache_manager.get_whisper_cache_info")
    def test_clean_whisper_cache_no_confirm(self, mock_get_info):
        """Test clean_whisper_cache with confirm=False (no prompt)."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            model_file = cache_dir / "test_model.pt"
            model_file.write_bytes(b"x" * 100)

            mock_get_info.return_value = (
                cache_dir,
                100,
                [{"name": "test_model.pt", "path": model_file, "size": 100}],
            )
    ...
    @patch("podcast_scraper.cache_manager.get_whisper_cache_info")
    def test_clean_whisper_cache_directory_model(self, mock_get_info):
        """Test clean_whisper_cache with directory-based model."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            ...
            mock_get_info.return_value = (
                cache_dir,
                100,
                [{"name": "model_dir", "path": model_dir, "size": 100}],
            )
    ...
    @patch("podcast_scraper.cache_manager.get_transformers_cache_info")
    def test_clean_transformers_cache_confirmed(self, mock_get_info, mock_input):
        """Test clean_transformers_cache with confirmation."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            model_dir = cache_dir / "models--facebook--bart-base"
            model_dir.mkdir()
            (model_dir / "config.json").write_text("{}")
            ...
            mock_get_info.return_value = (
                cache_dir,
                100,
                [{"name": "models--facebook--bart-base", "path": model_dir, "size": 100}],
            )

            deleted, freed = cache_manager.clean_transformers_cache(confirm=True)
    ...
    @patch("podcast_scraper.cache_manager.get_transformers_cache_info")
    def test_clean_transformers_cache_cancelled(self, mock_get_info, mock_input):
        """Test clean_transformers_cache cancelled by user."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            model_dir = cache_dir / "models--facebook--bart-base"
            model_dir.mkdir()
    ...
    @patch("builtins.input", return_value="yes")
    @patch("podcast_scraper.cache_manager.get_spacy_cache_info")
    def test_clean_spacy_cache_confirmed(self, mock_get_info, mock_input):
        """Test clean_spacy_cache with confirmation."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            ...
            mock_get_info.return_value = (
                cache_dir,
                100,
                [{"name": "en_core_web_sm", "path": model_dir, "size": 100}],
            )

            deleted, freed = cache_manager.clean_spacy_cache(confirm=True)
    ...
    @patch("builtins.input", return_value="no")
    @patch("podcast_scraper.cache_manager.get_spacy_cache_info")
    def test_clean_spacy_cache_cancelled(self, mock_get_info, mock_input):
        """Test clean_spacy_cache cancelled by user."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            model_dir = cache_dir / "en_core_web_sm"
            model_dir.mkdir()
    ...
    @patch("podcast_scraper.cache_manager.get_transformers_cache_info")
    def test_clean_transformers_cache_permission_error(self, mock_get_info):
        """Test clean_transformers_cache handles permission errors."""
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir)
            model_dir = cache_dir / "models--facebook--bart-base"
            model_dir.mkdir()
