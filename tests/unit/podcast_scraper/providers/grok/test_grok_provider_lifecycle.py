#!/usr/bin/env python3
"""Lifecycle and edge case tests for GrokProvider.
...
@pytest.mark.unit
class TestGrokProviderLifecycle(unittest.TestCase):
    ...
    def test_multiple_initialize_calls_idempotent(self, mock_openai):
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_cleanup_when_not_initialized(self, mock_openai):
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_partial_initialization_cleanup(self, mock_openai):
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_requires_separate_instances_false(self, mock_openai):
        """Test that GrokProvider does not require separate instances."""
        mock_client = Mock()
        mock_openai.return_value = mock_client

        provider = GrokProvider(self.cfg)

        # GrokProvider should not require separate instances (API clients are thread-safe)
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_requires_separate_instances_accessible_via_getattr(self, mock_openai):
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_state_after_cleanup(self, mock_openai):
    ...
    @patch("podcast_scraper.providers.grok.grok_provider.OpenAI")
    def test_client_persists_after_cleanup(self, mock_openai):
        """Test that Grok (OpenAI) client persists after cleanup (can be reused)."""
        mock_client = Mock()
        mock_openai.return_value = mock_client
