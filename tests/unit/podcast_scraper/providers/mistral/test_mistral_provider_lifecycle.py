#!/usr/bin/env python3
"""Lifecycle and edge case tests for MistralProvider.
...
class TestMistralProviderLifecycle(unittest.TestCase):
    ...
    def test_multiple_initialize_calls_idempotent(self, mock_mistral):
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_cleanup_when_not_initialized(self, mock_mistral):
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_partial_initialization_cleanup(self, mock_mistral):
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_requires_separate_instances_false(self, mock_mistral):
        """Test that MistralProvider does not require separate instances."""
        mock_client = Mock()
        mock_mistral.return_value = mock_client

        provider = MistralProvider(self.cfg)

        # MistralProvider should not require separate instances (API clients are thread-safe)
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_requires_separate_instances_accessible_via_getattr(self, mock_mistral):
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_state_after_cleanup(self, mock_mistral):
    ...
    @patch("podcast_scraper.providers.mistral.mistral_provider.Mistral")
    def test_client_persists_after_cleanup(self, mock_mistral):
        """Test that Mistral client persists after cleanup (can be reused)."""
        mock_client = Mock()
        mock_mistral.return_value = mock_client
