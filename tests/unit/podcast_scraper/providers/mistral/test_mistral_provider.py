...
@pytest.mark.unit
class TestMistralSummarizeBundled(unittest.TestCase):
    ...
    def _make_provider(self):
        with patch("podcast_scraper.providers.mistral.mistral_provider.Mistral") as mock_cls:
    ...
    def test_bundled_success_returns_expected_shape(self, mock_render, mock_meta):
        ...
        provider.client.chat.complete.return_value = self._mock_response(self.valid_json)
    ...
    def test_bundled_rejects_missing_summary_field(self, mock_render):
        ...
        provider.client.chat.complete.return_value = self._mock_response(bad_json)
    ...
    def test_bundled_rejects_invalid_json(self, mock_render):
        mock_render.side_effect = ["System", "User"]
        provider = self._make_provider()
        provider.client.chat.complete.return_value = self._mock_response("not json at all")
    ...
    def test_bundled_records_pipeline_metrics(self, mock_render, mock_meta):
        ...
        mock_meta.return_value = {"name": "test", "sha256": "abc"}
        provider = self._make_provider()
        provider.client.chat.complete.return_value = self._mock_response(
    ...
    def test_bundled_uses_provider_prefixed_prompt_names(self, mock_render, mock_meta):
        ...
        self.assertTrue(
            any("mistral/summarization/bundled" in c for c in calls),
            f"Expected mistral-prefixed prompt, got: {calls}",
        )
