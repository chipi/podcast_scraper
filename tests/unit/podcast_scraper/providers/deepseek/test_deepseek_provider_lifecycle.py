#!/usr/bin/env python3
"""Lifecycle and edge case tests for DeepSeekProvider.
...
class TestDeepSeekProviderLifecycle(unittest.TestCase):
    ...
    def test_requires_separate_instances_false(self, mock_openai):
        """Test that DeepSeekProvider does not require separate instances."""
        mock_client = Mock()
        mock_openai.return_value = mock_client

        provider = DeepSeekProvider(self.cfg)

        # DeepSeekProvider should not require separate instances (API clients are thread-safe)
    ...
    def test_client_persists_after_cleanup(self, mock_openai):
        """Test that DeepSeek (OpenAI) client persists after cleanup (can be reused)."""
        mock_client = Mock()
        mock_openai.return_value = mock_client
