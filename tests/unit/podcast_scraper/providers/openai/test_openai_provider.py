...
@pytest.mark.unit
class TestOpenAISummarizeBundled(unittest.TestCase):
    ...
    def test_bundled_uses_provider_prefixed_prompt_names(self, mock_render, mock_meta):
        ...
        self.assertTrue(
            any("openai/summarization/bundled" in c for c in calls),
            f"Expected openai-prefixed prompt name, got: {calls}",
        )
