...
class TestAnthropicProviderE2E:
    ...
    def test_anthropic_hybrid_cleaning_strategy(self, e2e_server: Optional[Any]):
        ...
        try:
            ...
            # Create config with Anthropic summarization and hybrid cleaning strategy
            # Note: Anthropic does NOT support transcription, so we use Whisper
            config_kwargs = {
                "rss_url": rss_url,
                "output_dir": temp_dir,
                "transcription_provider": "whisper",  # Anthropic doesn't support transcription
            ...
            _save_all_episode_responses(
                Path(temp_dir),
                metadata_files,
                "test_anthropic_hybrid_cleaning_strategy",
                validate_provider="anthropic",
