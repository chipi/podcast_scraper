"""E2E tests for insight clustering with real sentence-transformers model (#599, #601).

These tests use the real all-MiniLM-L6-v2 embedding model (pre-cached via
make preload-ml-models). They validate the full clustering flow end-to-end.
"""
...
# Require sentence-transformers (part of GIL evidence models, preloaded on CI)
try:
...
def test_cluster_browse_on_real_clusters(tmp_path: Path, capsys) -> None:
    ...
    rc = run_cluster_browse_cli(args, logging.getLogger(__name__))
    assert rc == 0
...
def test_topic_insights_on_real_data(tmp_path: Path, capsys) -> None:
    ...
    rc = run_topic_insights_cli(args, logging.getLogger(__name__))
    assert rc == 0
    out = json.loads(capsys.readouterr().out)
