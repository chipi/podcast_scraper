...
class TestMistralProviderE2E:
    ...
    def test_mistral_full_pipeline(self, e2e_server: Optional[Any]):
        ...
        try:
            ...
            # #650/#651 cost assertions: Mistral powers transcription +
            # speaker + summarization (rare — 1 of 2 providers besides OpenAI
            # that supports all three). All billable.
    ...
    def test_mistral_mega_bundled_pipeline_records_cost(self, e2e_server: Optional[Any]):
        """#650/#651 mega_bundled parity for Mistral."""
        temp_dir = tempfile.mkdtemp()
        try:
            rss_url, mistral_api_base, mistral_api_key = _get_test_feed_url(e2e_server)
            cfg = create_test_config(
