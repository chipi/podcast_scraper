...
def _save_all_episode_responses(
    ...
    """Save Grok API responses for all episodes processed in a test.

    All episodes from the same test run are saved to the same run folder,
    with one file per episode (e.g., grok-responses_ep01.txt, grok-responses_ep02.txt).

    Args:
        temp_dir: Temporary directory where test output was written
        metadata_files: List of metadata file paths (one per episode)
        test_name: Name of the test (e.g., "test_grok_full_pipeline")
        validate_provider: Optional provider name to validate (e.g., "grok")
    ...
    for idx, metadata_file in enumerate(sorted(metadata_files), 1):
        ...
        # Print summary of Grok APIs and models used for this episode
        if len(metadata_files) > 1:
            print(f"\nEpisode {idx}: {_summarize_grok_usage(metadata_content)}")
        else:
            print(f"\n{_summarize_grok_usage(metadata_content)}")

        # Save actual API responses (speakers, summary) to file
        episode_suffix = f"_ep{idx:02d}" if len(metadata_files) > 1 else ""
        response_file = _save_grok_responses(
            temp_dir,
            metadata_content,
    ...
    if len(saved_files) > 1:
        print(f"\n📁 Saved {len(saved_files)} Grok API response files:")
        for idx, file_path in enumerate(saved_files, 1):
            print(f"  {idx}. {file_path}")
    elif len(saved_files) == 1:
        print(f"📁 Grok API responses saved to: {saved_files[0]}")
...
def _save_grok_responses(  # noqa: C901
    temp_dir: Path,
    ...
    """Save actual Grok API responses (speakers, summary) to a file.

    Args:
        temp_dir: Temporary directory where test output was written
        metadata_content: Parsed metadata JSON content
        test_name: Name of the test (e.g., "test_grok_full_pipeline_ep01")
    ...
    if episode_suffix_match:
        ...
        filename = f"grok-responses_ep{episode_num}.txt"
    else:
        filename = "grok-responses.txt"
    ...
    while output_file.exists():
        filename_with_counter = f"grok-responses_{counter}.txt"
    ...
    response_lines.append("Grok API Responses")
    response_lines.append("=" * 80)
    ...
    # Note about Grok capabilities
    response_lines.append("")
    response_lines.append("Note: Grok does NOT support transcription (no audio API)")
    response_lines.append("      Grok is used for speaker detection and summarization only")
    response_lines.append("")
    response_lines.append("=" * 80)
    response_lines.append("")
...
class TestGrokProviderE2E:
    ...
    def test_grok_hybrid_cleaning_strategy(self, e2e_server: Optional[Any]):
        """Test Grok provider with hybrid cleaning strategy (pattern + conditional LLM).
        ...
        try:
            ...
            # Create config with Grok summarization and hybrid cleaning strategy
            # Note: Grok does NOT support transcription, so we use Whisper
            config_kwargs = {
                "rss_url": rss_url,
                "output_dir": temp_dir,
                "transcription_provider": "whisper",  # Grok doesn't support transcription
                "speaker_detector_provider": "grok",
                "summary_provider": "grok",
                "grok_speaker_model": "grok-3-mini",  # Use grok-3-mini (faster, cheaper)
                "grok_summary_model": "grok-3-mini",  # Use grok-3-mini (faster, cheaper)
            ...
            _save_all_episode_responses(
                Path(temp_dir),
                metadata_files,
                "test_grok_hybrid_cleaning_strategy",
                validate_provider="grok",
