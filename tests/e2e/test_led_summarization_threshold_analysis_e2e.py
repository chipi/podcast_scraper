"""LED Summarization Threshold Analysis Tool (E2E).
...
**NOT a regular test** - This is marked with @pytest.mark.analysis and will be skipped
in regular test runs. Use it explicitly when investigating threshold-related issues:

    pytest tests/e2e/test_led_summarization_threshold_analysis_e2e.py -v -s -m analysis
...
@pytest.mark.slow
@pytest.mark.analysis  # Skip in regular test runs - this is a diagnostic tool
class TestLEDSummarizationThresholdAnalysis:
    ...
    def test_p07_under_threshold_analysis(self, e2e_server):
        ...
        print(f"Checking MAP model: {config.TEST_DEFAULT_SUMMARY_MODEL}")
        print(f"  → Resolves to: {DEFAULT_SUMMARY_MODELS[config.TEST_DEFAULT_SUMMARY_MODEL]}")
        require_transformers_model_cached(config.TEST_DEFAULT_SUMMARY_MODEL, None)
        print("✅ MAP model cache check passed")
        print()

        print(f"Checking REDUCE model: {config.TEST_DEFAULT_SUMMARY_REDUCE_MODEL}")
        print(
            f"  → Resolves to: {DEFAULT_SUMMARY_MODELS[config.TEST_DEFAULT_SUMMARY_REDUCE_MODEL]}"
        )
        ...
        logger = logging.getLogger("podcast_scraper.summarization")
