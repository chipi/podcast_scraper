...
class TestMetadataGeneration(unittest.TestCase):
    ...
    def test_generate_metadata_skip_existing(self):
        """Test that metadata generation respects skip_existing."""
        cfg = create_test_config(
            output_dir=self.temp_dir,
            generate_metadata=True,
            skip_existing=True,
        ...
        # Try to generate again with skip_existing
        metadata_path2 = metadata.generate_episode_metadata(
            feed=self.feed,
            episode=self.episode,
            feed_url=TEST_FEED_URL,
            cfg=cfg,
        ...
        self.assertIsNone(metadata_path2)
    ...
    def test_generate_metadata_with_run_suffix(self):
    ...
    def test_generate_metadata_minimal_data(self):
        """Test metadata generation with minimal episode data."""
        minimal_episode = create_test_episode(
            title="Minimal Episode",
            title_safe="Minimal_Episode",
            transcript_urls=[],
            media_url=None,
            media_type=None,
        )
    ...
    def test_generate_metadata_transcript_urls(self):
        """Test that transcript URLs are included in metadata."""
        ...
        transcript_urls = data["content"]["transcript_urls"]
        self.assertEqual(len(transcript_urls), 2)
        self.assertEqual(transcript_urls[0]["url"], TEST_TRANSCRIPT_URL)
        self.assertEqual(transcript_urls[0]["type"], TEST_TRANSCRIPT_TYPE_VTT)
        self.assertIn("transcript_id", transcript_urls[0])
