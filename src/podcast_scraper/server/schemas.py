...
class AppInsight(BaseModel):
    ...
    # ADR-133/#1191 route-and-tag: surfaces sort by salience desc and may cap at
    # gi_surface_default_limit. Optional so pre-3.1 artifacts (no fields) still project.
    salience: float | None = Field(
        default=None,
        description="ADR-133/#1191 ranking score (tier x grounded x surfaceable); higher = first.",
    )
    rank: int | None = Field(
    ...
    routing_tag: str | None = Field(
        default=None,
        description="surface | connect | drop (ADR-133/#1191); 'drop' is filtered server-side.",
    )
    tier: int | None = Field(
