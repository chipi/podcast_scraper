...
class MistralProvider:
    ...
    def generate_insights(
        ...
        try:
            ...
            user_prompt = render_prompt(
                f"mistral/insight_extraction/{prompt_version}",
    ...
    def classify_insights(self, insights: List[str]) -> List[int]:
        ...
        if not insights:
            return []
        if not self._summarization_initialized:
            raise RuntimeError("Mistral summarization not initialized for classify_insights")
        ...
        user_prompt = render_prompt("mistral/insight_value_gate/v1", insights=listing)
        gate_max_tokens = max(
            config_constants.GI_INSIGHT_TOKENS_FLOOR,
    ...
    def complete_text(self, prompt: str) -> str:
        ...
        """
        if not self._summarization_initialized:
            raise RuntimeError("Mistral summarization not initialized for complete_text")
    ...
    def score_entailment(
        ...
        **kwargs: Any,
        ...
        system, user = render_entailment_prompt("mistral", premise, hypothesis)
        try:
            from ...utils.provider_metrics import (
                _safe_mistral_retryable,
                apply_gil_evidence_llm_call_metrics,
    ...
    def extract_quotes_bundled(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.debug("Mistral extract_quotes_bundled parse failed: %s", exc)
            raise
    ...
    def _score_entailment_bundled_chunk(
        ...
        try:
            ...
            logger.debug("Mistral score_entailment_bundled parse failed: %s", exc)
            raise
