...
def _record_deepseek_llm_call(
    ...
    *,
    ...
    """Record tokens + cost_usd for a DeepSeek LLM call into pipeline_metrics.

    Mirrors OpenAI wiring (#650/#651). DeepSeek uses an OpenAI-compatible SDK
    so usage lives at ``response.usage.prompt_tokens`` / ``.completion_tokens``.
    """
...
class DeepSeekProvider:
    ...
    def extract_quotes_bundled(
        ...
        **kwargs: Any,
        ...
        """Bundle ``extract_quotes`` (#698 Layer A) — DeepSeek (OpenAI-compat)."""
        ...
        try:
            ...
            logger.debug("DeepSeek extract_quotes_bundled parse failed: %s", exc)
            raise
    ...
    def score_entailment_bundled(
        ...
        **kwargs: Any,
        ...
        """Bundle ``score_entailment`` (#698 Layer B) — DeepSeek."""
    ...
    def _score_entailment_bundled_chunk(
        ...
        try:
            ...
            logger.debug("DeepSeek score_entailment_bundled parse failed: %s", exc)
            raise
