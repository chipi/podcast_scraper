...
class GrokProvider:
    ...
    def generate_insights(
        ...
        Uses grok/insight_extraction/v2 prompt; parses response as one insight per line.
        ...
        try:
            ...
            user_prompt = render_prompt(
                f"grok/insight_extraction/{prompt_version}",
    ...
    def classify_insights(self, insights: List[str]) -> List[int]:
        ...
        if not insights:
            return []
        if not self._summarization_initialized:
            raise RuntimeError("Grok summarization not initialized for classify_insights")
        ...
        user_prompt = render_prompt("grok/insight_value_gate/v1", insights=listing)
        gate_max_tokens = max(
            config_constants.GI_INSIGHT_TOKENS_FLOOR,
        ...
        _guardrails.check_chat_response(content, service="grok", expect_json=True)
    ...
    def complete_text(self, prompt: str) -> str:
        ...
        """
        if not self._summarization_initialized:
            raise RuntimeError("Grok summarization not initialized for complete_text")
    ...
    def score_entailment(
        ...
        **kwargs: Any,
        ...
        system, user = render_entailment_prompt("grok", premise, hypothesis)
        try:
            from ...utils.provider_metrics import (
                _safe_openai_retryable,
                apply_gil_evidence_llm_call_metrics,
