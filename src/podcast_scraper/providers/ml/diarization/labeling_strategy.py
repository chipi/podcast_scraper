"""Provider-specific speaker-labeling strategies (ADR-132).
...
sin ADR-132 fixes is provider-coupling disguised as generic logic. Deepgram is the legacy/frozen
...
class DiarizationLabelingStrategy:
    ...
    def recorded_voices(
        ...
        """Voices whose turns are recurring feed boilerplate (ads/promos), not people (ADR-132)."""
    ...
    def snap_extra(self, name: str, known_hosts: Sequence[str]) -> Optional[str]:
        """Provider-specific fallback to resolve a garbled host name; base has none (ADR-132)."""
...
class Community1LabelingStrategy(DiarizationLabelingStrategy):
    """pyannote community-1 (fine clustering) — the product strategy (ADR-132)."""
    ...
    def recorded_voices(
        ...
        """Recorded (ad/promo) voices — community-1's robust per-turn variant (ADR-132)."""
    ...
    def host_candidate_voices(
        ...
        *,
        ...
        conversational guests, montage clips, and sub-cameo fragments (ADR-132)."""
