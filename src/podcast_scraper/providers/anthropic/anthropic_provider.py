...
class AnthropicProvider:
    ...
    def summarize(
        ...
        # callsites (summarize_mega_bundled / summarize_bundled below)
        # keep the default ``structured=True`` and the floor still applies.
    ...
    def extract_quotes_bundled(
        ...
        **kwargs: Any,
        ...
        """Bundle ``extract_quotes`` across all insights into one Anthropic call (#698)."""
        ...
        try:
            ...
            logger.debug("Anthropic extract_quotes_bundled parse failed: %s", exc)
            raise
    ...
    def _score_entailment_bundled_chunk(
        ...
        """One bundled NLI Anthropic call for up to ``chunk_size`` pairs."""
        ...
        try:
            ...
            logger.debug("Anthropic score_entailment_bundled parse failed: %s", exc)
            raise
