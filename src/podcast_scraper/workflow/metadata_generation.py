...
def generate_episode_metadata(
    ...
    if pipeline_metrics:
        ...
        if hasattr(pipeline_metrics, "download_media_times") and episode.idx < len(
            pipeline_metrics.download_media_times
        ):
            download_time = pipeline_metrics.download_media_times[episode.idx]

        if hasattr(pipeline_metrics, "transcribe_times") and episode.idx < len(
            pipeline_metrics.transcribe_times
        ):
            transcribe_time = pipeline_metrics.transcribe_times[episode.idx]

        if hasattr(pipeline_metrics, "extract_names_times") and episode.idx < len(
            pipeline_metrics.extract_names_times
        ):
            extract_names_time = pipeline_metrics.extract_names_times[episode.idx]

        if hasattr(pipeline_metrics, "summarize_times") and episode.idx < len(
            pipeline_metrics.summarize_times
        ):
        ...
        if any(
            t is not None
            for t in [download_time, transcribe_time, extract_names_time, summarize_time]
        ):
            total_time = sum(
                t
                for t in [download_time, transcribe_time, extract_names_time, summarize_time]
