...
def detect_feed_hosts_and_patterns(
    ...
    # Skip speaker detection entirely if auto_speakers is disabled or in dry_run mode
    # Note: cache_detected_hosts only controls whether hosts are cached across episodes,
    # not whether the speaker detector is created (speaker detector is still needed for
    # per-episode guest detection even when caching is disabled)
    if not cfg.auto_speakers or cfg.dry_run:
    ...
    try:
        ...
        # When disabled, we still create the speaker_detector for per-episode guest detection
        if cfg.cache_detected_hosts:
            ...
            logger.debug(
                "Host caching disabled (cache_detected_hosts=false), "
                "speaker detector created for per-episode guest detection only"
            )
    ...
    for episode in episodes:
        ...
        if cfg.auto_speakers:
            ...
            if speaker_detector:
                ...
                # No speaker detector: expected in dry-run mode, unexpected otherwise
                if cfg.dry_run:
                    logger.debug(
                        "Dry-run: skipping speaker detection (no external calls in dry-run)"
                    )
                else:
                    # Should not happen in normal flow - log warning
