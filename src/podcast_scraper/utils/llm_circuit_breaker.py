...
class _BreakerState:
    ...
    # ADR-119 hold mode: monotonic time of the first trip since the last success (None = not
...
class LLMCircuitBreakerConfig:
    ...
    # ADR-119 failure strategy. "failover" (default): keep #697's wait-and-resume — the wrapping
...
def wait_if_overloaded(
    ...
    (ADR-119 consistency mode) it raises :class:`ResilienceFuseOpenError` once the endpoint has
    ...
    if str(config.failure_strategy) == "hold" and held_sec > config.on_open_max_wait_sec:
        # ADR-119 hold mode: sustained outage, no fallover — abort the batch. Reuse the ASR fuse's
...
def record_failure(
    ...
    with state.lock:
        ...
        if len(state.failure_times) >= config.failure_threshold:
            ...
            # ADR-119 hold mode: stamp the start of the sustained-down window on the FIRST trip
...
def record_success(provider_name: str, config: LLMCircuitBreakerConfig) -> None:
    ...
    with state.lock:
        ...
        state.hold_started_at = None  # ADR-119: endpoint recovered — reset the hold window
