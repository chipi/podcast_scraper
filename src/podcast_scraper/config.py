...
class Config(BaseModel):
    ...
    audio_evict_local_after_offload: bool = Field(
        ...
        description=(
            ...
            "confirmed present in the cold backend. The archive is internal-only (analysis + "
            "reproducible reprocessing), never served, so the local copy is disposable — this "
            "keeps on-disk audio near zero at rest. Default False (opt-in per prod profile). A "
            "start-of-run sweep also reclaims audio a crashed run left behind. Never deletes an "
            "episode whose audio is not in cold; never touches transcripts, art, or provenance."
        ),
    )
