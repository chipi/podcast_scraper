...
def load_cil_lift_overrides(corpus_root: Path) -> CilLiftOverrides:
    ...
    path = corpus_root / _OVERRIDES_FILENAME
    if not path.is_file():
        return CilLiftOverrides()
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        logger.warning("cil_lift_overrides: failed to read %s: %s", path, exc)
        return CilLiftOverrides()
    if not isinstance(raw, dict):
        logger.warning("cil_lift_overrides: expected object at top level: %s", path)
        return CilLiftOverrides()
