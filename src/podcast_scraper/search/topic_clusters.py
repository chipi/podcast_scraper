...
def theme_enrichment_by_topic_id(
...
def _load_theme_payload(corpus_root: Path) -> Optional[Dict[str, Any]]:
...
def load_theme_enrichment_map(corpus_root: Path) -> Dict[str, Dict[str, str]]:
    """Load ``search/topic_clusters.json`` and return enrichment map; empty if missing/invalid."""
    payload = _load_theme_payload(corpus_root)
    if payload is None:
        return {}
    return theme_enrichment_by_topic_id(payload)
