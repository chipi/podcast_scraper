...
| [`.github/workflows/drill-stack-playwright.yml`](../.github/workflows/drill-stack-playwright.yml) | Manual **`DRILL_STACK_PLAYWRIGHT`**, environment **`drill`**: join tailnet, run **`tests/stack-test/stack-viewer.spec.ts`** against **HTTPS** on the resolved drill host (browser + API + corpus **`/app/output`**). **`STACK_TEST_INSECURE_TLS=1`** for MagicDNS TLS. Also **`workflow_call`** with **`skip_confirm: true`** from **`dr-drill-exercise.yml`** (after **`drill-e2e`**). |
...
| [`.github/workflows/dr-drill-exercise.yml`](../.github/workflows/dr-drill-exercise.yml) | Manual **`DRILL_FULL_CYCLE`** (alias **`DRILL_EXERCISE`**): **`drill-infra-plan`** → **`drill-infra-apply`** → **`drill-tfstate-bridge`** (artifact **`drill-tfstate-for-teardown`**) → **`deploy-drill`** → **`drill-restore-corpus`** → **`drill-e2e`** → **`drill-stack-playwright`**, then **always** **`drill-infra-destroy`** (``finalize`` runs ``if: always()`` so destroy runs even when a middle job failed). |
