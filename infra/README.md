...
5. Stage `HCLOUD_TOKEN`, `TS_AUTHKEY` (device-join), `TS_INFRA_OAUTH_CLIENT_ID` + `TS_INFRA_OAUTH_SECRET` (OAuth client for terraform's tailscale provider; replaces the expiring `TS_API_KEY` — see [PROD_RUNBOOK.md "Tailscale credentials"](../docs/guides/PROD_RUNBOOK.md)), `TFSTATE_AGE_KEY` in repo Actions secrets (see [#714](https://github.com/chipi/podcast_scraper/issues/714)).
