...
| Workflow | Purpose |
| -------- | ------- |
| [`.github/workflows/drill-infra-plan.yml`](../.github/workflows/drill-infra-plan.yml) | On PRs touching `infra/**` or `tailscale/policy.hujson`, runs `tofu fmt -check`, `validate`, and `tofu plan` in workspace **`drill`** (posts a second PR comment next to `infra-ci.yml`). Also **`workflow_call`** from **`dr-drill-exercise.yml`**. |
| [`.github/workflows/drill-infra-apply.yml`](../.github/workflows/drill-infra-apply.yml) | Manual `workflow_dispatch` with confirm **`APPLY`**, GitHub Environment **`drill`**, `tofu apply`, re-encrypt **`terraform.tfstate.enc.drill`**, upload artifact. Also **`workflow_call`** with **`skip_confirm: true`** from **`dr-drill-exercise.yml`**. |
| [`.github/workflows/drill-infra-destroy.yml`](../.github/workflows/drill-infra-destroy.yml) | Manual confirm **`DRILL_DESTROY`**, environment **`drill`**: `tofu destroy` workspace **`drill`**, re-encrypt state, upload artifact. Also **`workflow_call`** (``skip_confirm: true`` only) from **`dr-drill-exercise.yml`** (always after the full-cycle finalize job). |
| [`.github/workflows/drill-e2e.yml`](../.github/workflows/drill-e2e.yml) | Manual **`DRILL_SMOKE`** confirm, environment **`drill`**: join tailnet, resolve **`DRILL_TAILNET_FQDN`**, SSH to **`deploy@`** and curl **`/api/health`** on host viewer port **8080** (read-only; no `tailscale serve` required). Requires ACL **`tag:gha-deployer` → `tag:dr-drill:22`** and secret **`DRILL_DEPLOY_SSH_PRIVATE_KEY`** (see `tailscale/policy.hujson`); ACL applied via prod **`infra-apply.yml`**. Also **`workflow_call`** from **`dr-drill-exercise.yml`**. |
| [`.github/workflows/drill-restore-corpus.yml`](../.github/workflows/drill-restore-corpus.yml) | Manual **`DRILL_RESTORE`** confirm, environment **`drill`**: download **`snapshot.tgz`** from the backup repo (default **`chipi/podcast_scraper-backup`**), restore **`corpus/`** on the drill host, recreate **api** + **viewer**. Optional secret **`BACKUP_REPO_TOKEN`** for a private backup repo. Also **`workflow_call`** from **`dr-drill-exercise.yml`**. |
| [`.github/workflows/dr-drill-exercise.yml`](../.github/workflows/dr-drill-exercise.yml) | Manual **`DRILL_FULL_CYCLE`** (alias **`DRILL_EXERCISE`**): **`drill-infra-plan`** → **`drill-infra-apply`** → **`deploy-drill`** → **`drill-restore-corpus`** → **`drill-e2e`**, then **always** **`drill-infra-destroy`** (``finalize`` runs ``if: always()`` so destroy runs even when a middle job failed). |
| [`.github/workflows/deploy-drill.yml`](../.github/workflows/deploy-drill.yml) | Manual **`workflow_dispatch`**, environment **`drill`**: same steps as **`deploy-prod.yml`** (Tailscale join, SSH as **`deploy@`**, **`infra/deploy/deploy.sh`**, external **`/api/health`**). Uses **`DRILL_TAILNET_FQDN`** and secret **`DRILL_DEPLOY_SSH_PRIVATE_KEY`** (public key must be in **`deploy@`** `authorized_keys` on the drill host). Also **`workflow_call`** from **`dr-drill-exercise.yml`**. |

**Secrets (repository):** `HCLOUD_TOKEN_DRILL` (Hetzner token scoped to the drill project only), plus the same `TS_API_KEY`, `TFSTATE_AGE_KEY`, and **`OPERATOR_SSH_PUBLIC_KEY`** (Actions **secret**, same value as prod infra workflows; masks the pubkey in workflow logs) as prod unless you document drill-only keys. For app deploys to the drill VPS, add **`DRILL_DEPLOY_SSH_PRIVATE_KEY`** (same PEM as **`PROD_SSH_PRIVATE_KEY`** is fine if the same CI public key is authorized for **`deploy@` on the drill host**). Optional **`BACKUP_REPO_TOKEN`** (same as **`backup-corpus-prod.yml`**) when **`chipi/podcast_scraper-backup`** is private so **`drill-restore-corpus.yml`** can download **`snapshot.tgz`**. **Variables:** reuse `TAILNET_NAME` from prod. Set **`DRILL_TAILNET_FQDN`** (e.g. `dr-podcast.tail-xxxx.ts.net`, matching `tailnet_hostname` in drill tfvars) for **`drill-e2e.yml`**, **`deploy-drill.yml`**, **`drill-restore-corpus.yml`**, and **`dr-drill-exercise.yml`**.
