...
| [`.github/workflows/dr-drill-exercise.yml`](../.github/workflows/dr-drill-exercise.yml) | Manual **`DRILL_FULL_CYCLE`** (alias **`DRILL_EXERCISE`**): **`drill-infra-plan`** → **`drill-infra-apply`** → **`drill-tfstate-bridge`** (artifact **`drill-tfstate-for-teardown`**) → **`deploy-drill`** → **`drill-restore-corpus`** → **`drill-e2e`**, then **always** **`drill-infra-destroy`** (``finalize`` runs ``if: always()`` so destroy runs even when a middle job failed). |
