...
# → Checks: black, isort, flake8, markdownlint, mypy
...
### ⚠️ CRITICAL: Check `mkdocs.yml` Before Pushing Documentation Changes
...
- Broken links → CI will fail with errors
- Wrong path format → Links won't work in generated docs

**Always check `mkdocs.yml` before committing documentation changes!**
...
OPENAI_API_KEY=sk-...
```
**`examples/.env.example` (template, can commit):**
...
```
### Always Run Full CI Before Pushing to PR

**CRITICAL: Before pushing to PR (new or updated), always run full local CI.**

**Required workflow before pushing:**
...
```bash
# ❌ WRONG: Pushing immediately after resolving conflicts
git add -A
git commit -m "..."
...
```text
Edit code → git commit → Hook checks → Commit succeeds →
make ci → Push → CI runs → PR ready
       ↓
