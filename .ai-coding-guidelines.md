...
**Note:** Pre-commit hooks already run `make ci-fast` checks. After pre-commit passes, you can push directly without running `make ci` again.
...
**🚨 TOP 2 CRITICAL RULES (MUST FOLLOW - NO EXCEPTIONS):**

1. ✅ **ALWAYS show changes and get approval before committing** (CRITICAL)
...
2. ✅ **ALWAYS show `git status` and `git diff` before pushing to PR** (CRITICAL)
   - **MANDATORY:** Show user what will be pushed → Get approval → THEN push
   - **NEVER push without showing changes first.**
3. ✅ **ALWAYS get explicit user approval before pushing to PR** (CRITICAL)
