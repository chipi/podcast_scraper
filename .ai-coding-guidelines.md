...
- **Wrong-package imports / pytest picking up another checkout / multi-worktree** → If **`WORKTREE.md`** exists at repo root, read it first (FUTURE checkout: venv per worktree, `__file__` sanity); then `docs/guides/GIT_WORKTREE_GUIDE.md` as needed
- **Code style/formatting work** → Load `docs/guides/DEVELOPMENT_GUIDE.md` (Code Style section)
- **Writing tests** → Load `docs/guides/TESTING_GUIDE.md`
- **Editing markdown** → Load `docs/guides/MARKDOWN_LINTING_GUIDE.md`
- **Cursor model selection** → Load `docs/guides/CURSOR_AI_BEST_PRACTICES_GUIDE.md`
- **Complete reference needed** → Load all 5 files (explicit request)
...
# - "load testing guide" → docs/guides/TESTING_GUIDE.md
# - "load markdown guide" → docs/guides/MARKDOWN_LINTING_GUIDE.md
# - "load cursor guide" → docs/guides/CURSOR_AI_BEST_PRACTICES_GUIDE.md
# - "load development guide" → docs/guides/DEVELOPMENT_GUIDE.md

```text

**CRITICAL RULES (MUST FOLLOW ALWAYS - NO EXCEPTIONS):**

** FILE LOCATION RULE - ABSOLUTE PROHIBITION (NO EXCEPTIONS):**

**When creating ANY analysis, plan, review, or WIP document:**

1. **Never:** **NEVER create `docs/analysis/` folder** - This folder MUST NOT EXIST
2. **Never:** **NEVER create `docs/plan/` folder** - This folder MUST NOT EXIST
3. **Never:** **NEVER create analysis files anywhere except `docs/wip/`**
4. **ALWAYS create analysis/plan/review documents in `docs/wip/` folder**
5. **If you see yourself typing `docs/analysis/` → STOP and use `docs/wip/` instead**
...
** GITHUB OPERATIONS - MCP SERVER FIRST (MANDATORY):**
...
1. **ALWAYS use MCP GitHub server tools first** - Preferred method for all GitHub operations
...
3.  **Fallback to GitHub API only if:**
...
4. **Never:** **NEVER use GitHub API directly when MCP server is available**
...
# Correct: CORRECT: Use MCP server first
...
> **For detailed technical information**, see [`docs/guides/DEVELOPMENT_GUIDE.md`](docs/guides/DEVELOPMENT_GUIDE.md) which covers:
>
...
> For detailed technical patterns and implementation details, refer to `docs/guides/DEVELOPMENT_GUIDE.md`.
...
1. Keep this file updated when patterns change
2. Reference specific sections when asking Cursor to implement features
3. Use clear, actionable language (Cursor reads this as instructions)
...
- **Auto** → fast iteration, small/local changes (mechanical work)
- **Manual selection** → planning, debugging, architecture, CI failures (deep reasoning)
...
- Control scope: small selection → fast model, multiple files/logs → stronger model
...
### End-of-session review (agents — Rule 18)

At **meaningful work completion** — same **cadence** as **`.metrics/rule-adherence.jsonl`** milestones, not
only the phrases **done** / **wrap up** (use judgment: green slice, before commit/push, user clearly closing
out, thread winding down after a shipped fix). In the **chat reply**, add a short **visible** retrospective:
what would have closed the task faster, what to verify next time, risks of a false **fixed**. **Silent or
skip** only if the user explicitly asked for no reflection. **Do not** put that text into
**`rule-adherence.jsonl`** — metrics stays rule/skills/subagents only. When a lesson should become **shared
process**, update the right guide (e.g. **`docs/guides/AGENT_BROWSER_LOOP_GUIDE.md`** for UI/browser) or
**`.cursorrules`** / **`.cursor/rules/*.mdc`** (or **`docs/wip/`** for drafts, after user confirms a file is
wanted). Authoritative text: **`.cursorrules` Rule 18**.
...
### When Something Is Broken: Reuse Existing Patterns First

**Before adding workarounds or changing how something works, look for similar examples in the repo first.**
...
** CRITICAL: Write Lint-Valid Code from the Start**

**When writing new code or modifying existing code, ALWAYS consider linting rules from the beginning:**

- **Write code that passes linting checks immediately** - Don't write code and fix linting errors later
- **Follow line length limits** - Keep lines under 100 characters (black/flake8 E501)
- **Follow import organization** - Use isort-compatible import order (stdlib → third-party → local)
- **Follow naming conventions** - Use snake_case for functions/variables, PascalCase for classes
- **Remove unused imports** - Don't leave unused imports (flake8 F401)
- **Remove unused variables** - Don't leave unused variables (flake8 F841)
- **Follow type hints** - Add type hints to function signatures (mypy compliance)
- **Follow docstring style** - Use Google-style docstrings for public functions
...
- **Stop and refactor immediately** - Break long lines, organize imports, remove unused code
- **Don't defer linting fixes** - Fix issues as you write, not after
- **Use `make format` and `make lint` frequently** - Catch issues early during development
...
│   ├── acceptance/          # README + MAIN_ACCEPTANCE_CONFIG.yaml + fragments/ tracked; other *.yaml local
│   ├── …                    # Some subtrees are gitignored (see .gitignore) — local only; Rule 4f in .cursorrules
│   └── playground/          # Playground configs, ad-hoc / one-off (gitignored)
├── scripts/                 # Utility scripts (eval, fix_markdown, etc.)
├── prompts/                 # Prompt templates (planned, RFC-017)
...
## CRITICAL: Running Tests After Making Changes
...
- After any code changes (default choice)
- After bug fixes
...
### Unit tests and `pyproject` optional extras

**When adding or editing `tests/unit/`:**

- Do **not** require **`[ml]`**, **`[llm]`**, **`[compare]`**, or real ML/LLM stacks. Use mocks, **`patch.dict(sys.modules, …)`**, or move the test to
  **`tests/integration/`**. Keep **FastAPI `TestClient` / `create_app`** tests in **`tests/integration/server/`** even though **`[dev]`** ships FastAPI.
- **`[dev]`** is the only extra installed by CI `test-unit`. Any test that needs FastAPI, httpx,
  torch, spaCy, lancedb, etc. belongs in **`tests/integration/`** (where CI installs
  `.[dev,ml,llm]`).
- Do **not** use **`pytest.importorskip()`** in `tests/unit/` to guard non-`[dev]` imports — it
  causes the test to be silently skipped in CI, meaning it never runs. Use `importorskip` in
  **`tests/integration/`** instead.
- Do **not** use real **LanceDB index** builds, real Whisper / spaCy, or real HTTP clients
  in unit tests.

**Canonical doc:** `docs/guides/UNIT_TESTING_GUIDE.md` (section *Pyproject extras: what unit tests may depend on*)
and `docs/architecture/TESTING_STRATEGY.md` (section *Unit tests and optional extras (`pyproject`)*).

### ML/AI boundary policy (3-tier)

| Layer | ML models (Whisper, spaCy, transformers) | AI APIs (OpenAI, Gemini, Ollama, etc.) |
|-------|------------------------------------------|----------------------------------------|
| **Unit** | Always mocked | Always mocked |
| **Integration** | Always mocked | Always mocked |
| **E2E** | Real (the only place) | Real or E2E server mocks |

**If a test loads a real ML model or calls a real AI API, it is E2E -- not integration.**
`@pytest.mark.ml_models` belongs on E2E tests only. Integration tests verify how our
components wire together; the ML/AI boundary is always a mock or stub there.

### CI skips are bugs, not features

- Do not add `skipIf`, `importorskip`, or `skipTest` that would cause a test to be
  silently skipped on CI. A test that never runs on CI is worse than no test.
- If you see a skipped test in CI output, investigate why -- do not treat it as normal.
- `tests/unit/`: zero skips expected on CI `test-unit` (`.[dev]` only).
- `tests/integration/`: zero skips expected on CI integration jobs (`.[dev,ml,llm]`).

### GI/KG browser viewer (Vue + Playwright)

**Path:** `web/gi-kg-viewer/`. **Not collected by pytest** — use **`make test-ui`** (Vitest) and **`make test-ui-e2e`** (Playwright).

**Invoking viewer tools — use `npm run …` or `./node_modules/.bin/<tool>`, never `npx`.** `web/gi-kg-viewer/node_modules/` is the Node analog of `.venv/` (project-pinned, gitignored). The viewer pins **`@playwright/test`** (CLI exposed as `node_modules/.bin/playwright`) but **not** a separate package literally named `playwright`. **`npx playwright …`** doesn't find a `playwright` package in `node_modules`, silently fetches a fresh copy from the npm registry, and runs that. The fresh runner then loads spec files whose `import { test } from '@playwright/test'` resolves to the project's copy — **two module instances at the same version, no shared state**. The runner's test registry rejects the spec's `test.describe()` calls with *"did not expect test.describe() to be called here"*; the worker process crashes; the parent shell sees exit code **137** (SIGKILL) — easy to misread as user-canceled or OOM. Smoking gun: `npm warn exec ... playwright@… will be installed` near the top of the output. Use `npm run test:e2e -- <args>`, `npm run test:unit`, `npm run dev`, `npm run build`, or the direct `./node_modules/.bin/playwright test …` instead. `npx` is **not** the Node analog of `python -m`. Polyglot details: **`docs/guides/POLYGLOT_REPO_GUIDE.md`** — *Invoking viewer tools*.

**Pre-merge validation (UI-heavy):** Prefer **`make ci-ui-fast`** before commit when the diff is mostly viewer work. It runs the same **lint / type / docs / build** chain as **`make ci-fast`**, but skips Python **`tests/e2e/`** and runs **Playwright** instead. Use **`make ci-fast`** when you also need Python pipeline E2E coverage or want exact parity with the **pre-commit** hook (which still runs **`make ci-fast`** unless **`git commit --no-verify`**).

**When you change viewer UX** (labels, routes, panels, layout, theme tokens, accessible names, or any flow that E2E touches), treat updates as a **single workflow** (same PR):

1. **`web/gi-kg-viewer/e2e/E2E_SURFACE_MAP.md`** — Refresh the automation contract (selectors, surfaces, entry paths) when E2E-visible behavior changes.
2. **`web/gi-kg-viewer/e2e/*.spec.ts`**, **`helpers.ts`**, **`fixtures.ts`** — Keep Playwright green; run **`make test-ui-e2e`** (or **`make ci-ui-fast`** for the full local gate above).
3. **`docs/uxs/`** — Update **`VIEWER_IA.md`** when **shell information architecture** changes (regions, navigation axes, persistence, clearing, first-run). Update **`UXS-001`** for **shared** tokens and **visual** chrome rules; update the relevant **feature UXS** (`docs/uxs/index.md`) when a **surface-specific** spec changes, not only when tests fail. **Active** UXS should track **shipped** chrome (Draft while in flight; align at merge). See **`docs/uxs/index.md`** — [Living documents and ship boundary](docs/uxs/index.md#living-documents-and-ship-boundary).

4. **FastAPI JSON contracts** — New or changed routes: **`tests/unit/podcast_scraper/server/`** and **`tests/integration/server/`** (not root `tests/integration/test_server_api.py`).

**Guides:** `docs/guides/E2E_TESTING_GUIDE.md` (Playwright), `docs/guides/TESTING_GUIDE.md` (*Browser E2E*), `docs/guides/DEVELOPMENT_GUIDE.md` (GI/KG viewer), `docs/guides/SERVER_GUIDE.md` (`/api/*`, OpenAPI `/docs`), `docs/uxs/index.md`, `docs/uxs/VIEWER_IA.md` (viewer shell IA).

**Agent MCP (interactive repro / validate):** Default **Chrome DevTools MCP**; use **Playwright MCP** only when clearly better for scripted automation, and state that in one line. Re-validate in the **same** channel you used to reproduce. See **`docs/guides/AGENT_BROWSER_LOOP_GUIDE.md`** (*Default MCP choice* + *Symmetry rule*).

**Local FastAPI while iterating:** If you change server code the running Uvicorn process must reload to pick up, **restart `make serve-api` yourself**. **`SERVE_OUTPUT_DIR`:** when the **agent** starts or restarts the server **without** the user naming a root, use **`.test_outputs`** (see **Agent default corpus root** in **`.cursorrules`**); otherwise infer from the session or ask once. Verify **`GET /api/health`**. End with **Ready for tests** and the URL + root — do **not** only tell the user to restart. See **API restart discipline** in **`.cursorrules`** (GI/KG viewer section); background **`serve-api`** is allowed under terminal Rule **9**.
...
| -------------- | ------------------ | --------- |
| **Integration tests** (slow marked) | Slow integration tests | `make test-integration-slow` |
| **E2E tests** (slow/ml_models marked) | Slow E2E tests | `make test-e2e-slow` |
| **Both integration and E2E** (slow) | All slow tests | `make test-all-slow` |
...
- Changed code that uses ML models (Whisper, transformers, spaCy)
- Changed integration test code marked with `@pytest.mark.slow`
- Changed E2E test code marked with `@pytest.mark.slow` or `@pytest.mark.ml_models`
- Changed core workflow that affects ML model loading/usage
- Changed configuration that affects ML model selection
- Before final commit/push (if changes touch slow test areas)

**When NOT to run slow tests:**

- Skip when: only unit test changes
- Skip when: only documentation changes
- Skip when: only configuration file changes (that don't affect ML)
- Skip when: only fast integration/E2E test changes
- Skip when: code changes that don't touch ML model code paths
...
```bash
    │   │   └─ Slow tests pass? → Ready to commit/push
    │   └─ No → Ready to commit/push
    └─ No → Fix issues → Run fast tests again
```text
```bash
...
# Step 2: Fast tests pass, no slow test areas touched → Ready!
...
# Step 2: Fast tests pass, but changed ML model code → Run slow tests
...
# Step 3: All tests pass → Ready to commit/push

```text

# Step 2: Fast tests pass → Ready!
...
# Step 2: Fast tests pass, but workflow.py affects ML → Run slow tests
...
```text
- Run `make ci-fast` before committing (pre-commit hook will also run this; exception: workflow-only changes); for viewer-heavy diffs prefer `make ci-ui-fast` locally first
...
- **`check_unit_test_imports.py`** - Verifies selected **library** modules import without the heavy ML stack
  - **When to use:** After adding new modules, refactoring imports, debugging CI failures, or before committing if you changed import statements
  - **Usage:**
    ```bash
    ...
    ```
  - **What it does:** Ensures listed modules can be imported without **ML** optional deps (spacy, torch, transformers, etc.) at import time.
  - **Why it matters:** Prevents regressions where core code pulls ML in at import time. **Authoring rule:** `tests/unit/` must not depend on any non-`[dev]` extra (`[ml]`, `[llm]`, `[compare]`) — if a test needs one, move it to `tests/integration/`. See `docs/guides/UNIT_TESTING_GUIDE.md` (*Pyproject extras*).
  - **Runs automatically in CI:** Before unit tests in both `python-app.yml` and `nightly.yml` workflows
- **`analyze_dependencies.py`** - Analyzes module dependencies and detects architectural issues
  - **When to use:** After refactoring modules, adding new imports, debugging architecture issues, or before major refactoring
  - **Usage:**
    ```bash
    # Using make target (recommended)
    make deps-analyze    # Full analysis with JSON report
    make deps-check      # Quick check (exits with error if issues found)

    # Or directly
    python scripts/tools/analyze_dependencies.py --report  # Generate detailed report
    python scripts/tools/analyze_dependencies.py --check   # Check mode (CI-friendly)
    ```
  - **What it does:**
    - Checks for circular imports
    - Analyzes import patterns across modules
    - Checks thresholds (max 15 imports per module)
    - Generates JSON report with detailed analysis
  - **Why it matters:** Helps maintain clean module boundaries, prevents circular dependencies, and identifies coupling issues early
  - **Runs automatically in CI:** In nightly workflow (`nightly-deps-analysis` job) with 90-day artifact retention
- **`analyze_test_memory.py`** - Analyzes test suite memory usage and resource consumption
  - **When to use:** Debugging memory issues, optimizing test performance, investigating memory leaks, capacity planning, or before major test changes
  - **Usage:**
    ```bash
    # Using make target (recommended)
    make analyze-test-memory                    # Default: test-unit
    make analyze-test-memory TARGET=test-unit   # Analyze unit tests
    make analyze-test-memory TARGET=test-integration WORKERS=4  # With limited workers
    ...
    ```
  - **What it does:**
    - Monitors memory usage during test execution (process + child processes)
    - Tracks peak/average memory, worker count, memory growth over time
    - Detects memory leaks (memory growth detection)
    - Provides recommendations (warnings for high memory usage, excessive workers)
  - **Why it matters:** Helps identify memory leaks, optimize parallelism, understand resource needs, and debug OOM errors
  - **Dependencies:** Requires `psutil` package for process monitoring
  - **Not in CI:** This is a diagnostic/profiling tool (manual use only, not integrated into CI)
- **Eval tooling** — Cleaning and summarization evaluation scripts are evolving in parallel (see Issue #477 and `data/eval/`)

**Best practices:**
- Run `make check-unit-imports` after modifying imports or adding new modules
- Run `make deps-check` before committing if you refactored modules or added new imports
- Run `make analyze-test-memory` when debugging memory issues or optimizing test performance
- Run `python scripts/tools/fix_markdown.py` before committing markdown files to catch common issues early

## What CI Runs Automatically

> **See also:** [`docs/guides/DEVELOPMENT_GUIDE.md`](docs/guides/DEVELOPMENT_GUIDE.md) for the CI/CD pipeline documentation (`docs/CI_CD.md` was removed).
...
4. **Docker Workflow** (on `docker/pipeline/Dockerfile` / project structure changes):
   - **When to test:** Only when `docker/pipeline/Dockerfile`, `pyproject.toml` structure, or package layout changes
...
- **Docs Deployment** (on main): Build → Deploy to GitHub Pages
...
```text
- **Never:** Docker builds (unless working on Docker specifically OR before pushing Docker-related changes to PR)
- **Never:** Deployment commands (GitHub Actions handles this)
...
- Add HTTP calls to `cli.py` → **Wrong:** Use `downloader.py` instead
- Add business logic to `config.py` → **Wrong:** Use `workflow.py` or appropriate module
- Add CLI prompts to `service.py` → **Wrong:** Service is non-interactive

### Provider-Specific Code (CRITICAL)

** Provider-specific code MUST stay in the provider module - NEVER in central config/workflow**
...
- SDK logging configuration (e.g., suppressing OpenAI verbose debug logs)
...
**Bad:** Provider-specific code in central config

```python
# workflow.py - WRONG!
def apply_log_level(level: str):
    # ... setup logging ...
        # Wrong: OpenAI-specific logging config doesn't belong here
    openai_logger = logging.getLogger("openai._base_client")
    openai_logger.setLevel(logging.WARNING)
```

**Good:** Provider manages its own logging

```python
# openai/openai_provider.py - CORRECT
class OpenAIProvider:
    def __init__(self, cfg: Config):
        # Provider-specific logging config belongs here
...
**When adding provider-specific code:**
1. Put it in the provider's `__init__()` or `initialize()` method
2. Keep it private to the provider module
3. **Never:** Don't put it in `workflow.py`, `config.py`, or other central modules
4. **Never:** Don't make central code aware of provider internals
...
from podcast_scraper import cli, service  # Works (lazy loaded)
import podcast_scraper.cli as cli  # Also works
...
- **Always start from the template** - Don't create PRDs/RFCs from scratch
- **Check existing numbers** - Use the next available number in the sequence
- **Remove unused sections** - If a section doesn't apply, remove it rather than leaving placeholders
- **Update index files** - Always add new documents to the index
- **Follow naming conventions** - Use lowercase with hyphens for filenames
...
** CRITICAL: Where to Create Analysis and Plan Documents**

** ABSOLUTE RULE: All analysis or plan documents MUST be created in `docs/wip/` folder:**

- **Analysis documents** (code analysis, test pyramid analysis, performance analysis, critical path analysis, episode length analysis, etc.) → **`docs/wip/`**
- **Plan documents** (implementation plans, refactoring plans, migration plans, trimming proposals, etc.) → **`docs/wip/`**
- **Work-in-progress documentation** (draft RFCs, experimental designs, etc.) → **`docs/wip/`**
- **Review documents** (code reviews, architecture reviews, etc.) → **`docs/wip/`**
- **Summary documents** (test fixture summaries, trimming summaries, etc.) → **`docs/wip/`**

**There is NO exception to this rule. If it's analysis, plan, review, or WIP → it goes in `docs/wip/`.**
...
** ABSOLUTELY FORBIDDEN - DO NOT CREATE THESE FOLDERS OR FILES:**

- **Never:** **`docs/analysis/`** - THIS FOLDER MUST NEVER EXIST. All analysis goes in `docs/wip/`
- **Never:** **`docs/plan/`** - THIS FOLDER MUST NEVER EXIST. All plans go in `docs/wip/`
- **Never:** Root directory
- **Never:** `docs/` root (without subdirectory)
- **Never:** `docs/rfc/` (only for formal RFCs)
- **Never:** `docs/prd/` (only for formal PRDs)

**If you see yourself about to create `docs/analysis/` or any file in it:**
- **Never:** **STOP IMMEDIATELY**
- **Create the file in `docs/wip/` instead**
- **Remember: Analysis = WIP, Plans = WIP, Reviews = WIP**
...
- If an analysis becomes a formal RFC → Move to `docs/rfc/` and follow RFC format
- If a plan becomes a formal PRD → Move to `docs/prd/` and follow PRD format
- If it's no longer needed → Delete or archive

## Specification IDs in product code (CRITICAL)

**Do not embed formal document identifiers in runnable code, comments, CLI strings, CSS class names, test titles, user-visible UI copy, or persisted JSON keys** except when the value is literally a path under `docs/rfc/`, `docs/prd/`, or `docs/uxs/` (for example, a markdown link target).

**Forbidden in code and tooling prose (examples):**

- `RFC-076`, `RFC076`, `rfc076-*` (use feature names: cross-episode graph expand, `POST /api/corpus/node-episodes`, and neutral CSS such as `graph-expand-eligible`)
- `PRD-021`, `PRD-019` (use product terms: semantic corpus search, KG quality metrics)
- `UXS-004`, `UXS-001` (use UX terms: graph exploration shell, shared viewer tokens)

**Allowed:**

- Files that live under `docs/rfc/`, `docs/prd/`, `docs/uxs/`, and RFC/PRD index tables there
- Commit messages and PR descriptions that cite specs for traceability (still prefer human-readable feature names in the subject line)

**Traceability:** Link specs from design docs and reviews; in code, point to APIs, modules, GitHub issues (`#nnn`), or short neutral phrases. If you need a long-form rationale, put it in `docs/wip/` or the relevant spec under `docs/`, not in a code comment.

**Existing persisted keys:** Do not rename JSON fields or on-disk identifiers that are already part of a public or frozen contract without an explicit migration plan and user approval.

### Markdown Best Practices

** CRITICAL: Follow These Patterns to Prevent Linting Errors**
...
- Run `python scripts/tools/fix_markdown.py` before committing
...
Bad (no spaces around pipes in separator):
| Header | Column2 | Column3 |
| -------- | --------- | --------- |
| Value1 | Value2  | Value3  |
...
- Use spaces around pipes in all rows: `| Column |`
- Separator row should match header row spacing: `|--------|`
- For complex tables, use Python to generate exact alignment (see `docs/guides/MARKDOWN_LINTING_GUIDE.md`)
- Run `python scripts/tools/fix_markdown.py` to auto-fix table separators
...
- Common languages: `python`, `bash`, `yaml`, `json`, `markdown`, `text`
...
```text
- Run `python scripts/tools/fix_markdown.py` to auto-fix multiple blank lines

#### 6. General Markdown Formatting

**Additional best practices:**

- **Headings**: Use proper heading hierarchy (H1 → H2 → H3, don't skip levels)
- **Lists**: Use consistent list markers (`-` for unordered, `1.` for ordered)
- **Links**: Use descriptive link text: `[Link Text](url)` not `[url](url)`
- **Code**: Use backticks for inline code: `` `code` `` not `'code'`
...
1. **Write markdown following the patterns above**

2. **Run `python scripts/tools/fix_markdown.py`** to auto-fix common issues:
...
3. **Self-check before finishing** - Scan for: (a) one H1 only, (b) blank line after headings,
   (c) no trailing spaces, (d) list indentation consistent, (e) code fences have language tags

4. **Verify with `make lint-markdown`** if markdownlint is installed

5. **MANDATORY: Run `make docs`** to validate docs build correctly (catches broken links,
...
- `scripts/tools/fix_markdown.py` - Automated fixer script
...
| `docs/architecture/ARCHITECTURE.md`     | Module changes, data flow changes, design decisions  |
| `docs/architecture/TESTING_STRATEGY.md` | Testing approach changes, new test infrastructure    |
| `docs/api/`                | Public API changes (auto-generated from docstrings)  |

### CRITICAL: Check `mkdocs.yml` and Links Before Pushing Documentation Changes
...
- Missing files in `nav` → CI will fail with warnings
- Broken links → CI will fail with errors (~3-5 min wasted per build)
- Wrong path format → Links won't work in generated docs
...
** CRITICAL: Do NOT modify historical release notes**
...
- You CAN update current/upcoming release notes (draft releases)
- **Never:** You CANNOT update past release notes (already shipped versions)
- You CAN update other documentation (README, guides, etc.) to reflect current state
- **Never:** You CANNOT "fix" historical release notes to match current codebase

**Examples:**

- **Never:** Don't update `RELEASE_v2.3.1.md` to remove `requirements.txt` references (it was there when v2.3.1 shipped)
- You can update `README.md` to remove `requirements.txt` references (current documentation)
- **Never:** Don't change dependency lists in old release notes
- You can update current documentation to reflect new dependency management
...
4. Update `config/examples/config.example.json` and `config/examples/config.example.yaml`
...
**See:** `RFC-015` (moved to the private eval repo as RFC-004) and `docs/prd/PRD-007-ai-quality-experiment-platform.md`
...
│   ├── factory.py           # Factory
│   ├── whisper_provider.py  # Local Whisper provider
│   └── openai_provider.py   # NEW: OpenAI Whisper API
├── summarization/
│   ├── base.py              # SummarizationProvider protocol
│   ├── factory.py           # Factory
│   ├── local_provider.py    # Local transformers provider
...
- Document required env vars in `README.md` and `CONTRIBUTING.md`
- Use `config/examples/.env.example` as template
...
- **Never:** Passwords or credentials
- **Never:** Personal data
- **Never:** Real RSS feed URLs in tests (use example.com)
- **Never:** Private configuration files
- **Never:** `.env` files with secrets
...
2. **Ensure the branch contains ONLY new commits** - No old/unrelated commits
3. **Start from a clean state** - Branch should be based on the latest target branch
...
**If you need to discard edits in specific files:** do **not** run `git checkout -- <path>` (or `git restore` to index/`HEAD`) unless the user **explicitly** asked to discard that work or you **asked first** and they **confirmed** (`.cursorrules` Rule 4e). That path restores from the index / last commit and **removes uncommitted changes**. Prefer: **commit**, **`git stash`** (never during a merge — see `.cursorrules` 4a), or **editor Local History / Timeline**. See `.cursor/rules/git-working-tree-safety.mdc`.

**If you see any output, handle it first!**

**What happens if you don't follow this:**
- **Never:** Uncommitted changes from previous work get included in your new branch
- **Never:** Your commit will show more files than you actually changed
- **Never:** PR will show confusing diffs with unrelated changes
- **Never:** Harder to review and understand what actually changed
...
- Branch contains ONLY commits related to this PR
- No merge commits from other branches
- No old commits that were already merged
- No WIP commits that should be squashed
...
- **Never:** Branch contains old commits that are already merged
- **Never:** Branch has commits from other features/PRs
- **Never:** Branch is based on an outdated version of main
- **Never:** Branch has merge commits from other branches (unless intentional)
...
** MANDATORY CHECKLIST - NO EXCEPTIONS - FOLLOW THIS EXACT SEQUENCE:**
...
1. Make code changes
2. **Run `make ci-fast` or `make ci-ui-fast` when needed** — **`make ci-fast`**: formatting, linting, type checks, fast Python tests (unit + critical integration + critical Python **`tests/e2e`**), Vitest, docs, build. **`make ci-ui-fast`**: same gates except skips Python **`tests/e2e/`** and runs **Playwright**; use for **viewer-heavy** changes under `web/gi-kg-viewer/` when you do not need Python pipeline E2E. **Exceptions:** workflow-only changes (only `.github/workflows/*.yml`); **recent green run:** the same target (`ci-fast` or `ci-ui-fast`) or `make ci` already passed in this session on the **same** tree and **no substantive edits** were made after that run (state briefly that you are skipping a duplicate run); user explicitly waives re-validation; **incremental validation:** user says no full CI rerun or the change is a narrow follow-up — run the Makefile targets in `.cursorrules` (**rule 5c**), report what was skipped, and note **pre-commit still runs `make ci-fast`** unless `--no-verify`. **Re-run** the appropriate full target if anything material changed after the last green run, the last run failed, you are unsure the tree matches what was validated, or rule 5c does not apply.
3. **If editing markdown files:** Run `make fix-md` (or `python scripts/tools/fix_markdown.py`) to fix common linting issues
4. **MANDATORY: Self-check markdown files** (if any were edited):
...
6. **MANDATORY: Run `git diff`** - Show user the actual changes (for each modified file)
7. **MANDATORY: Wait for explicit user approval** - User must explicitly say "commit", "yes", "go ahead", etc.
8. **MANDATORY: Get commit message** - User provides message OR you ask "What commit message should I use?"
9. **THEN commit** - Only after validation (step 2) when required, and steps 3-8 as applicable (pre-commit hook will run additional checks unless `--no-verify`)

** CRITICAL:** Steps 3-8 are mandatory when applicable. Step 2 (`ci-fast` / `ci-ui-fast`) may be skipped only per **recent CI on current tree**, **user waiver**, or **incremental validation** (see `.cursorrules` rules **5** and **5c**). Do NOT assume approval.
...
1. **After pre-commit hook passes, push directly** - Pre-commit hook already runs `make ci-fast` checks
2. **Show `git status` and `git diff`** before asking to push
3. **Get explicit user approval** before pushing
4. **Conditionally run `make docker-test`** if Docker-related changes exist (see below) - only if needed
...
**Testing Strategy:**
- **Before commit:** Run `make ci-fast` or **`make ci-ui-fast`** for viewer-heavy work (fast validation: formatting, linting, fast tests; **`ci-ui-fast`** runs **Playwright** instead of Python **`tests/e2e/`**), **unless** the same check already passed in-session on the current diff with no substantive edits after (efficiency — see `.cursorrules` rule 5)
- **Pre-commit hook:** Automatically runs `make ci-fast` checks when committing (unless `git commit --no-verify`)
- **After pre-commit passes:** Push directly (no need to run `make ci` again if `ci-fast` already covered what changed; full `make ci` optional for extra confidence)
- **If changes touch slow test areas:** Consider running slow tests before commit (optional)
...
- **Pipeline Dockerfile** (`docker/pipeline/Dockerfile`) - Any changes to build process, dependencies, or structure
- **Project structure** - Changes to package layout, subpackages, or file organization that could affect Docker build
- **Dependencies** - Changes to `pyproject.toml` that affect Docker installation (core dependencies, package metadata, subpackages)
- **Package configuration** - Changes to `[tool.setuptools]` in `pyproject.toml` (packages list, package-dir, etc.)

**When NOT to run `make docker-test`:**

- **Never:** Only Python code changes (no `docker/pipeline/Dockerfile` or structure changes)
- **Never:** Only documentation changes
- **Never:** Only test file changes
- **Never:** Only configuration file changes (that don't affect Docker)
...
# Pre-commit hooks passed. Should I push these changes to the PR?"

# Step 3: WAIT for user to explicitly approve (user says "yes", "push", "go ahead", etc.)

# Step 4: ONLY THEN push (after user approval)

git push origin feature/branch-name

# Note: Pre-commit hook already ran make ci-fast checks, so no need to run make ci again

```text

- **Never:** **User hasn't explicitly approved the push** (MOST IMPORTANT - CHECK THIS FIRST)
- **Never:** **You haven't shown `git status` first**
- **Never:** **You haven't shown `git diff` or change summary first**
- **Never:** **User said "don't push" or "wait"**
- **Never:** Pre-commit hook failed (fix issues before pushing)
- **Never:** `make docker-test` hasn't been run (when Docker-related changes exist)
- **Never:** `make docker-test` has failures (when Docker-related changes exist)
...
# Step 3: WAIT for user to say "yes", "push", "go ahead", etc.
...
**Bias toward action:** When the user’s goal is clear, run tools and commands, apply edits, and verify in-session (tests, lint, scripts, downloads, etc.). Do **not** default to long “what you need to do next” lists for steps the agent can perform itself.

**Ask only when you cannot proceed:** missing credentials/secrets or authenticated access you do not have; repo **CRITICAL** rules that require explicit user approval (e.g. commit/push); genuinely **ambiguous** scope or intent (then use “When in Doubt, Ask”); unsafe requests.

This complements **Don’t Ask When** (clear patterns → just do) and **When in Doubt, Ask** (unclear goals → clarify).
...
### Showing command output to the user

When you run a Bash command, the harness can't stream stdout live — the user only sees the output once the command exits. Two implications:

- **Don't pipe to `| tail -N`** (or `| head` / `| grep`-and-discard) when the user is waiting for a result. A short tail truncates the body they would otherwise see in full. Prefer no pipe; if the command genuinely produces > 1k lines, use a generous tail (`tail -100`) or run in background and report a typed summary.
- **Never silently filter the command body the user asked you to run.** They want to see what it did, not what you decided was relevant.
- **`is_background: true` is for genuinely long-running things** (servers, multi-minute builds where you have other work to do). For everything else, run foreground and let the full output reach them.
...
make test-fast         # Fast tests (unit + fast integration + fast e2e, ~6-10 min)
...
- **[docs/guides/POLYGLOT_REPO_GUIDE.md](docs/guides/POLYGLOT_REPO_GUIDE.md)** - Python + Node layout, env files, viewer Makefile targets
- **[docs/guides/DEVELOPMENT_GUIDE.md](docs/guides/DEVELOPMENT_GUIDE.md)** - ** Detailed technical information** (code style, testing, CI/CD, architecture, logging, documentation standards, markdown linting)
- **`docs/CI_CD.md` (removed; CI is documented in [docs/guides/DEVELOPMENT_GUIDE.md](docs/guides/DEVELOPMENT_GUIDE.md))** - CI/CD pipeline documentation with visualizations
- **[docs/architecture/ARCHITECTURE.md](docs/architecture/ARCHITECTURE.md)** - Architecture design and module responsibilities
- **[docs/architecture/TESTING_STRATEGY.md](docs/architecture/TESTING_STRATEGY.md)** - Comprehensive testing approach
- **[docs/api/](docs/api/)** - Auto-generated API reference
- **[README.md](README.md)** - User-facing documentation
- **`RFC-015`** - AI experiment pipeline design (moved to the private eval repo as RFC-004)
...
- Python + web viewer repo layout → See `docs/guides/POLYGLOT_REPO_GUIDE.md`
- Code style details → See `docs/guides/DEVELOPMENT_GUIDE.md` (Code Style Guidelines section)
- Testing details → See `docs/guides/DEVELOPMENT_GUIDE.md` (Testing Requirements section)
- CI/CD details → See `docs/guides/DEVELOPMENT_GUIDE.md` (CI/CD Integration section)
- Architecture patterns → See `docs/guides/DEVELOPMENT_GUIDE.md` (Architecture Principles section)
- Logging patterns → See `docs/guides/DEVELOPMENT_GUIDE.md` (Logging Guidelines section)
- Documentation standards → See `docs/guides/DEVELOPMENT_GUIDE.md` (Documentation Standards section)
...
3. **ALWAYS show `git status` and `git diff` before pushing to PR** (CRITICAL)
   - **MANDATORY:** Show user what will be pushed → Get approval → THEN push
   - **NEVER push without showing changes first.**

4. **ALWAYS get explicit user approval before pushing to PR** (CRITICAL)
   - **MANDATORY:** Ask user → Wait for "yes"/"push"/"go ahead" → THEN push
   - **NEVER push without explicit user approval.**
4. **Run `make ci-fast` or `make ci-ui-fast` before committing when needed** (CRITICAL)
   - **Default:** Run `make ci-fast` before committing (fast validation: formatting, linting, fast tests including Python **`tests/e2e`**)
   - **Viewer-heavy diffs** (`web/gi-kg-viewer/`): prefer **`make ci-ui-fast`** locally (includes **Playwright**; skips Python **`tests/e2e/`**). Pre-commit still runs **`make ci-fast`**
   - **Pre-commit hook** also runs `make ci-fast` unless `git commit --no-verify`
   - **If changes touch slow test areas** (ML models, slow integration/E2E), consider running slow tests before commit (optional)
   ...
   - **Exception (efficiency):** `ci-fast` or full `make ci` already **passed in this session** on the **current** tree and **no substantive edits** after — skip duplicate run; say so briefly
   - **Exception (incremental):** User declines a full rerun or the change maps to a narrow target — follow `.cursorrules` (**rule 5c**); state what was skipped; pre-commit still runs `make ci-fast` unless `--no-verify`
   - **After pre-commit passes:** Push directly (no need to run `make ci` again right after a green `ci-fast` on the same diff)
   - **When any make target fails (test, ci, lint, format, docs, etc.):** Establish root cause first (reproduce, read failure, trace to source); fix from there. Do not experiment randomly.
6. **CONDITIONALLY run `make docker-test` before committing** (when Docker-related changes exist) (CRITICAL)
   - **MANDATORY:** If you changed `docker/pipeline/Dockerfile`, `pyproject.toml` structure, or package layout → Run `make docker-test` before committing
   - **NEVER commit Docker-related changes without running `make docker-test` first.**
7. Install pre-commit hook with `make install-hooks` (prevents CI failures)
8. Run `make fix-md` before committing markdown files (fixes common linting issues)
9. **Self-check markdown files before finishing:** Scan for one H1 only, blank lines after headings, no trailing spaces, consistent list indentation, code fences have language tags
9. Run `make format` and `make test-fast` before committing (if no hook)
5. Respect module boundaries (no business logic in CLI, no HTTP in config, etc.)
6. Mock external dependencies in tests
7. Add docstrings to all public functions (Google-style)
8. Use Config for all runtime options
9. Never commit secrets or personal data
10. Create PRD for user-facing features, RFC for architecture changes
11. Update README when CLI changes
12. Follow conventional commit format
13. Ask when uncertain, don't ask for obvious best practices
14. Use lazy loading pattern for `cli` and `service` imports
15. Update index files (`docs/index.md`, `docs/rfc/index.md`, `docs/prd/index.md`) when adding RFCs/PRDs
16. Use `python-dotenv` for environment variable management (`.env` files)
17. Treat prompts as provider-specific concerns (not part of core protocol)

**When in doubt:**

- Check existing code for patterns
- Look at similar functions for examples
- Read docs/architecture/ARCHITECTURE.md for design principles
...
## How to Ensure AI Follows These Rules
