...
1. ✅ **Fix linting issues** - Run `make format` or fix issues manually
2. ✅ **Fix markdown issues** - Run `python scripts/fix_markdown.py` for markdown files
3. ✅ **Run checks locally** - Verify `make ci` passes before committing
4. ✅ **Only use `--no-verify`** if the user explicitly says "use --no-verify" or "skip hooks"
...
**🚨 TOP 3 CRITICAL RULES (MUST FOLLOW - NO EXCEPTIONS):**

1. ✅ **NEVER bypass pre-commit hooks** (CRITICAL)
