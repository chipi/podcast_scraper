...
At **session end**, **"done"**, or **"wrap up"** (same **cadence** as a final metrics append when you use
one), do a short **retrospective** (can be silent): what would have closed the task faster or avoided
claiming **"fixed"** too early? **Do not** put that text into **`rule-adherence.jsonl`** — metrics stays
rule/skills/subagents only. When a lesson should become **shared process**, update the right guide (e.g.
**`docs/guides/AGENT_BROWSER_LOOP_GUIDE.md`** for UI/browser) or **`.cursorrules`** /
**`.cursor/rules/*.mdc`** (or **`docs/wip/`** for drafts). Authoritative text: **`.cursorrules` Rule 18**.
...
- **Dockerfile** (`docker/Dockerfile`) - Any changes to build process, dependencies, or structure
...
- **Never:** Only Python code changes (no Dockerfile or structure changes)
...
6. **CONDITIONALLY run `make docker-test` before committing** (when Docker-related changes exist) (CRITICAL)
   - **MANDATORY:** If you changed Dockerfile, pyproject.toml structure, or package layout → Run `make docker-test` before committing
