...
- **`check_unit_test_imports.py`** - Verifies unit tests can import without ML dependencies
  ...
  - **Usage:**
    ...
    ```
  - **What it does:** Ensures modules can be imported without ML dependencies (spacy, torch, transformers) installed, which is required for unit tests to run in CI
  - **Why it matters:** Unit tests run without ML dependencies in CI for speed. If a module imports ML deps at the top level, CI will fail
