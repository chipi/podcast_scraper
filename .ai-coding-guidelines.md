...
1. ❌ **NEVER create `docs/analysis/` folder** - This folder MUST NOT EXIST
2. ❌ **NEVER create `docs/plan/` folder** - This folder MUST NOT EXIST
3. ❌ **NEVER create analysis files anywhere except `docs/wip/`**
...
- `docs/wip/critical-path-analysis.md` ✅
- `docs/wip/episode-length-analysis.md` ✅
- `docs/wip/e2e-fast-trimming-proposal.md` ✅
- `docs/wip/integration-fast-trimming-proposal.md` ✅

**Examples of what is FORBIDDEN:**
- `docs/analysis/critical-path-analysis.md` ❌ **NEVER DO THIS**
- `docs/plan/implementation-plan.md` ❌ **NEVER DO THIS**
- `docs/review/architecture-review.md` ❌ **NEVER DO THIS**
...
1. ❌ **NEVER push directly to main branch** - Even if branch protection allows it, DON'T DO IT
2. ❌ **NEVER commit fixes directly to main** - Always use a feature branch
3. ❌ **NEVER bypass the PR process** - Even for "quick fixes" or "urgent" changes
...
- ❌ This means you pushed directly to a protected branch - YOU MADE A MISTAKE
...
3. ⚠️ **Fallback to GitHub API only if:**
...
4. ❌ **NEVER use GitHub API directly when MCP server is available**
...
- ❌ Only configuration file changes (that don't affect ML)
- ❌ Only fast integration/E2E test changes
- ❌ Code changes that don't touch ML model code paths
...
```text
- ❌ Docker builds (unless working on Docker specifically OR before pushing Docker-related changes to PR)
...
def apply_log_level(level: str):
    # ... setup logging ...
    # ❌ NO! OpenAI-specific logging config doesn't belong here
...
3. ❌ Don't put it in `workflow.py`, `config.py`, or other central modules
4. ❌ Don't make central code aware of provider internals
...
- ❌ **`docs/analysis/`** - THIS FOLDER MUST NEVER EXIST. All analysis goes in `docs/wip/`
- ❌ **`docs/plan/`** - THIS FOLDER MUST NEVER EXIST. All plans go in `docs/wip/`
- ❌ Root directory
- ❌ `docs/` root (without subdirectory)
- ❌ `docs/rfc/` (only for formal RFCs)
- ❌ `docs/prd/` (only for formal PRDs)
...
### ⚠️ CRITICAL: Check `mkdocs.yml` and Links Before Pushing Documentation Changes
...
- ❌ You CANNOT update past release notes (already shipped versions)
- ✅ You CAN update other documentation (README, guides, etc.) to reflect current state
- ❌ You CANNOT "fix" historical release notes to match current codebase

**Examples:**

- ❌ Don't update `RELEASE_v2.3.1.md` to remove `requirements.txt` references (it was there when v2.3.1 shipped)
- ✅ You can update `README.md` to remove `requirements.txt` references (current documentation)
- ❌ Don't change dependency lists in old release notes
...
- ❌ Branch contains old commits that are already merged
- ❌ Branch has commits from other features/PRs
- ❌ Branch is based on an outdated version of main
- ❌ Branch has merge commits from other branches (unless intentional)
...
**⚠️ CRITICAL:** Steps 3-8 are mandatory when applicable. Step 2 (ci-fast) may be skipped only per **recent CI on current tree** / user waiver (see `.cursorrules` rule 5). Do NOT assume approval.
...
- ❌ Only Python code changes (no Dockerfile or structure changes)
- ❌ Only documentation changes
- ❌ Only test file changes
- ❌ Only configuration file changes (that don't affect Docker)
...
- ❌ **User hasn't explicitly approved the push** (MOST IMPORTANT - CHECK THIS FIRST)
- ❌ **You haven't shown `git status` first**
- ❌ **You haven't shown `git diff` or change summary first**
- ❌ **User said "don't push" or "wait"**
- ❌ Pre-commit hook failed (fix issues before pushing)
