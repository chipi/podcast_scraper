...
1. ✅ Make code changes
2. ✅ **MANDATORY: Run `make ci-fast`** - Fast validation before committing (formatting, linting, fast tests). Exception: workflow-only changes (only `.github/workflows/*.yml`).
...
9. ✅ **THEN commit** - Only after steps 2-8 are complete (pre-commit hook will run additional checks)

**⚠️ CRITICAL: Steps 2-8 are MANDATORY. Do NOT skip any step. Do NOT assume approval.**
...
**Testing Strategy:**
- **Before commit:** Run `make ci-fast` (fast validation: formatting, linting, fast tests)
- **Pre-commit hook:** Automatically runs `make ci-fast` checks when committing
- **After pre-commit passes:** Push directly (no need to run `make ci` again)
...
4. ✅ **ALWAYS run `make ci-fast` before committing** (CRITICAL)
   - **MANDATORY:** Run `make ci-fast` before committing (fast validation: formatting, linting, fast tests)
   - **MANDATORY:** Pre-commit hook will also run `make ci-fast` checks automatically
   - **MANDATORY:** If changes touch slow test areas (ML models, slow integration/E2E), consider running slow tests before commit (optional)
