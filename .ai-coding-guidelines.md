...
[How to test this]
```
**PRD Template Location:** `docs/prd/PRD_TEMPLATE.md`

**RFC Template Location:** `docs/rfc/RFC_TEMPLATE.md`
...
- ✅ Add blank line after list if next line is not empty
- ✅ Blank lines are required even if list is inside a code block or table cell (when possible)
...
- ✅ For complex tables, use Python to generate exact alignment (see `docs/MARKDOWN_LINTING.md`)
...
```
**Rules:**
- ✅ Always include language specifier: ` ```python`, ` ```bash`, ` ```yaml`, etc.
...
- ✅ Use single blank line to separate paragraphs, lists, code blocks, etc.
...
3. ✅ **Verify with `make lint-markdown`** if markdownlint is installed
...
**See also:**
- `docs/MARKDOWN_LINTING.md` - Complete markdown linting guide
...
```
**🚨 MANDATORY: Always Create a New Clean Branch for PRs**

**When creating a PR, you MUST:**

1. ✅ **Create a new branch from the target branch** (usually `main` or `master`)
...
```
- ✅ Branch starts from the latest `main` (or target branch)
...
```
# Option 1: Create a fresh branch (RECOMMENDED)
git checkout main
git pull origin main
git checkout -b feature/new-feature-name-clean
...
1. ✅ Make code changes
2. ✅ **If editing markdown files:** Run `python scripts/fix_markdown.py` to fix common linting issues
3. ✅ **MANDATORY: Run `git status`** - Show user what files changed
4. ✅ **MANDATORY: Run `git diff`** - Show user the actual changes (for each modified file)
5. ✅ **MANDATORY: Wait for explicit user approval** - User must explicitly say "commit", "yes", "go ahead", etc.
6. ✅ **MANDATORY: Get commit message** - User provides message OR you ask "What commit message should I use?"
7. ✅ **THEN commit** - Only after steps 3-6 are complete

**⚠️ CRITICAL: Steps 2-5 are MANDATORY. Do NOT skip any step. Do NOT assume approval.**
...
```
1. ✅ Run `make ci` locally to verify all checks pass
2. ✅ **Conditionally run `make docker-test`** if Docker-related changes exist (see below)
3. ✅ Fix any failures before pushing
4. ✅ Only push after `make ci` (and `make docker-test` if applicable) passes completely
...
# If CI fails → Fix issues → Run make ci (and docker-test) again → Ask for approval again
```
...
git push origin feature/branch-name
```
git push  # ❌ NO! User hasn't reviewed or approved!
```
...
Am I about to push?
  └─ Yes → Run make ci
      └─ Passes? → Push
          └─ No? → Fix and retry
```
    Hook fails → make format → Retry commit
```
...
- **[docs/DEVELOPMENT_NOTES.md](docs/DEVELOPMENT_NOTES.md)** - **📚 Detailed technical information** (code style, testing, CI/CD, architecture, logging, documentation standards, markdown linting)
...
- Code style details → See `docs/DEVELOPMENT_NOTES.md` (Code Style Guidelines section)
- Testing details → See `docs/DEVELOPMENT_NOTES.md` (Testing Requirements section)
- CI/CD details → See `docs/DEVELOPMENT_NOTES.md` (CI/CD Integration section)
- Architecture patterns → See `docs/DEVELOPMENT_NOTES.md` (Architecture Principles section)
- Logging patterns → See `docs/DEVELOPMENT_NOTES.md` (Logging Guidelines section)
- Documentation standards → See `docs/DEVELOPMENT_NOTES.md` (Documentation Standards section)
...
4. ✅ **ALWAYS run `make ci` before pushing to PR** (new or updated PR) (CRITICAL)
   - **MANDATORY:** Run `make ci` → Verify passes → Fix failures → Ask for approval → THEN push
   - **NEVER push without running `make ci` first.**
5. ✅ **CONDITIONALLY run `make docker-test` before pushing to PR** (when Docker-related changes exist) (CRITICAL)
...
4. ✅ Install pre-commit hook with `make install-hooks` (prevents CI failures)
5. ✅ Run `python scripts/fix_markdown.py` before committing markdown files (fixes common linting issues)
6. ✅ Run `make format` and `make test` before committing (if no hook)
