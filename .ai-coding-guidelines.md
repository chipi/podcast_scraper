...
>
> This file (`.ai-coding-guidelines.md`) focuses on **AI-specific workflow** and **critical rules** for AI assistants. For detailed technical patterns and implementation details, refer to `docs/DEVELOPMENT_GUIDE.md`.
...
**For detailed guidance:** See [`docs/guides/CURSOR_AI_BEST_PRACTICES_GUIDE.md`](docs/guides/CURSOR_AI_BEST_PRACTICES_GUIDE.md) for comprehensive model selection strategies, project-specific workflows, and prompt templates.
...
# Step 1: Ensure you're on the target branch and it's up to date
...
1. ✅ Make code changes
2. ✅ **If editing markdown files:** Run `make fix-md` (or `python scripts/fix_markdown.py`) to fix common linting issues
3. ✅ **MANDATORY: Self-check markdown files** (if any were edited):
...
4. ✅ **MANDATORY: Run `git status`** - Show user what files changed
5. ✅ **MANDATORY: Run `git diff`** - Show user the actual changes (for each modified file)
6. ✅ **MANDATORY: Wait for explicit user approval** - User must explicitly say "commit", "yes", "go ahead", etc.
7. ✅ **MANDATORY: Get commit message** - User provides message OR you ask "What commit message should I use?"
8. ✅ **THEN commit** - Only after steps 3-7 are complete

**⚠️ CRITICAL: Steps 2-7 are MANDATORY. Do NOT skip any step. Do NOT assume approval.**
...
1. ✅ **Run appropriate tests first** (see [Testing After Changes](#-critical-running-tests-after-making-changes) section):
...
2. ✅ Run `make ci` locally to verify all checks pass (includes fast tests with coverage)
3. ✅ **Conditionally run `make docker-test`** if Docker-related changes exist (see below)
4. ✅ Fix any failures before pushing
5. ✅ Only push after all tests and `make ci` (and `make docker-test` if applicable) pass completely
...
# If tests or CI fails → Fix issues → Run tests and make ci again → Ask for approval again
...
4. ✅ **ALWAYS run appropriate tests after making changes** (CRITICAL)
...
5. ✅ **ALWAYS run `make ci` before pushing to PR** (new or updated PR) (CRITICAL)
   - **MANDATORY:** Run `make ci` → Verify passes → Fix failures → Ask for approval → THEN push
   - **NEVER push without running `make ci` first.**
6. ✅ **CONDITIONALLY run `make docker-test` before pushing to PR** (when Docker-related changes exist) (CRITICAL)
   - **MANDATORY:** If you changed Dockerfile, pyproject.toml structure, or package layout → Run `make docker-test` → Verify passes → Ask for approval → THEN push
