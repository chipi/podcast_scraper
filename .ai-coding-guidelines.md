...
**Note:** Pre-commit hooks already run `make ci-fast` checks. After pre-commit passes, you can push
directly without running `make ci` again.
...
2. **`web/gi-kg-viewer/e2e/*.spec.ts`**, **`helpers.ts`**, **`fixtures.ts`** — Keep Playwright green; run **`make test-ui-e2e`**.
3. **`docs/uxs/`** — Update **`UXS-001`** for **shared** tokens/shell UX; update the relevant **feature UXS** (`docs/uxs/index.md`) when a **surface-specific** spec changes, not only when tests fail.
...
```text
- Run `make ci-fast` before committing (pre-commit hook will also run this; exception: workflow-only changes)
...
1. Make code changes
2. **Run `make ci-fast` when needed** — Fast validation before committing (formatting, linting, fast tests). **Exceptions:** workflow-only changes (only `.github/workflows/*.yml`); **recent green run:** `make ci-fast` or `make ci` already passed in this session on the **same** tree and **no substantive edits** were made after that run (state briefly that you are skipping a duplicate run); user explicitly waives re-validation; **incremental validation:** user says no full CI rerun or the change is a narrow follow-up — run the Makefile targets in `.cursorrules` (**rule 5c**), report what was skipped, and note pre-commit still runs `make ci-fast` unless `--no-verify`. **Re-run** full `make ci-fast` if anything material changed after the last green run, the last run failed, you are unsure the tree matches what was validated, or rule 5c does not apply.
...
** CRITICAL:** Steps 3-8 are mandatory when applicable. Step 2 (`ci-fast`) may be skipped only per **recent CI on current tree**, **user waiver**, or **incremental validation** (see `.cursorrules` rules **5** and **5c**). Do NOT assume approval.
...
**Testing Strategy:**
- **Before commit:** Run `make ci-fast` (fast validation: formatting, linting, fast tests), **unless** the same check already passed in-session on the current diff with no substantive edits after (efficiency — see `.cursorrules` rule 5)
...
4. **Run `make ci-fast` before committing when needed** (CRITICAL)
   - **Default:** Run `make ci-fast` before committing (fast validation: formatting, linting, fast tests)
