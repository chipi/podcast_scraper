...
1. ✅ Install pre-commit hook with `make install-hooks` (prevents CI failures)
2. ✅ Run `make format` and `make test` before committing (if no hook)
3. ✅ Respect module boundaries (no business logic in CLI, no HTTP in config, etc.)
4. ✅ Mock external dependencies in tests
5. ✅ Add docstrings to all public functions (Google-style)
6. ✅ Use Config for all runtime options
7. ✅ Never commit secrets or personal data
8. ✅ Create PRD for user-facing features, RFC for architecture changes
9. ✅ Update README when CLI changes
10. ✅ Follow conventional commit format
11. ✅ Ask when uncertain, don't ask for obvious best practices
12. ✅ Use lazy loading pattern for `cli` and `service` imports
13. ✅ Update index files (`docs/index.md`, `docs/rfc/index.md`, `docs/prd/index.md`) when adding RFCs/PRDs
14. ✅ Use `python-dotenv` for environment variable management (`.env` files)
15. ✅ Treat prompts as provider-specific concerns (not part of core protocol)
