...
- Do **not** require **`[ml]`**, **`[llm]`**, **`[compare]`**, or other non-`dev` extras. Use mocks,
  **`patch.dict(sys.modules, …)`**, or move coverage to **`tests/integration/`**.
- **`[dev]`** is the baseline declared in `pyproject.toml`. **`[server]`** is added **only** on CI
  (and typical local full dev) for the **viewer FastAPI** unit slice — use
  **`pytest.importorskip("fastapi")`** at module top if the file imports FastAPI, so plain
  **`pip install -e .[dev]`** still collects tests (viewer tests skip locally without server).
- Do **not** use **`importorskip("faiss")`**, real **`FaissVectorStore`** builds, or real Whisper /
  spaCy in unit tests.
...
- ✅ Changed integration test code marked with `@pytest.mark.slow` or `@pytest.mark.ml_models`
...
- **`check_unit_test_imports.py`** - Verifies selected **library** modules import without the heavy ML stack
  ...
  - **Usage:**
    ...
    ```
  - **What it does:** Ensures listed modules can be imported without **ML** optional deps (spacy, torch, transformers, etc.) at import time. It does **not** install **`[server]`**. CI **`test-unit`** also uses **`.[dev]`** only; FastAPI unit tests **skip** without **`[server]`** — install **`.[dev,server]`** locally to run them.
  - **Why it matters:** Prevents regressions where core code pulls ML in at import time. **Authoring rule:** `tests/unit/` must not depend on **`[ml]`** / **`[llm]`** — see `docs/guides/UNIT_TESTING_GUIDE.md` (*Pyproject extras*).
