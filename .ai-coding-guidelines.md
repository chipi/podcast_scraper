...
- **`check_unit_test_imports.py`** - Verifies selected **library** modules import without the heavy ML stack
  ...
  - **Usage:**
    ...
    ```
  - **What it does:** Ensures listed modules can be imported without **ML** optional deps (spacy, torch, transformers, etc.) at import time. It does **not** install **`[server]`**; full unit coverage including FastAPI still relies on **`.[dev,server]`** in the **`test-unit`** workflow step.
