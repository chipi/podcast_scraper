...
1. ❌ **NEVER push to PR without running `make ci` first**
2. ❌ **NEVER push when `make ci` has failures**
3. ✅ **ALWAYS run `make ci` before pushing to PR (new or updated)**
4. ✅ **ALWAYS fix all CI failures before pushing**
...
# Step 1: Always run full CI
make ci

# Step 2: Conditionally run Docker tests (if Docker-related changes)
...
# If passes → Push to PR
# If fails → Fix issues → Run make ci (and docker-test) again → Push
```
...
2. ✅ **ALWAYS run `make ci` before pushing to PR** (new or updated PR) (CRITICAL)
   - **MANDATORY:** Run `make ci` → Verify passes → Fix failures → THEN push
   - **NEVER push without running `make ci` first.**
3. ✅ **CONDITIONALLY run `make docker-test` before pushing to PR** (when Docker-related changes exist) (CRITICAL)
   - **MANDATORY:** If you changed Dockerfile, pyproject.toml structure, or package layout → Run `make docker-test` → Verify passes → THEN push
   - **NEVER push Docker-related changes without running `make docker-test` first.**
3. ✅ Install pre-commit hook with `make install-hooks` (prevents CI failures)
4. ✅ Run `make format` and `make test` before committing (if no hook)
