...
14. [Git Worktrees (Recommended)](#git-worktrees-recommended)
15. [Decision Trees](#decision-trees)
16. [When to Ask](#when-to-ask)
...
1.  ✅ **Use worktrees for parallel tasks**: If you need to work on a new feature while waiting for PR feedback, suggest creating a new worktree.
2.  ✅ **Isolation is critical**: Each worktree MUST have its own `.venv`.
3.  ✅ **Reference the guide**: See `docs/guides/GIT_WORKTREE_GUIDE.md` for full details.

### Common Commands (AI use only when instructed)

```bash

# Setup new worktree (interactive)
...
- ✅ Run `python scripts/fix_markdown.py` before committing
...
- ✅ Run `python scripts/fix_markdown.py` to auto-fix table separators
...
```text
- ✅ Run `python scripts/fix_markdown.py` to auto-fix multiple blank lines
...
2. ✅ **Run `python scripts/fix_markdown.py`** to auto-fix common issues:
...
3. ✅ **If editing markdown files:** Run `make fix-md` (or `python scripts/fix_markdown.py`) to fix common linting issues
