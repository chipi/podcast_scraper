...
- Do **not** require **`[ml]`**, **`[llm]`**, **`[compare]`**, **`[server]`**, or any other
  non-`[dev]` extra. Use mocks, **`patch.dict(sys.modules, …)`**, or move the test to
  **`tests/integration/`**.
...
- `tests/integration/`: zero skips expected on CI integration jobs (`.[dev,ml,llm,server]`).
...
- **`check_unit_test_imports.py`** - Verifies selected **library** modules import without the heavy ML stack
  ...
  - **Why it matters:** Prevents regressions where core code pulls ML in at import time. **Authoring rule:** `tests/unit/` must not depend on any non-`[dev]` extra (`[ml]`, `[llm]`, `[server]`, `[compare]`) — if a test needs one, move it to `tests/integration/`. See `docs/guides/UNIT_TESTING_GUIDE.md` (*Pyproject extras*).
