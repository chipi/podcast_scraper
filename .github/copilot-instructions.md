...
- **Never:** NEVER push any branch without explicit user approval (commits OK after diff approval, pushes NEVER by default)
- **Never:** NEVER commit without showing changes and getting user approval
- **Never:** NEVER push to main branch (always use feature branches)
...
1. **Never:** **NEVER commit without first showing `git status`**
2. **Never:** **NEVER commit without first showing `git diff`**
3. **Never:** **NEVER commit without explicit user approval**
4. **Never:** **NEVER commit when user said "don't commit" or "wait"**
...
1. **Never:** **NEVER push to PR without explicit user approval**
2. **Never:** **NEVER push without showing `git status` and `git diff` first**
3. **Never:** **NEVER push when user said "don't push" or "wait"**
4. **Never:** **NEVER push to PR without running `make ci` first**
5. **Never:** **NEVER push when `make ci` has failures**
