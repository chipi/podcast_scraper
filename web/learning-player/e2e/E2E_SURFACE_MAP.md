...
| `OrgCardContent` | The organization BODY of the entity card (#2031): mentioned-in episodes + co-occurring people / other orgs / topics, rendered by the `EntityCardBody` shell. Lean by design — orgs have no web bio/photo. | ⚠️ **no e2e** — the `app-validation-corpus` fixture contains **no organizations** (`MENTIONS_ORG` count 0), so no e2e can open an org card against it. Unit-tested in `OrgCardContent.test.ts` + the projection in `test_app_relational_view.py`. e2e follows once the fixture (or an org-bearing corpus) carries orgs. |
| `ShareMenu` | Share affordance (#2036) in the entity-card header (`share-menu`): a menu of **Share card** (editorial PNG via `entityShareCard`), **Share link** (canonical URL), **Share text** (caption). Card render is client-canvas (bridge-only). | ⚠️ **no e2e yet** — unit-tested (`entityShareCard.test.ts` covers the caption + link + graceful null-canvas render). e2e (menu opens, actions fire) follows when share surfaces are wired app-wide. |
