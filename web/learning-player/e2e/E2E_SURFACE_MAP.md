...
| `/browse` | `browse` | [BrowseView](../src/views/BrowseView.vue) | auth | Discover hub — `data-testid="browse-view"`. A trending-shows tile row on top (`trending-shows-rail`, "See all →" `trending-shows-seeall` → the Shows tab pre-sorted by Trending), then the **search box** (`browse-search-section` / `browse-search-input` / `browse-search-submit`) — search folded into Discovery, operator 2026-09-20, placed where the page turns from "what's popular" to "go find something"; the same search-form markup as Home's Ask box (shared CSS class, not a selector) so the two entry points are one control, and submitting pushes `/search?q=` — then a **"Trends"** heading over the shared `DiscoveryExplorer` (`data-testid="discovery-explorer"`; same tabbed `DiscoveryList` as Home, capped at 10 rows, with a per-kind expand control `discovery-see-all` that uncaps the list IN PLACE — it was a RouterLink to `/browse?trends={kind}`, i.e. to the page it is already on, so the tap did nothing; operator 2026-09-19), then a content band with two tabs (`data-testid="browse-tab-{tab}"`, where `{tab}` is `episodes` or `shows`). Each panel is kept mounted (`data-testid="browse-panel-episodes"`, `data-testid="browse-panel-shows"`). (operator 2026-09-14, replaced the top-3 `DiscoveryDashboard`.) |
...
| ------- | -------------- | ------------- | ---------- |
| **App shell / nav** | Header brand → home; `<nav>` NavIconLink **Search** (all widths) / **Browse** / **Library** / **Profile** (`sm:` and up); **Sign in** / **Sign up** when signed out. Phone bar = **Home · Discover · Library**, each `bottom-nav-{tab}` (three tabs; Search folded into Discovery 2026-09-20 — the bottom-nav-search testid is deliberately absent, and specs assert its absence) | Every page | `smoke.spec.ts` (+ implicit in all), `search-in-discovery.spec.ts` (tab count, masthead reachability, Discovery highlight) |
...
| **Search** | Corpus semantic search; passage hits + **KnowledgePanel** (entity chips → card); entity-in-search resolution. NOT a tab: `/search` lights **Discovery** in the bottom bar (operator 2026-09-20 — knowingly overrules BottomNav's "a wrong 'you are here' is worse than none", because search has a canonical parent where an episode does not) | `goto('/search?q=…')`, Home Ask-box submit, Discovery's box (`browse-search-input`/`browse-search-submit`), masthead magnifier (`masthead-search`) | `home-search.spec.ts`, `consolidation.spec.ts` (`?q=index`), `search-in-discovery.spec.ts` (entry points + highlight) |
...
| **Podcast (show)** | Show page → episode list, **show signals band** (`podcast-signals` + `ps-theme` / `ps-distinctive-topic` / `ps-topic` / `ps-person` rows — the corpus-wide "Trending here"/`ps-trending` row was removed, its velocity being corpus-wide not show-specific), publishing-cadence chart (`show-activity`), the **feed meta line** — authors, episode count, cadence, typical length and last-updated, composed once and rendered TWICE with one visible at a time: `podcast-feed-meta` under the artwork on a phone (`sm:hidden`) and `podcast-feed-meta-wide` under the title from `sm` up, because the line cannot fit a 144px gutter. The separate `podcast-byline` element is GONE (#2043's author names are now the line's first entry, so separators fall between the fields actually present), and **language is deliberately omitted** — a constant across an all-English corpus. A spec running on both viewport projects must therefore match whichever copy is visible, not pick one. Also the **Follow show** toggle (`follow-show`, `aria-pressed`) — a *feed subscription*, distinct from interest follows | `goto('/podcast/:feedId')` (e.g. `p05`) | `follow-show.spec.ts`; also reached by `auth-queue`, `capture`, `consolidation`, `perspectives`, `entity-signals`, `transcript*` |
...
| **Saved → highlight card** | A capture in Library → Saved (`HighlightsView`). Each episode group is headed by `highlight-group-heading` — the episode's artwork at the downloads-row size (`h-10 w-10`) in front of its title, linking to the episode. Each card leads with its kind kicker (Marked moment / Transcript / Insight) sharing ONE line with the icon actions pushed hard right: the colour control, share (`↗`) and `highlight-delete`. The captured words render as `highlight-quote` — a moment stores the spoken line at capture time, so a card that shows only its kicker means a record written before that landed. `highlight-add-note` opens the inline note editor. | `goto('/library')` with ≥1 capture | `capture.spec.ts` (asserts the moment carries a non-empty `highlight-quote`, end to end from marking it); `HighlightsView.test.ts` |
...
| **Storyline page** — `StorylineView` (`storyline-view`, `storyline-follow`, route `storyline`). Reached by opening a storyline from the Discover explorer (`/browse`, `discovery-row`) or via the deep link `/browse?trends=storyline` (`discovery-row`). | `storyline.spec.ts` |
...
| **Highlights view** — `HighlightsView` | The PDF chip (`export-pdf`) opens `/api/app/highlights/export.html` — the same export document with a print stylesheet, so the browser's own Save-as-PDF does the conversion and no PDF library enters the API image. It opens a real tab rather than printing from a hidden iframe (the user has to see what they are printing), which is why e2e asserts the URL and the route's bytes rather than driving a print dialog Playwright cannot see. Export is covered by `capture.spec.ts`, which now FETCHES `/api/app/highlights/export.md` and asserts the bytes — an absolute `[m:ss](https://…/episode/…?t=N)` jump link, the capture kind, the capture date, and that a filtered export actually narrows. Asserting the href shape alone passed while the export had an unreachable link and no date or kind. Notes are covered there too; the share-card control (`highlights.share`) hands off to the OS share sheet, which a browser cannot drive. |
...
| `CheckIcon` / `BellOffIcon` | Drawn icons for two of the Revisit card's three outcomes — a tick (`revisit-dismiss`, mark as reviewed) and the masthead's own bell with a slash through it (`revisit-retire`, stop resurfacing this). The third is `BookmarkIcon`, below. Both are SVG, not characters: `CloseIcon` records that "✕" as a glyph rendered as a tofu box in the iOS UI font on every sheet at once, and that drawing it was the only fix that held — so "✓" and "🔕" are not re-run as experiments. `BellOffIcon` reuses the bell path from `NotificationsBell` verbatim, so the icon inherits a meaning the user already learned. Library → Revisit cards. | `ResurfacingInbox.test.ts` |
| `BookmarkIcon` | The capture bookmark — filled when saved, outlined when not. Shared by the transcript's save-this-line control and the Revisit card's third outcome (`revisit-delete`, "Saved — tap to remove"), which shows it FILLED: that action is an unsave, so it shows the glyph that did the saving rather than a generic ✕ (operator 2026-09-18). A bookmark and not a heart because of which save it was — the heart (`FavoriteButton`) takes episodes, people, topics, shows and storylines, while a Revisit item is always a capture, and `types.ts` states that an insight "is a capture, saved via the highlights path, never a favorite" (RFC-121 / #1593). Drawn, never the character 🔖, per `CloseIcon`. | `ResurfacingInbox.test.ts`; `icon-rendering.spec.ts` (drawn-icon assertion covers the transcript control) |
| `RevisitRail` | Home's "Worth revisiting" rail (`home-revisit-rail`) — up to four due captures, STRICTLY one per episode (filling from an episode already shown rendered two cards with identical words). Each is the quote with the episode as a small square thumbnail on its right, plus two inline 32px circles — tick and bell — that drop the card and backfill the slot. It links to the Revisit tab focused on that capture (`?focus=<id>`), NOT to the player, which would have marked it reviewed on arrival (#35) and decided the one thing the user went there to decide. It fills the right half of the `lg` row beside `home-discovery`, which had been half-width with an empty neighbour. Exists because review VOLUME is the only thing that moves the resurfacing loop (7/week -> 50% of captures, 14/week -> 99%) while cadence alone is worth nothing — so the win is asking someone already on Home, not sending more email. `home-personal-rails.spec.ts` pins the EMPTY case end to end (no rail when nothing is due, and the Revisit tab agreeing). **PARTIAL GAP:** the populated case cannot be produced in e2e — a capture is not due until the 2-day rung and a run creates them seconds ago — so one-card-per-episode, drop-and-backfill on both actions, and restore-on-failure are covered in `RevisitRail.test.ts`, where the store can be driven directly. | `RevisitRail.test.ts`, `home-personal-rails.spec.ts` |
| `CollectionsTeaser` | Home's "Your boards" teaser (`home-collections-teaser`) — up to four boards as cover + name + count (`home-collection-tile`), most recently CHANGED first, filling the right half of the `lg` row beside the ask box, which was empty because that input is deliberately capped. Boards were reachable only via Library -> Boards, which made the thing the user ASSEMBLED the least visible thing they own. A board with no members renders a flat tile rather than a broken image, since `cover_url` is derived from its first member. Covered end to end by `home-personal-rails.spec.ts`, which creates a board with a member, asserts it reaches Home, then clicks the tile and asserts the board lands EXPANDED (mutation-tested: removing the deep-link open fails it). | `CollectionsTeaser.test.ts`, `home-personal-rails.spec.ts` |
| `SectionHeading` | The ONE section header for Home (`section-kicker` + `section-title`, plus an optional action slot). Home had grown three by hand — kicker above the title on some sections and beside it on others, three title fonts (`h1` display, `h2` display, `lp-section`), and nothing stopping either line wrapping to a second row. Both lines truncate; the kicker sits ABOVE so every title starts at the same x and does not fight the action link for the right edge. Used by Jump back in, Your Week, Worth revisiting, Ask, Your boards, What's new and Recommended. The resume hero keeps its `h1` as the page's one deliberate exception. Asserted structurally by `HomeView.test.ts` (delegation) and rendered in every Home e2e. | `HomeView.test.ts` |
...
| `EpisodeGroupCard` | An episode-grouped result block (`episode-group`): the shared `EpisodeCard` as the header — artwork, show, title, date — with the group's rows collapsible beneath it via `episode-group-toggle` / `episode-group-body` ("Hide/Show {noun}", `aria-expanded`, `v-show` so inner state survives a collapse). Groups start EXPANDED and the per-group COUNT rides the card's `#aside` slot, so the toggle never repeats it. **Search only.** Library → Revisit used this until 2026-09-18 and now renders the flat Saved idiom instead (`EpisodeRow` heading + fold control in its `#trailing` slot + a flat list of capture cards) — Revisit is the same captures as Saved surfaced because they are DUE, so it reads the same way; Search's groups are results, and keep the full card. | exercised via `SearchView.test.ts` (`an episode group collapses its matches and starts expanded`); e2e via `search-listener-features.spec.ts` |
...
| `DiscoveryExplorer` | The shared discovery section (`discovery-explorer`) — the tabbed `DiscoveryList` (topics/storylines/people) + the Rising⇄Trending sort (`discovery-sort`) and Corpus⇄Mine scope (`home-trending-scope`) switches. Used by BOTH Home (capped 5, with an inline `discovery-expand` link OUT to Discover) and Discover (capped 10, with a `discovery-see-all` button that expands in place — Discover IS the `browse` route, so a link there goes nowhere). Extracted from HomeView so the two cannot drift (operator 2026-09-14, replaced the top-3 `DiscoveryDashboard`). | `browse-and-profile.spec.ts`, `trending.spec.ts` |
