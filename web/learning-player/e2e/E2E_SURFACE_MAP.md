...
| `CloseIcon` | The shared dismiss ✕, drawn as an inline SVG. It was the CHARACTER U+2715, which is absent from the iOS UI font and rendered as a tofu box on device — every sheet, panel and modal showed "?" where its close control belongs (2026-09-16). Font-stack fallbacks did not fix it; drawing it did. | no dedicated spec — exercised wherever a sheet/panel is dismissed (entity card, storyline, queue, interests, knowledge panel, player). A GAP: nothing asserts the icon RENDERS, because it is `aria-hidden` and invisible to both Playwright and XCUITest; it was caught by cropping a device screenshot. |
