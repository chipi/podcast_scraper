...
| `ShowAllToggle` | The "Show all (N) / Show less" control (`show-all-toggle`) under a capped Library section (#2042 follow-up) — each per-type section shows the top N and expands in place; a search lifts every cap. Library → Following + Saved | exercised via `LibraryView`; unit-adjacent |
