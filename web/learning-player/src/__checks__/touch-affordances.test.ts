...
describe('affordances survive on touch', () => {
  ...
  it('the queue reorder arrows carry it too', () => {
    ...
    const arrows = queueViewSrc.split('<button').filter((b) => b.includes('queue.up') || b.includes('queue.down'))
  ...
  it('the action row gap keeps neighbouring targets from overlapping', () => {
    ...
    const row = episodeCardSrc.slice(episodeCardSrc.indexOf('<div class="flex shrink-0 items-center'))
  ...
  it('the highlight swatches are 44px buttons, not 16px dots', () => {
    ...
    for (const b of swatches) {
      ...
      expect(b, 'the dot must be an inner span, not the button itself').toMatch(/<span[^>]*c\.swatch|<span[\s\S]*?c\.swatch/)
    }
  })
})
