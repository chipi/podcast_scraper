...
> **⚠️ This section is TEMPORARY.** It documents the *mocked* auth setup that exists only because 33
> of 38 specs here still route-mock `**/api/**`. That is drift from the intended architecture — the
> target is fixture-driven specs against a real backend bootstrapped from the synthetic corpus, as
> `web/learning-player` already does. When
> [#1619](https://github.com/chipi/podcast_scraper/issues/1619) lands, `mockSignIn` and this section
> are **deleted**, not maintained. Do not build on it.

**Every mocked-API spec must call `mockSignIn(page, role)` before navigating.** Omit it and the app
boots to `<LoginView>`, so every `getByTestId` on the shell times out. This is not hypothetical — it
is the ci-ui-full failure of 2026-07-18, **152 of 158 mocked-API specs timing out at 15 s**, which is
why the helper exists.

36 of 38 top-level specs call it; the exceptions are `auth-roles.spec.ts` (inline equivalent) and
`theme.spec.ts`.

### The gate

`App.vue:848-850` picks one of three renders, and **only when auth is enabled on the backend**:

| Condition | Renders |
| --------- | ------- |
| `auth.enabled && !auth.isAuthenticated` | `<LoginView>` — `login-button` |
| `auth.enabled && !auth.canUseViewer` | `<NoAccessView>` — `no-access-message`, `no-access-signout` |
| otherwise | the shell — also the path when auth is **not** configured (open, pre-auth behaviour) |

### Roles

Canonical vocabulary from `src/podcast_scraper/server/app_roles.py`.

| Role | Sees |
| ---- | ---- |
| `listener` | Learning Player only — `<NoAccessView>` here. Use only when asserting the no-access flow. |
| `creator` | Base shell: Digest / Library / Search / Graph. **No** Dashboard, Ops or Admin. **Use this for most mocked specs.** |
| `admin` | creator + Dashboard / Ops / Admin. Required for any admin-only surface. |

### Role-gated tabs — exact conditions

These differ from one another; do not assume a single rule.

| Tab | testid | Gate (`App.vue`) |
| --- | ------ | ---------------- |
| Dashboard | `main-tab-dashboard` | `!auth.enabled \|\| auth.isAdmin` — **:925** (so it also renders in open mode) |
| Ops | `main-tab-ops` | `!auth.enabled \|\| auth.isAdmin` — **:932** |
| Admin | `main-tab-admin` | `auth.isAdmin` **only** — **:946**. Hidden even when auth is disabled. |
| User menu | — | `auth.enabled` — :1020 |

> `helpers.ts` cites App.vue lines 675/682/696 for these gates; the real lines are 925/932/946, and
> its shorthand of `auth.isAdmin` for all three misses that **Admin alone** is hidden in open mode.
> The table above is the source of truth — a comment inside a helper is not a contract.

A Dashboard spec signed in as `creator` (the documented default for most mocked specs) times out
waiting for a tab that is deliberately absent. Sign in as `admin`.

### Other helper contracts specs depend on

| Helper | Contract |
| ------ | -------- |
| `mockSignIn(page, role)` | Installs a fallback route matching **host-rooted** `/api/` only (`/^https?:\/\/[^/]+\/api\//`) so boot calls don't hang, then stubs `/api/app/auth/status`. Registered **first** deliberately — Playwright routes are LIFO, so any per-spec `page.route` added afterwards wins. The host-rooted regex must not match the viewer's own `/src/api/*.ts` module URLs. |
| `signInIsolated(page, who, testInfo)` | Real mock-OAuth sign-in (`?as=` hint) for tier-3 walks — the pattern the whole suite moves to under #1619. |
| `signInAsAdmin(page)` | Admin variant of the above. |
| `waitForFsmReady` / `waitForFsmState` | Polls `window.__GIKG_FSM__`, with a 15 s stuck-detector budget. |
| `resetUserPreferences(page)` | USERPREFS-1 reset. Required discipline — a real cross-role state leak (V-G1 → V4) is documented at `helpers.ts:244-262`. |
| `readGraphEventsLog(page)` | Reads the per-user `graph_events.jsonl`. |

`playwright.config.ts:45-58` also sets `VITE_DEFAULT_GRAPH_LENS_DAYS=0` for fixture stability and
`VITE_ANALYTICS_OFF=1`. Specs depend on both.
...
| Palette per-hit actions | ``command-palette-action-open-workspace`` / ``command-palette-action-pin-rail`` / ``command-palette-action-pin-compare`` (§S8 — pins the hit's primary subject into ``search.comparePins``, prefilling the Compare picker) / ``command-palette-action-show-graph`` |
...
- `navigation` **Main views** — header tabs. Always present: **Digest** (`main-tab-digest`), **Library** (`main-tab-library`), **Search** (`main-tab-search`), **Graph** (`main-tab-graph`). Role-gated: **Dashboard**, **Ops**, **Admin** — see [Auth gate and roles](#auth-gate-and-roles-1128--read-this-before-writing-any-spec) for the exact conditions, which differ per tab. Scope tab clicks here so Playwright does not match **Load into graph** or unrelated **Library** substrings (`mainViewsNav` in [helpers.ts](helpers.ts)). *(This row previously listed four tabs and omitted Search, contradicting the Search-v3 sections; `helpers.ts:134` carries the same stale comment.)*
...
- `button` **all** (under **Edges** in **`graph-popover-edges`**) — re-enables all edge-type toggles (substring-safe: scope under popover or graph chrome).
- `button` **all** / **none** (under **Types** in **`graph-popover-types`**, opened via **`graph-chip-types`**) — node-type visibility; scope under graph chrome to avoid clashing with **Edges** in the sibling popover.
- Visible text **Sources** (inside **`graph-popover-sources`**) — merged **GI** / **KG** checkboxes, **Hide ungrounded** when applicable; each chip in the **`graph-filter-bar`** shows a **warning** dot when its facet is non-default (type toggles use **`graph-types-reset`** on the Types row inside the popover).
- `data-testid="graph-gesture-overlay"` / `graph-gesture-overlay-dismiss` / `graph-gesture-overlay-reopen` — first-load gesture card over **`.graph-canvas`**; **`graph-gesture-overlay-reopen`** sits **right** on the stats strip when a full graph is loaded, else in the bottom bar (see **Graph shell** table row); **Types** popover is **`graph-popover-types`** (opened via **`graph-chip-types`**).
...
- ~~Explore (#671 chip bar)~~ — **RETIRED, see the Search v3 §S1 note above.** `explore-filter-bar`, `explore-chip-topic`, `explore-chip-speaker`, `explore-chip-more`, the *Advanced explore* dialog and the **Explore** / **Clear** buttons **do not exist in `src/`** (verified 2026-08-12). Those filters are now chips on the search filter bar. Do not write specs against any `explore-*` selector.
...
- **PRD-033 FR1 (#884) results chrome + cards:**
  ...
  - ~~**`search-result-entity-link`**~~ (FR1.5) — **RETIRED** in the 2026-07-22 UX cleanup, along with the standalone **L** and **S** buttons: the whole result-row body is now the affordance, with the aria-label carrying the intent ("Open Topic/Person panel"). The id exists nowhere in `src/`, and `ResultCard.test.ts:291` asserts its **absence**. Target the row, not this selector. (Supporting-quote speaker links keep their existing **`search-result-speaker-link`**.)
...
- ~~**`node-detail-open-topic-profile`** / **`node-detail-open-person-profile`**~~ (PRD-033 FR5.3, #887) — **RETIRED by the node-view fold (#1141).** These buttons **do not exist in `src/`**; `TopicEntityView` / `PersonLandingView` are now embedded directly in NodeDetail's Details tab, which is what the "standalone rail retired" notes elsewhere in this file describe. The unit tests assert their **absence** — `NodeDetail.test.ts:211`, `:247`, `:448`. A spec written from this row would wait forever for a button that is deliberately gone.
...
- **`data-testid="node-detail-insight-prefill-search"`** / **`data-testid="node-detail-insight-explore-filters"`** — **Prefill semantic search** (truncated insight text) and **Set Explore filters** (clears topic/speaker, sets **Grounded only** + optional **Min confidence** in the store); disabled when API unhealthy. Asserted in **`search-to-graph-mocks.spec.ts`**. **Both testids exist, but the labels lie**: they still say "Explore", while the component emits `open-search-topic-filter` into the **Search** chip bar — Explore is gone. Inspect the result there, not via the retired `explore-advanced-open`. Label fix tracked in [#1600](https://github.com/chipi/podcast_scraper/issues/1600).
...
- **`data-testid="supporting-quote-speaker-unavailable"`** — muted line under expanded **supporting quotes** on **Search** result cards when the quote has text but no speaker (#541 contract). Asserted in **`search-to-graph-mocks.spec.ts`**. *(The Explore half of this row is retired: `explore-supporting-quotes-mocks.spec.ts` **does not exist** — verified 2026-08-12 — and neither does the Explore surface it covered.)*
...
- Collapsed **left** rail: the vertical affordance reads **Saved queries** — `LeftPanel` is now a Saved + Recent queries surface, not a search launcher, and it is visible on **all** tabs including Search (§S4-shell pivot). *(This row previously said the collapsed rail expands to "Search mode" and that **Explore** opens from the expanded column via "Explore corpus →" — both describe retired surfaces.)* Collapsed **right** rail: vertical **Details** (**`data-testid="rail-collapsed-subject"`**) expands the **SubjectRail** column only; `right-rail-edge-toggle` is its counterpart control.
...
Every spec file, so the map accounts for the suite and not only for surfaces. Audited 2026-08-12.

### Default suite (`playwright.config.ts`, 38 files)

Documented above **except** these, which had no map row at all:

| Spec | Covers | Note |
| ---- | ------ | ---- |
| `auth-roles.spec.ts` | the whole auth/role matrix — anonymous → `login-button`; listener → `no-access-message` / `no-access-signout`; creator sees no Dashboard/Ops/Admin; admin sees all + `users-admin`, `user-row-{email}`, and **self-lockout** (`role-select-admin@x.io` disabled) | Rules existed **only** here. See [Auth gate and roles](#auth-gate-and-roles-1128--read-this-before-writing-any-spec). |
| `search-fr1.spec.ts` | FR1 tier/compound badges | The testids were documented; the owning spec was not, so coverage looked absent. |
| `person-landing.spec.ts` (307 lines) | Person Landing | The Person sections name no owning spec. |
| `topic-entity-view.spec.ts` (387 lines) | Topic Entity View | Ditto — the densest testid sections in this file cited no spec. |
| `dashboard-pipeline-metrics-mocks.spec.ts` | `PipelineCleanupMetrics`, `PipelineAdExcisionMetrics` (#652/#663/#656) | Neither panel nor spec appeared in the Dashboard row. |
...
`playwright.config.ts` ignores `validation/` and `live/`; the handoff and perf suites run in the
default project but were never linked here.

| Directory | Files | Config | Status |
| --------- | ----- | ------ | ------ |
| `e2e/handoff/` | 11 specs + `_handoff-helpers.ts` | default | **Unlinked.** The graph-navigation E2E matrix above names an owning spec for one row and says "link new Playwright specs here" — never done, though these 20 files exist to do exactly that. |
| `e2e/handoff-production/` | 9 specs | default | **Unlinked**, same. |
| `e2e/perf/` | `perf-demonstrations.spec.ts` | default | Unlinked. |
