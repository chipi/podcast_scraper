...
| H1.2 | Digest recent topic pill (D1) | `cold-start.spec.ts` | `handoffRequested({source:'digest'})` | ✅ `test()` |
| H1.3 | Digest topic band hit row (D2) | `cold-start.spec.ts` | `handoffRequested({source:'digest'})` | ✅ `test()` |
| H1.4 | Digest topic band title (D3) | `cold-start.spec.ts` | `handoffRequested({source:'digest', camera:{kind:'fit'}})` | ✅ `test()` |
...
| H2.2 | Digest A → Digest B (D1 hot) | `hot-state.spec.ts` | Highlight clearing (asymmetry #10) | ✅ `test()` |
| H2.3 | Search A → Search B (S1 hot) | `hot-state.spec.ts` | Generation supersession | ✅ `test()` |
| H2.4 | Episode panel re-click (E1 hot) | `hot-state.spec.ts` | Asymmetry #2 (E1 missing setLoadSource) | ✅ `test()` |
| H2.5 | Mixed: Digest → Library (D1→L1) | `hot-state.spec.ts` | Highlight cleared on Library (asymmetry #10) | ✅ `test()` |
| H2.6 | Mixed: Library → Digest (L1→D1) | `hot-state.spec.ts` | Load-source flip-flop | ✅ `test()` |
| H2.7 | Mixed: Search → NodeDetail Load (S1→O3) | `hot-state.spec.ts` | graph-internal vs subject-external | ✅ `test()` |
...
| H3.2 | Digest pill × 2 (same topic) | `repeat-click.spec.ts` | Same as H3.1 for source: 'digest' | ✅ `test()` |
...
| --- | --- | --- | --- | --- |
| H4.1 | Library → Digest → Search | `cross-entry.spec.ts` | 3 different load-sources tracked + cleared in order | ✅ `test()` |
| H4.2 | Digest band → Library row → Digest pill | `cross-entry.spec.ts` | Camera strategy switch (`fit` → `center`) | ✅ `test()` |
| H4.3 | Search → NodeDetail Load → Search | `cross-entry.spec.ts` | `subject-external` → `graph-internal` → `subject-external` | ✅ `test()` |
...
| H6.2 | Handoff target id resolves to non-existent cy node | `failure.spec.ts` | `HandoffResult.status === 'failed'`; error strip; previous selection preserved | ✅ `test()` |
| H6.3 | Stuck handoff (load never returns) — 5s timeout | `failure.spec.ts` | After 5s, handoff cleared; `handoffStuck` event logged; ready for next click | ✅ `test()` |
