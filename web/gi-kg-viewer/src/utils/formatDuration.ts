...
/** Dashboard run wall time: ``3m 24s`` style. */
export function formatDashboardRunDurationSeconds(total: number | null | undefined): string {
  if (total == null || total < 0 || !Number.isFinite(total)) {
    return ''
  }
  const s = Math.floor(total)
  ...
  const sec = s % 60
  if (h > 0) {
    return m > 0 ? `${h}h ${m}m ${sec}s` : `${h}h ${sec}s`
  }
  if (m > 0) {
    return `${m}m ${sec}s`
  }
