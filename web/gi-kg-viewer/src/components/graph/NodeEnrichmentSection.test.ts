...
// ADR-108 — stance_timeline: this person's (topic) stances that shifted over time.
describe('NodeEnrichmentSection — stance shifts (ADR-108)', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    fetchEnvelope.mockReset()
  })

  function stanceEnvelope() {
    fetchEnvelope.mockImplementation((_root: string, enricher: string) => {
      if (enricher === 'stance_timeline') {
        return Promise.resolve({
          data: {
            timelines: [
              {
                ...
                topic_label: 'AI safety',
                points: [{ date: '2025-01-01', stance: -0.6 }, { date: '2025-09-01', stance: 0.5 }],
                deviation: { range: 1.1, slope: 0.4, shifted: true },
              },
              {
                // Flat trajectory for the same person — must be filtered out.
                person_id: 'person:jack',
                topic_id: 'topic:open-source',
                topic_label: 'Open source',
                points: [{ date: '2025-02-01', stance: 0.3 }, { date: '2025-08-01', stance: 0.31 }],
                deviation: { range: 0.01, slope: 0.0, shifted: false },
              },
            ],
          },
        } as never)
  ...
  it('renders only shifted trajectories, biggest swing first, with a direction glyph', async () => {
    stanceEnvelope()
    const w = await mountFor({ nodeId: 'person:jack', nodeType: 'person' })
    const block = w.find('[data-testid="node-enrichment-stance-shifts"]')
    expect(block.exists()).toBe(true)
    const text = block.text()
    expect(text).toContain('AI safety')
    expect(text).toContain('↗') // slope >= 0 → rising
    // The flat open-source trajectory is filtered out (shifted=false).
