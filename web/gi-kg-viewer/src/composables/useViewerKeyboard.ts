 ...
 *
 * - Slash — When focus is not in an editable control: expand the left query column if needed and
 *   focus `#search-q` (semantic search). Does not bind single-letter G / L globally; those remain
 *   click targets on each search hit row (avoids clashing with browser find and OS bindings).
