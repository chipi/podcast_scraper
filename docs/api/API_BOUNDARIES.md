...
The service module provides a programmatic interface optimized for non-interactive use:
...
### Service API Use (Recommended for Automation)
...
The public API follows **semantic versioning** and is tied to the module version:

- **API Version**: `podcast_scraper.__api_version__` (currently `"2.3.0"`)
- **Module Version**: `podcast_scraper.__version__` (same as API version)
...
- **Within major version**: All changes are backward compatible
- **Major version bump**: Requires migration (see `API_MIGRATION_GUIDE.md`)
