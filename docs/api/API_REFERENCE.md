...
Orchestrates the complete podcast scraping workflow:
...
Pydantic model that defines all configuration options for the podcast scraper. Supports both programmatic creation and loading from JSON/YAML files.
...
Loads configuration from a file and returns a dictionary. Supports both JSON and YAML formats. The file format is auto-detected from the file extension (`.json` or `.yaml`/`.yml`).
...
Structured result object returned by service API functions. Provides clear success/failure status and detailed information about the pipeline execution.
...
Main entry point for programmatic use. Executes the full pipeline and returns a structured result suitable for service/daemon use.
...
The API version follows semantic versioning (major.minor.patch) and is tied to the module version. The API version and module version are always the same.
...
- [API Boundaries](API_BOUNDARIES.md#api-versioning) - Detailed versioning policy
- [API Migration Guide](API_MIGRATION_GUIDE.md) - Migration between major versions
...
```python
from podcast_scraper import load_config_file, run_pipeline
...
All public functions and classes include complete type hints for IDE support and static type checking.
...
- [API Boundaries](API_BOUNDARIES.md) - API design and boundaries
