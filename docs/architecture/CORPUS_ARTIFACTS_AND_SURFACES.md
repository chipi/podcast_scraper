...
| FAISS index | `search/vectors.faiss` | embed / index build | `GET /api/search`, explore | n/a (binary) | 2.5+ |
...
| `GET /api/search` | `search/vectors.faiss` + sidecars | 200 with `error` field | no (structured error) |
