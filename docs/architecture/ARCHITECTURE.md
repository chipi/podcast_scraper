...
- **2 ML — gated dark**: `topic_consensus` + `stance_timeline` (cross-Person corroboration via symmetric NLI-entailment / per-person stance over time). The ADR-108 reimagining of the retired 0%-precision `nli_contradiction` + `stance_disagreement` — both flip to the robust entailment side of NLI and dissolve the shared-question gate without an LLM. Excluded by the accuracy gate below until an eval records precision ≥ 0.5. See [ADR-108](../adr/ADR-108-nli-disagreement-enrichers-gated-dark.md).
