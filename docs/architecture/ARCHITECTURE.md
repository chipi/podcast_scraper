...
- **6 deterministic** (pure Python over KG/GI, validated by exact-match gold): `grounding_rate`, `guest_coappearance`, `insight_density` (episode), `temporal_velocity`, `topic_cooccurrence_corpus`, `topic_theme_clusters`.
- **1 embedding**: `topic_similarity` (per-Topic Top-K cosine neighbours; validated recall@10 99%, top_k tuned to 7, #1105).
- **2 ML** (ADR-108 reimagining of the retired 0%-precision `nli_contradiction` + `stance_disagreement`, no LLM): **`topic_consensus` — promoted** (cross-Person corroboration via a composite of embedding cosine + low NLI contradiction; precision 0.91 on prod-v2, so the accuracy gate admits it); **`stance_timeline` — gated dark** (per-person stance over time; its stance signal is genuinely absent on prod-v2's factual insights, so no eval clears it). Membership is data-driven, not hardcoded. See [ADR-108](../adr/ADR-108-nli-disagreement-enrichers-gated-dark.md).
