...
| **Server API latency** | Viewer API endpoints (`/api/*`) respond within 500 ms for typical corpus sizes (≤ 500 episodes). Corpus search queries (LanceDB hybrid) return within 2 s including embedding. | Met | [RFC-062](../rfc/RFC-062-gi-kg-viewer-v2.md), [RFC-090](../rfc/RFC-090-hybrid-retrieval.md), [RFC-061](../rfc/RFC-061-semantic-corpus-search.md) |
| **Vector index build** | LanceDB index build completes within 5 min for a 500-episode corpus on target hardware. Background rebuild does not block API reads. | Met | [RFC-090](../rfc/RFC-090-hybrid-retrieval.md), `server/index_rebuild.py` |
...
| **Response integrity** | `200 OK` responses from cloud + self-hosted inference services that contain semantically corrupted content (empty, thinking-prose, `finish_reason=length`, malformed JSON, single-speaker labels for multi-speaker audio) are caught by content-aware guardrails and routed through the same fallback path as connection-level failures. Per-stage policy: summary / GI / KG / speaker fail-up; cleaning degrades gracefully. Violations emit a Prometheus counter for alerting. | Met | [ADR-099](../adr/ADR-099-response-shape-guardrails-for-self-deployed-services.md) (self-hosted), [ADR-100](../adr/ADR-100-response-shape-guardrails-for-cloud-llm-providers.md) (cloud) |
...
| **Index rebuild resilience** | Background LanceDB index rebuild is coordinated via locking; concurrent rebuild requests are rejected gracefully. A failed rebuild does not corrupt the existing index. | Met | [RFC-090](../rfc/RFC-090-hybrid-retrieval.md), `server/index_rebuild.py` |
...
| **Vector search scale** | Embedded **LanceDB** hybrid backend (BM25 + dense + RRF) is the sole search backend at v2.7+ ([RFC-090](../rfc/RFC-090-hybrid-retrieval.md)); the legacy FAISS store was retired (ADR-099 / #995); remote / multi-tenant backends ([RFC-070](../rfc/RFC-070-semantic-corpus-search-platform-future.md)) **Superseded** by RFC-090 — no longer planned. | Met (LanceDB hybrid) | [RFC-090](../rfc/RFC-090-hybrid-retrieval.md), [RFC-061](../rfc/RFC-061-semantic-corpus-search.md), [RFC-070](../rfc/RFC-070-semantic-corpus-search-platform-future.md) |
