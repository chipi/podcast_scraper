...
- **Related PRDs**:
  - [PRD-028: Position Tracker](../prd/PRD-028-position-tracker.md) -- full
    requirements, API contract, Phase 6 analysis vision
- **Related RFCs**:
...
- **Related UX specs**:
  ...
  - [UXS-010: Guest Intelligence Brief](UXS-010-guest-intelligence-brief.md) --
    "Open brief" handoff; **owns the shared Person Landing** that hosts both Guest
    Brief and Position Tracker
...
topic selector, Phase 6 placeholder regions, and graceful degradation states. All
...
- Phase 6 placeholder regions (change markers, stance banner)
...
- Phase 6 analysis UI implementation (only placeholder regions)
...
- **View in graph** -- navigates to the Graph tab and focuses on this person's node.
  Uses `primary` token.
- **Open brief** -- navigates to the Guest Brief (PRD-029) for this person. Uses
  `primary` token.
...
### Stance summary banner (Phase 6)

- Visible only when `stance_summary` is non-null.
- `surface` background with `gi` domain token left border (4px).
- Summary text in `text-sm`, `canvas-foreground`.
- A `muted` `text-xs` caption: "Generated by [provider] -- derived from corpus data."
...
- Each episode card:
  ...
  - Position-change marker (Phase 6): when `position_change_detected: true`, a
    "Position shifted" badge appears on the card. Uses `warning` token background
    with `warning-foreground` text. Hidden when `false`.
...
- "View in graph" and "Open brief" buttons
- All five degradation empty states
- Phase 6: "Position shifted" badge, stance summary banner
