...
| ~04:18 | Wipe succeeded; apply created all resources; placement succeeded in `nbg1`. Server created (id `<HETZNER_SERVER_ID>`, IPv4 `<HETZNER_IP>`). | Workflow log |
...
| ~05:10 | Operator re-pasted with the correct `.pub` content. Retriggered. Apply succeeded. Server created (final id `<HETZNER_SERVER_ID>` again, same IPv4 `<HETZNER_IP>`). | Workflow run 26618330139 |
...
- **Hetzner reused the public IPv4** (`<HETZNER_IP>` in nbg1) — saved cycles vs needing to update DNS or other IP-pinned config. (We don't actually pin on IP, but it's worth noting.)
