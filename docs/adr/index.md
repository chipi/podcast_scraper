...
| [ADR-099](ADR-099-lancedb-first-single-index-search.md) | LanceDB-first single-index search; retire FAISS | Proposed | [RFC-090](../rfc/RFC-090-hybrid-retrieval.md) | Serving ran LanceDB like FAISS (open-per-query, over-fetch with vectors, runtime fallback); LanceDB already holds all content so FAISS is a redundant copy. Make LanceDB the single index — opened once, native hybrid, `select()` projection — and remove FAISS (build/serve/config); rebuild, no migration | No |
