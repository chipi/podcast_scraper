...
| [ADR-108](ADR-108-nli-disagreement-enrichers-gated-dark.md) | Reimagine the NLI enrichers → `topic_consensus` + `stance_timeline` | Accepted | [RFC-088](../rfc/RFC-088-enrichment-layer-architecture.md), [RFC-103](../rfc/RFC-103-momentum-layer.md) | `nli_contradiction`/`stance_disagreement` hit 0% precision (sentence-NLI can't do the shared-question gate w/o an LLM). Reimagined onto NLI **entailment**: `nli_contradiction`→`topic_consensus` (cross-speaker corroboration), `stance_disagreement`→`stance_timeline` (per-person stance over time, plugs into RFC-103). | Reimagined |

## Gap analysis {:#gaps}

**Counts (reconcile when adding ADRs):** **108** files under `docs/adr/ADR-*.md` (ADR-001–ADR-108;
