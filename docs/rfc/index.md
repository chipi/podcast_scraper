...
| [RFC-065](RFC-065-live-pipeline-monitor.md) | Live Pipeline Monitor (macOS Developer Tooling) | #512 | `--monitor`, `spawn`, `.pipeline_status.json`, `rich` or `.monitor.log`; **`[monitor]`**: memray re-exec + py-spy **`f`**; tmux split deferred; [guide](../guides/LIVE_PIPELINE_MONITOR.md); split from RFC-064 |
