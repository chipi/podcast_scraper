...
| [RFC-089](RFC-089-dgx-spark-tailnet-integration.md) | DGX Spark tailnet integration | — | **Draft:** operator DGX on tailnet for dev, eval, pre-prod, and prod Whisper-with-fallback ([ADR-096](../adr/ADR-096-dgx-spark-prod-primary-with-fallback.md), [ADR-097](../adr/ADR-097-self-hosted-gha-runner-policy.md)). Runbook: [DGX_RUNBOOK](../guides/DGX_RUNBOOK.md). |
| [RFC-090](RFC-090-hybrid-retrieval.md) | Hybrid Retrieval Pipeline | [PRD-032](../prd/PRD-032-hybrid-corpus-search.md) | **Draft (shipped):** two-tier (segment + insight) + BM25 + dense + RRF over LanceDB; hybrid default-on with FAISS fallback; compound results. |
...
| [RFC-097](RFC-097-unified-kg-gi-ontology-v2.md) | Unified KG + GI ontology v2 | [PRD-017](../prd/PRD-017-grounded-insight-layer.md) / [PRD-019](../prd/PRD-019-knowledge-graph-layer.md) | **Draft:** formalization PR on RFC-072 (CIL + bridge already shipped) + #1035 (NER pre-pass). Entity → Person/Org/Podcast first-class; ABOUT + MENTIONS_PERSON + MENTIONS_ORG edges materialize per-artifact; `insight_type` + `position_hint` ship; two-tier edge contract; Position Tracker + Person Profile viewer surfaces ([#1036](https://github.com/chipi/podcast_scraper/issues/1036)). |
