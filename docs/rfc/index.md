...
| `RFC-046` (moved to the private eval repo as RFC-006) | Materialization Architecture | PRD-007 | v2.5.0 | Dataset materialization for honest evaluation comparisons |
| `RFC-047` (moved to the private eval repo as RFC-007) | Lightweight Run Comparison & Diagnostics Tool | PRD-007 | v2.5.0 | Streamlit-based visual tool for comparing runs |
| `RFC-048` (moved to the private eval repo as RFC-001) | Evaluation ↔ Application Alignment | PRD-007 | v2.5.0 | Fingerprinting and single-path eval-app alignment |
...
| `RFC-057` (moved to the private eval repo as RFC-002) | AutoResearch Optimization Loop (Prompts & ML Params) | PRD-007 | v2.6.0 | Closed per `ADR-073` (moved to the private eval repo as ADR-005); Tracks A/B complete; silver refs + 72-config eval matrix |
...
| `RFC-064` (moved to the private eval repo as RFC-008) | Performance Profiling and Release Freeze Framework | - | v2.6.0 | Frozen profiles under `data/profiles/`, `scripts/eval/profile/freeze_profile.py`, `diff_profiles.py`, `make profile-freeze` / `profile-diff`; guide (moved to the private eval repo) |
...
| `RFC-066` (moved to the private eval repo as RFC-009) | Run Comparison Tool — Performance Tab | - | v2.6.0 | Streamlit **Performance** page (`?page=performance`) joining run metrics with frozen RFC-064 profiles |
...
| `RFC-015` (moved to the private eval repo as RFC-004) | AI Experiment Pipeline | PRD-007 | v2.4–v2.7 | Phases 1–3 (runner, metrics, storage/comparison) shipped earlier; Phase 4 CI integration shipped via standalone `.github/workflows/ai-experiment-pipeline.yml` workflow (operator-triggered, no auto-gating). |
| `RFC-041` (moved to the private eval repo as RFC-005) | Podcast ML Benchmarking Framework | PRD-007 | v2.5–v2.7 | Phases 0–1 (dataset materialization, baseline, metrics) shipped earlier; Phase 4 CI integration shipped via standalone `.github/workflows/ml-benchmark.yml` workflow (smoke / benchmark dataset choice, operator-triggered, no auto-gating). |
...
| [RFC-109](RFC-109-per-episode-observability-manifest.md) | Per-episode observability manifest — measure in production, introspect, evolve | Draft | v2.4–v2.5 | The measure-in-prod operating model behind the prod-v2.x reprocess (#1190): run 100–500 eps/night, review the next morning, find the weak episodes + systemic gaps from a metric (not by hand), evolve one stage, reprocess only the affected subset. Proposes a per-episode processing manifest ([ADR-132](../adr/ADR-132-per-episode-processing-manifest-schema.md)) written stage-by-stage, layered versioning as the reprocess key, an append-only corpus ledger for query, and a clean SoT split from `metadata.json` ([ADR-133](../adr/ADR-133-metadata-vs-manifest-source-of-truth.md)). Seeded by ADR-131's `.asr.json`. |
| [RFC-110](RFC-110-outbound-delivery-and-seam.md) | Outbound Delivery & the App↔Infra Seam | Draft | v2.8 | The delivery loop for PRD-046: a channel-agnostic `DeliveryEnvelope` written to an app-owned outbox and drained by an infra delivery worker (the frozen app↔infra boundary, [ADR-145](../adr/ADR-145-channel-agnostic-outbox-seam.md)). Web Push self-hosted (VAPID); email via self-hosted Listmonk → SES last-mile ([ADR-144](../adr/ADR-144-self-hosted-delivery-queue-outsourced-last-mile.md)). Extractive, graph-carrying payload; no request-time LLM (D6). Reuses RFC-101 resurfacing selection, NOT the corpus-wide RFC-068 digest. |
| [RFC-111](RFC-111-curation-surfaces.md) | Curation Surfaces — Collections, Share Cards, Highlight↔Graph Wiring | Draft | v2.8 | The curation layer for PRD-046: per-user collections/boards spanning episodes, text-only share cards (bridge-only audio), and highlight↔KG-entity wiring that freezes the serialization shape the next-arc Obsidian/Notion export emits. Reuses RFC-072 canonical identity; additive, no ML. |
| [RFC-112](RFC-112-remote-mcp-transport-and-auth.md) | Remote MCP Transport + OAuth Authorization Server | Draft | v2.9 | Resolves RFC-095 OQ-1: one MCP server, both transports (stdio local + remote Streamable HTTP on prod). **OAuth 2.1 + PKCE (+ DCR) primary** — required so claude.ai per-user connectors work from day one (research-decided 2026-08-05); PAT secondary for CLI/dev. New orthogonal `mcp_access` entitlement; MCP process verifies via an internal app endpoint. Shared-corpus v1 (personal toggle → RFC-114 Phase 1). BYO cloud agent, D6-safe. New public ingress → THREAT_MODEL expansion. |
| [RFC-113](RFC-113-graph-aware-pkm-export.md) | Graph-Aware PKM Export (Obsidian) | Draft | v2.9 | The Obsidian export named as PRD-046's next arc: highlights become wikilinked, **id-keyed** `[[Entity]]`/`[[Guest]]`/`[[Topic]]`/`[[Episode]]` notes so the vault mirrors the personal KG (not a flat dump — the Snipd-differentiator). One-way incremental pull over RFC-114 Phase 1's revision counter (adds **and** tombstones); full re-export fallback. Obsidian-only v1; Notion is a separate OAuth-push slice. |
| [RFC-114](RFC-114-personal-corpus.md) | Personal Corpus — a Unified Definition Across Surfaces | Draft | v2.9 | Defines "personal corpus" once. **Phase 1 (keystone)**: faceted membership (`experienced` = heard∪captured, recall stays pinned here — no shipped-semantics change; vs `saved` = favorited) + a per-user **revision counter + change log** (deletions expressible) + `/api/app/corpus`. **Phase 2**: weighted strength (ranking; blocks nobody). The dependency for MCP `scope=mine` (RFC-112) and the export cursor (RFC-113). Read-time, no rebuilt index (D6). |

| [RFC-115](RFC-115-transcript-prefix-caching-llm-stages.md) | Transcript-Prefix Caching for LLM Stages | Proposed | v2.5+ | The cleaned transcript is re-sent to ~5 LLM stages/episode (~71K input tok/ep); every provider we use except Gemini's OpenAI-compat endpoint caches an identical leading token prefix at ~0.1× price, but the current layout puts stage-specific system prompts first so the transcript never caches (probed 0%). Move the transcript to the front of the system prompt → 69–99% cache hit (deepseek 98% / Novita 96% / openai 99% / qwen 69%), ~4–5× input-cost cut, step-change for reprocessing. Staged rollout gated by a quality-parity validation. |
| [RFC-117](RFC-117-pipeline-supervision-and-absence-detection.md) | Pipeline Supervision — Containment, Recovery and Absence-Detection | Draft | v2.7+ | Two 2026-08-12 outages were **one defect with two endings**: `CostCapExceeded` raised in a region of `orchestration.py` with no `try/finally`, orphaning a non-daemon worker whose continue-predicate defaults to `True` — 4h15m of silent spinning in one run, `cannot schedule new futures after interpreter shutdown` in the other. Argues the codebase is **defensively decorated, not undefended** (a per-job try/except, retry classifier, `timeout_context`, `stale` enum, reconcile endpoint, status writer and job webhook all exist, each gated off or non-enforcing) and needs a supervisor that makes them fire. Three independent pillars: containment (bounds + cancellation), crash-only recovery (the corpus is the ledger), and **absence-detection** — every existing signal is exception-triggered, so a process that stops working while staying alive is invisible. Key subtlety: a liveness heartbeat would NOT have caught the wedge (the loop was iterating); a progress watermark is required. |
| [RFC-118](RFC-118-incremental-corpus-derivations-and-delta-backbone.md) | Incremental Corpus Derivations — the Shared Delta Backbone | Draft | v2.7+ | A 1-episode prod repair (2026-08-23) drove a **full-corpus** enrichment pass: `topic_consensus` (pairwise DeBERTa NLI) ran ~28 min and timed out (run `efdca585`, 0 records). Reverses RFC-088's "incremental is a non-goal" (§11 amendment): the orchestrator computes one per-run `CorpusDelta` (fingerprint-derived `changed_ids`/`removed_ids` + full bundles) in `finalize` and distributes it to **every** corpus derivation — index (retrofit), enrichment, clustering. Only the two pairwise ML enrichers get true delta-merge with caches (raw NLI pair scores; topic vectors); the five cheap deterministic ones get skip-gates. Non-negotiable §7 gate: a Tier-2 reconciliation test asserting delta output ≡ full output, plus a scheduled prod `--force` reconciliation diff. Adds `compute_enrichment_staleness()` + `GET /api/enrichment/stats`, an explicit full-re-derive lever, and MCP `reindex`/`reenrich`/`corpus_status`. |

## Gap analysis {:#gaps}

**Counts (reconcile when moving RFCs):** **117** files under `docs/rfc/RFC-*.md` -- IDs **RFC-001--RFC-118**
with **no RFC-014**. Open / completed / Draft breakdown in the tables above (last recount 2026-08-08);
...
| `RFC-057` (moved to the private eval repo as RFC-002) | Closed per `ADR-073` (moved to the private eval repo as ADR-005) |
...
| `RFC-064` (moved to the private eval repo as RFC-008) | Frozen profiles, freeze/diff scripts (`ADR-075` (moved to the private eval repo as ADR-012)) |
| [RFC-065](RFC-065-live-pipeline-monitor.md) | `--monitor`, `.pipeline_status.json`, optional `[monitor]` |
| `RFC-066` (moved to the private eval repo as RFC-009) | Streamlit **Performance** vs frozen profiles (`ADR-076` (moved to the private eval repo as ADR-013)) |
