...
- **v2 closure (RFC-097, 2026-06-20)**: `Entity(kind=...)` discriminator replaced by typed `Person` + `Organization` + `Podcast` nodes; KG ontology v2.0 ships with `edge_class` metadata + new `HAS_EPISODE` edge. Migration: `cli upgrade run` (m0006). Topic semantic dedup remains v3. See [RFC-097](RFC-097-unified-kg-gi-ontology-v2.md).
