...
- **v2 closure (RFC-097, 2026-06-20)**: cross-layer use cases (now owned by RFC-072) get shipped viewer surfaces via Position Tracker + Person Profile (RFC-097 chunk 8 + PRDs-028/029). NL query layer remains deferred. See [RFC-097](RFC-097-unified-kg-gi-ontology-v2.md).
...
For interactive exploration, a Jupyter notebook template
...
- Supporting quote resolution with highlighted
  transcript spans
- Sanity checks (char range validation, timestamp
  validation, quote verbatim match)
- Topic link exploration

This template serves as a reference implementation for
the "Insight → Quote → Evidence Viewer" requirement
and can be adapted for other inspection tasks.
...
This RFC (RFC-050) is part of the Grounded Insight
...
RFC-049 (GIL Core)              → extraction
    ├── RFC-050 (this RFC)      → consumption
    ├── RFC-051 (DB Projection) → serving
    ▼
...
2. **RFC-050 (This RFC): Use Cases & Consumption** —
   Defines how GIL data is consumed (query patterns,
   output shapes, CLI)
3. **RFC-051: Database Projection** — Defines Postgres
   export for fast Insight Explorer queries
4. **PRD-017: Grounded Insight Layer** — Defines
   product requirements and user value

**Prerequisite RFCs:**

5. **RFC-044: Model Registry** — Model metadata
...
7. **RFC-052: Locally Hosted LLM Models** — Optimized
   prompts for local LLM extraction (Tier 2)
8. **RFC-053: Adaptive Summarization Routing** —
   Downstream consumer that routes episodes to
   optimal strategies

**Key Distinction:**

- **RFC-049**: *How* knowledge is extracted and stored
- **RFC-050**: *How* knowledge is consumed (this RFC)
- **RFC-051**: *How* knowledge is served at scale
...
2. **Natural Language Queries**: When should NL → graph
   ...
   **Post-v1, after Insight Explorer is validated.**
   ...
   map them to existing structured queries. This avoids
   free-form hallucination.

3. **Grounding Rate Thresholds**: What grounding rate
   is acceptable for v1?
   **≥80% grounded insights is the launch bar.** Below
   80%, the GIL is not trustworthy enough for user
   consumption. Improvement strategies for difficult
   transcripts: (a) use multiple question formulations
   in extractive QA (RFC-042 §11.4.3), (b) relax NLI
   threshold from 0.5 to 0.4, (c) increase QA top-k
   from 3 to 5. If a specific episode consistently
   scores below 60%, mark it as `gil_quality: "low"`
   in metadata.
...
RFC-050 defines **how GIL data delivers user value**
through five use cases centered on the insight + quote
pattern. The Insight Explorer (UC5) is the canonical
...
**Key principle**: The graph is internal plumbing; the
product is **trust + navigation**. Users retrieve
insights, see supporting quotes with timestamps, and
verify evidence against the transcript. Everything
else is implementation detail.

**RFC-050 works in parallel with RFC-049 (core
extraction) and RFC-051 (database projection). Together
