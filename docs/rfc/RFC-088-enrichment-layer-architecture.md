...
- **Status**: Active (in implementation, 2026-06-27) — Epic [#1101](https://github.com/chipi/podcast_scraper/issues/1101) carries the live implementation across 9 chunks. **Chunks 0 + 1 complete** (ADR-104 boundary settlement; foundation types, resilience + cost-cap, health, metrics, status, JSONL events, observability, executor, CLI + JSON Schema, jobs API + HTTP routes, MCP enrichment source + 8 tools, mock scorers + resilience integration tests, stack-test E2E specs, `docs/api/ENRICHMENT_LAYER_API.md`). Chunks 2–8 in progress. Chunk 8 promotes Active → Completed. Live plan: `docs/wip/RFC-088-ENRICHMENT-LAYER-IMPLEMENTATION-PLAN.md`; lock audit: `docs/wip/RFC-088-CHUNK1-LOCK-AUDIT.md`.
