...
- **Status**: Completed via redirect (v2.7) — proactive alerting moved to operator-side wiring on Sentry + Grafana + Langfuse (the deployed o11y surface). The original RFC plan to ship `scripts/generate_pr_comment.py` + `scripts/send_webhook_alert.py` was abandoned in favour of letting alert thresholds live where the signal lives (vendor side). The codebase already emits everything alerts need; operator-facing alerting recipes (Sentry error/perf rules, Grafana panel alerts on `metrics/history.jsonl`, Langfuse cost alerts) live in [`docs/guides/OBSERVABILITY_EXTENSIONS.md` §Operator alerting](../guides/OBSERVABILITY_EXTENSIONS.md#operator-alerting-sentry-grafana). The nightly `alerts[]` detection in `scripts/dashboard/generate_metrics.py` (`detect_deviations()`) still ships and feeds into the Grafana panels. RFC body kept as historical design record.
- **Related ADRs**:
