...
- **Quote**: A **verbatim** span from the transcript used as evidence. Quotes are first-class: they have character offsets, optional **audio timestamps**, and optional **speaker attribution** (`speaker_id`). Canonical rules (when each field is set vs empty) live in the Development Guide — [GI quote `speaker_id` (issue 541)](DEVELOPMENT_GUIDE.md#gi-quote-speaker-id) and [Transcript hash cache / provider timing (issue 543)](DEVELOPMENT_GUIDE.md#transcript-hash-cache-json). Short checklist:
  1. **`speaker_id` populated** only when timed segments align with the GI transcript **and** overlapping segment rows include **`speaker`** or **`speaker_id`** (e.g. diarized segment lists). Otherwise **`null`** ([#541](https://github.com/chipi/podcast_scraper/issues/541)).
  2. **When non-null**, the value is typically a **`person:{slug}`** graph id (RFC-072); pre-migration artifacts may still use **`speaker:{slug}`** ([GI ontology — Person / Quote](../architecture/gi/ontology.md)).
  3. **`speaker_id` null** also when segments are missing/misaligned or **time + text only** — typical Whisper/OpenAI shapes — see [#540](https://github.com/chipi/podcast_scraper/issues/540), [#542](https://github.com/chipi/podcast_scraper/issues/542), [#545](https://github.com/chipi/podcast_scraper/issues/545). The screenplay-marker offset-drift case (#545) is **fixed by #974**: GI reads the ad-free base whose segments carry explicit `char_start`/`char_end`, so a quote maps to its segment exactly (the 50-char guard is now a legacy-only fallback).
  4. **`timestamp_*_ms`** use the same segment alignment gate; they stay **0** without usable timed chunks (missing sidecar, empty provider **`segments`**, or backends that do not expose them — [#543](https://github.com/chipi/podcast_scraper/issues/543)).
  **UC2** / **`gi explore --speaker`** stay **best effort** when diarization is absent — see [Recorded product decisions (v1, issue 460)](#recorded-product-decisions-v1-issue-460) below.
...
- **`gi inspect`**: Inspect a single episode’s grounded insights. Use `--episode-path <path>` to point to a `.gi.json` file, or `--output-dir <dir>` and `--episode-id <id>` to locate the artifact (works from a **multi-feed corpus parent** under `feeds/`). If the same `episode_id` exists in more than one feed, add **`--feed-id`** matching metadata **`feed.feed_id`** ([CONFIGURATION.md — RSS and multi-feed](../api/CONFIGURATION.md#rss-and-multi-feed-corpus-github-440)). Options: `--format pretty|json`, `--show` (full insight text and quotes), `--stats` / `--no-stats`, `--strict` (strict schema validation).
