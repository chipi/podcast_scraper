...
**Storage:** `data/eval/references/{dataset_id}/{reference_id}/`

**Reference Quality:**

- **Silver**: Machine-generated by a strong model (e.g., GPT-5)
- **Gold**: Human-verified summaries
...
1. Moves artifacts from `runs/` → `references/{dataset_id}/`
2. Assigns stable ID (`silver_gpt5_2_v1`)
3. Marks as immutable
4. Creates `README.md` explaining purpose
5. Updates `baseline.json` with promotion metadata
6. Removes source run
...
└── references/              # Promoted references (truth)
    └── curated_5feeds_benchmark_v1/
        └── silver_gpt5_2_v1/
            ...
            ├── baseline.json
            ├── config.yaml
            └── README.md    # Explains why this reference exists
```
