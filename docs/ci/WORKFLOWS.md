...
**OpenTofu / DR drill (RFC-082 / #752):** `infra-ci.yml` posts the **default** workspace plan on PRs touching `infra/**`. `drill-infra-plan.yml` posts a second comment with the **`drill`** workspace plan (same path filters); it also accepts **`workflow_call`** from **`dr-drill-exercise.yml`** (fmt-check, validate, **`tofu plan`** — no apply). Manual apply uses `infra-apply.yml` (environment **prod**) versus **`drill-infra-apply.yml`** (environment **drill**); **`drill-infra-apply.yml`** also accepts **`workflow_call`** with **`skip_confirm: true`** from **`dr-drill-exercise.yml`**. **`drill-infra-destroy.yml`** (environment **drill**, confirm **`DRILL_DESTROY`**) downloads **`drill-tfstate-for-teardown`** (from **`drill-tfstate-bridge`**) or **`terraform-state-after-apply-drill`**, then **`tofu destroy`**, **Hetzner `hcloud` sweep**, **Tailscale device removal** for **`tag:dr-drill`** / drill hostname, then re-encrypts state; callable standalone or always last from **`dr-drill-exercise.yml`** (finalize + **`skip_confirm: true`**). **`deploy-drill.yml`** (environment **drill**) mirrors **`deploy-prod.yml`** but uses **`DRILL_TAILNET_FQDN`**, **`resolve_drill_tailnet_host.sh`**, and **`DRILL_DEPLOY_SSH_PRIVATE_KEY`**; it also accepts **`workflow_call`** from **`dr-drill-exercise.yml`**. **`drill-restore-corpus.yml`** (environment **drill**, confirm **`DRILL_RESTORE`**) downloads **`snapshot.tgz`** from the backup repo (default **`chipi/podcast_scraper-backup`**, tag **`snapshot-prod-*`** or pinned), restores under **`/srv/podcast-scraper/corpus`** on the drill host, and recreates **api** + **viewer**; optional secret **`BACKUP_REPO_TOKEN`** when the backup repo is private. **`dr-drill-exercise.yml`** chains **`drill-infra-plan`** → **`drill-infra-apply`** → **`drill-tfstate-bridge`** (re-upload **`drill-tfstate-for-teardown`** on the caller run) → **`deploy-drill`** → **`drill-restore-corpus`** → **`drill-e2e`** after **`DRILL_FULL_CYCLE`** (alias **`DRILL_EXERCISE`**) → **`drill-stack-playwright`** (**`tests/stack-test/stack-viewer.spec.ts`**, HTTPS **`STACK_TEST_BASE_URL`** via MagicDNS), then **always** **`drill-infra-destroy`** so the drill workspace returns to empty state even if a middle job failed. **`drill-e2e.yml`** (environment **drill**, confirm **`DRILL_SMOKE`**) probes **`/api/health`** on the drill VPS over tailnet SSH to **`deploy@`** (host viewer **8080**; read-only); callable standalone or from **`dr-drill-exercise.yml`**. **`drill-stack-playwright.yml`** (environment **drill**, confirm **`DRILL_STACK_PLAYWRIGHT`** when run alone) runs the same subset from the GHA runner over Tailscale (TLS, **`STACK_TEST_INSECURE_TLS=1`** in CI); callable standalone or from **`dr-drill-exercise.yml`** after **`drill-e2e`**. See `infra/README.md` and
[DR drill runbook](../guides/DR_DRILL_RUNBOOK.md).

**Corpus backups (RFC-082):** `backup-corpus.yml` (**Backup corpus snapshot**) is **`workflow_dispatch` only** (start the codespace, then dispatch, or `make codespace-backup-cloud`). `backup-corpus-prod.yml` (**Backup corpus snapshot (prod VPS)**) pulls `/srv/podcast-scraper/corpus` over Tailscale SSH as `deploy@` using **`PROD_SSH_PRIVATE_KEY`** (same as **`deploy-prod.yml`**; see `docs/guides/PROD_RUNBOOK.md` **GitHub Actions SSH to prod**); also manual until issue **#723** enables a prod schedule. **`prod-restore-corpus.yml`** (**Prod restore corpus**) is **`workflow_dispatch` only**, environment **prod**, confirm **`PROD_RESTORE`**: downloads **`snapshot.tgz`** from the backup repo (default **`chipi/podcast_scraper-backup`**, tag **`snapshot-prod-*`** or pinned), restores **`corpus/`** on the prod VPS, recreates **api** + **viewer**; optional **`BACKUP_REPO_TOKEN`** when the backup repo is private. Release tags: `snapshot-YYYYMMDD` (codespace) vs `snapshot-prod-YYYYMMDD` (VPS).
