...
For how **always-on prod hosting** (Tailscale, OpenTofu, GitHub Actions, Compose on the VPS) fits
together, see [Hosting and infrastructure](../architecture/HOSTING_AND_INFRASTRUCTURE.md).
...
The table above is the **core CI** lane only. Additional hosting, deploy, corpus backup/restore, and
DR drill workflows are summarized in the **OpenTofu / DR drill** and **Corpus backups** paragraphs
below and in [STACK_CONTRACT.md](../guides/STACK_CONTRACT.md).
...
**OpenTofu / DR drill (RFC-082 / #752):** `infra-ci.yml` posts the **default** workspace plan on PRs touching `infra/**`. `drill-infra-plan.yml` posts a second comment with the **`drill`** workspace plan (same path filters); it also accepts **`workflow_call`** from **`drill-exercise.yml`** (fmt-check, validate, **`tofu plan`** — no apply). Manual apply uses `infra-apply.yml` (environment **prod**) versus **`drill-infra-apply.yml`** (environment **drill**); **`drill-infra-apply.yml`** also accepts **`workflow_call`** with **`skip_confirm: true`** from **`drill-exercise.yml`**. **`drill-infra-destroy.yml`** (environment **drill**, confirm **`DRILL_DESTROY`**) downloads **`drill-tfstate-for-teardown`** (from **`drill-tfstate-bridge`**) or **`terraform-state-after-apply-drill`**, then **`tofu destroy`**, **Hetzner `hcloud` sweep**, **Tailscale device removal** for **`tag:dr-drill`** / drill hostname, then re-encrypts state; callable standalone or always last from **`drill-exercise.yml`** (finalize + **`skip_confirm: true`**). **`drill-deploy.yml`** (environment **drill**) mirrors **`deploy-prod.yml`** but uses **`DRILL_TAILNET_FQDN`**, **`resolve_drill_tailnet_host.sh`**, and **`DRILL_DEPLOY_SSH_PRIVATE_KEY`**; it also accepts **`workflow_call`** from **`drill-exercise.yml`**. **`drill-restore-corpus.yml`** (environment **drill**, confirm **`DRILL_RESTORE`**) downloads **`snapshot.tgz`** from the backup repo (default **`chipi/podcast_scraper-backup`**, **newest compatible** **`snapshot-prod-*`** with sibling manifest or pinned tag), restores under **`/srv/podcast-scraper/corpus`** on the drill host via **`scripts/ops/restore_corpus_from_tarball_host.sh`**, and recreates **api** + **viewer**; optional secret **`BACKUP_REPO_TOKEN`** when the backup repo is private. **`drill-exercise.yml`** chains **`drill-infra-plan`** → **`drill-infra-apply`** → **`drill-tfstate-bridge`** (re-upload **`drill-tfstate-for-teardown`** on the caller run) → **`drill-deploy`** → **`drill-restore-corpus`** → **`drill-e2e`** after **`DRILL_FULL_CYCLE`** (alias **`DRILL_EXERCISE`**) → **`drill-stack-playwright`** (**`tests/stack-test/stack-viewer.spec.ts`**, HTTPS **`STACK_TEST_BASE_URL`** via MagicDNS), then **always** **`drill-infra-destroy`** so the drill workspace returns to empty state even if a middle job failed, then **`assert-post-conditions`**. **`drill-e2e.yml`** (environment **drill**, confirm **`DRILL_SMOKE`**) runs **`post_deploy_smoke.sh`** over tailnet **HTTPS** (six surfaces; same as prod); callable standalone or from **`drill-exercise.yml`**. **`drill-stack-playwright.yml`** (environment **drill**, confirm **`DRILL_STACK_PLAYWRIGHT`** when run alone) runs the same subset from the GHA runner over Tailscale (TLS, **`STACK_TEST_INSECURE_TLS=1`** in CI); callable standalone or from **`drill-exercise.yml`** after **`drill-e2e`**. See `infra/README.md`,
[DR drill runbook](../guides/DR_DRILL_RUNBOOK.md), and [Stack contract](../guides/STACK_CONTRACT.md) (audit table, [ADR-093](../adr/ADR-093-canonical-stack-contract-and-environment-adapters.md)).

**Corpus backups (RFC-082, manifest RFC-084):** `backup-corpus.yml` and `backup-corpus-prod.yml` upload **`snapshot.tgz`** plus sibling **`snapshot.manifest.json`** when **`dry_run`** is false (default **`dry_run=true`** — tarball build only, no release upload). **`prod-restore-corpus.yml`** and **`drill-restore-corpus.yml`** default to **newest compatible** `snapshot-prod-*` via `scripts/ops/resolve_latest_snapshot_prod_tag.sh` (fail closed when no sibling manifest matches `config/corpus_snapshot_reader_support.json`); optional pin **`backup_tag`**; runner **`download_and_verify_snapshot.sh`** before SCP. Codespace restore locally: **`make restore-corpus`** (`snapshot-YYYYMMDD`); prod VPS on-host: **`make restore-corpus-prod`**. Release tags: `snapshot-YYYYMMDD` (codespace) vs `snapshot-prod-YYYYMMDD` (VPS).

**Prod deploy hardening (#796 / #797):** **`deploy-prod.yml`** requires confirm **`PROD_DEPLOY`**, validates **`override_image_sha`**, preflight GHCR manifests (`packages: read`), passes **`DEPLOY_GIT_SHA`** / **`DEPLOY_IMAGE_SHA`** to **`infra/deploy/deploy.sh`** (rejects 7-char prefix mismatch), then runs **`scripts/ops/post_deploy_smoke.sh`** over tailnet HTTPS (six API surfaces). Local mirror: **`make smoke-prod`**. **`python-app.yml`** job **`test-corpus-version-compat`** exercises N-1 corpus fixtures.

**GHCR retention (#802):** **`ghcr-retention.yml`** runs weekly (plus **`workflow_dispatch`**): deletes stale **`ghcr.io`** package versions per **`scripts/ops/ghcr_compute_retention.py`** (keep recent **`main`** SHAs, cap untagged). Local dry-run: **`make ghcr-prune-dry-run`**. See [GHCR retention](../operations/GHCR_RETENTION.md).

**Backup restore verify (#798):** **`verify-backup-restore.yml`** runs **Sundays 04:00 UTC** (plus **`workflow_dispatch`**): download newest compatible **`snapshot-prod-*`**, restore into ephemeral **`docker compose`** on GHA (`stack.yml` + `prod.yml`, GHCR **`:main`**), **`post_deploy_smoke.sh --base-url http://127.0.0.1:8090`**, tear down. Optional **`SMOKE_WEBHOOK_URL`** on failure; **`BACKUP_REPO_TOKEN`** when the backup repo is private.

**Weekly data baseline — REMOVED.** **`nightly-data-baseline.yml`** captured a weekly snapshot into **`data/baselines/`** and **`release.yml`** diffed against it. It moved to the private eval repo (`chipi/podcast-scraper-eval-data`, parked under `deploy/nightly/`) together with the two workflows **`release.yml`** used to call, **`ai-experiment-pipeline.yml`** and **`ml-benchmark.yml`** — all three read research data that no longer lives in this repo. There is no weekly snapshot and no `baseline-drift` issue on this side any more, and the release-notes draft says where the eval block comes from instead of embedding one.

**Scheduled DR drill (#799):** **`drill-exercise.yml`** adds **Wednesdays 02:00 UTC** cron (schedule skips typed confirm). **`drill-e2e.yml`** uses the same **`post_deploy_smoke.sh`** as prod. **`assert-post-conditions`** fails when any orchestrator job is not **`success`** or **`delete_drill_hetzner_orphans.sh --check-only`** finds Hetzner orphans after destroy. Scheduled failures ping **`SMOKE_WEBHOOK_URL`** when set. **`drill-deploy.yml`** mirrors prod manifest preflight (`packages: read`).

**Prod failover (RFC-083, ADR-089/090/091/093, #764):** **`prod-failover-stand-up.yml`** brings up a prod failover spare on the **DR drill VPS row** of the canonical stack contract ([ADR-093](../adr/ADR-093-canonical-stack-contract-and-environment-adapters.md)) without destroying it. Confirm token **`PROD_FAILOVER_STAND_UP`**. Composes **`drill-infra-plan`** → **`drill-infra-apply`** (`skip_confirm: true`) → **`drill-deploy`** → **`drill-restore-corpus`** (`skip_confirm: true`, newest compatible `snapshot-prod-*`) → **`freeze-ingestion`** (SCPs **`scripts/ops/failover/freeze_ingestion_on_spare.sh`** to the spare, strips **`scheduled_jobs`** from **`corpus/viewer_operator.yaml`** then read-back-verifies the key is absent; api is **not** restarted — the spare isn't serving traffic during stand-up validation, and APScheduler will read the key-less yaml on the api's next natural restart at manual cutover per `src/podcast_scraper/server/scheduler.py:271`; RFC-083 §6 dual-writer guard) → **`drill-e2e`** (`DRILL_SMOKE`) → **`drill-stack-playwright`** (`skip_confirm: true`). Per [ADR-089](../adr/ADR-089-prod-failover-orchestrator-separate-from-drill.md) the orchestrator **MUST NOT** compose **`drill-exercise`** or **`drill-infra-destroy`** — cutover (phase E) and failback (phase F) are **manual** per [PROD_RUNBOOK § Prod failover](../guides/PROD_RUNBOOK.md#prod-failover); spare decommission is a separate **`drill-infra-destroy`** dispatch with its own **`DRILL_DESTROY`** confirm ([ADR-091](../adr/ADR-091-prod-failover-gha-triggers-and-gates.md)). The orchestrator reuses the drill VPS row's **`DRILL_DEPLOY_SSH_PRIVATE_KEY`**, **`DRILL_TAILNET_FQDN`**, and **`resolve_drill_tailnet_host.sh`**; **`concurrency: prod-failover`** with **`cancel-in-progress: false`** prevents two operators racing a stand-up.
