...
**Weekly data baseline:** **`nightly-data-baseline.yml`** runs **Mondays 04:00 UTC** (plus **`workflow_dispatch`**): captures the project's data + setup state plus a fresh eval + benchmark run, commits the snapshot to **`data/baselines/baseline-YYYY-WNN.json`** on **main**. **`release.yml`** diffs against the most recent snapshot for the "what changed since last week" block in the release-notes draft. Opens/updates/closes a weekly `baseline-drift` issue via the `issues: write` permission.
