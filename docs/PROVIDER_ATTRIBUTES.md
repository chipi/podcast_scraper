...
This document describes provider-specific attributes that are exposed for backward compatibility and internal access. These attributes are **not part of the protocol interface** but are available on specific provider implementations.
...
**Important**: These attributes are implementation-specific and may not be available on all providers. Always use `getattr()` with a default value when accessing them.
...
**Note**: `OpenAITranscriptionProvider` does not expose a `model` attribute (uses API instead).

## SpeakerDetector Attributes
...
**Note**: `OpenAISpeakerDetector` does not expose an `nlp` attribute (uses API instead).

### `heuristics` (NERSpeakerDetector)
...
## SummarizationProvider Attributes

### `map_model` (TransformersSummarizationProvider)
...
**Note**: `OpenAISummarizationProvider` does not expose a `map_model` attribute (uses API instead).

### `reduce_model` (TransformersSummarizationProvider)
...
**Note**: `OpenAISummarizationProvider` does not expose a `reduce_model` attribute (uses API instead).
