...
- **Status**: Draft
- **v2 closure (RFC-097, 2026-06-20; chunk-8 scope-cut 2026-06-21)**: Person Profile **data foundation** delivered by RFC-097 v2 (chunks 1–7) — typed Person/Organization nodes, `MENTIONS_PERSON` + `MENTIONS_ORG` descriptive edges, `insight_type` LLM classification, `SPOKE_IN`/`SPOKEN_BY` already shipped. **Viewer surface delivered** by **#1048** (Person Landing shared component shell, 2026-06-23 — Person Profile + Position Tracker tab pair, identity-header role/episode-count/organization-chips, `ABOUT∩MENTIONS_PERSON` ranked-topic overview) + **#1050** (Person Profile aggregate view, deepens the Profile tab content). `person_profile` query pattern (RFC-072 §5B) consumed via `GET /api/persons/{id}/brief`. Analytical ranking/curation (analysis layer) + multi-corpus aggregation remain open. See [RFC-097](../rfc/RFC-097-unified-kg-gi-ontology-v2.md).
- **Authors**: Marko
- **Target Release**: TBD
- **Related RFCs**:
