...
- **FR1.1**: The user profile gains `email_verified` (trust the OAuth verified claim) and a `comms`
  ...
  `unsubscribe_ref`.
- **FR1.2**: `GET/PUT /api/app/comms` + a public no-auth unsubscribe: `GET /api/app/comms/unsubscribe?ref=` (prefetch-safe confirm page) and `POST` (the mutation — form submit + RFC-8058 one-click)
