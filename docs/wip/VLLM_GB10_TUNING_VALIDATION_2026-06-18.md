...
# COHORT COMPARISON ADDENDUM — #1033 corrected-pipeline rerun (2026-06-19)

The cohort comparison above was measured under the broken pipeline
(`kg_extraction_src: summary_bullets` — lossy, name-stripping). The
#1033 audit surfaced the eval/prod code-path divergence; step 2 reran
the full 7-candidate cohort GI + KG stages under the corrected
pipeline (`provider` source + tightened entity-vs-topic prompt).

**The corrected-pipeline rankings supersede the above for GI / KG
ordering.** Cell F's "GI cohort #1 WINNER" claim above no longer
holds. The daily-driver decision (Cell F as the autoresearch
single-model champion) **does still hold** — it just changes from
"wins outright on GI quality + is fastest" to "best speed-quality
trade-off; Qwen3.5-35B-A3B is the top-quality reserve."

Full evidence: `docs/wip/EVAL_1033_COHORT_RERUN_2026-06-19.md`.

## Corrected GI ranking (`provider` source, tightened prompt)

| # | Candidate | GI avg_sim | KG topic cov | GI s/run | KG s/run |
|---|---|---:|---:|---:|---:|
| 1 | **Qwen3.5-35B-A3B** | **0.618** | **50%** | 311 | 215 |
| 2 | **Cell F NVFP4** | **0.595** | 45% | **286** | **154** |
| 3 | Gemma-4-26B-A4B-it | 0.593 | 35% | 349 | 204 |
| 4 | Moonlight-16B-A3B | 0.581 | 40% | **169** | **140** |
| 5 | Ministral-3-14B | 0.574 | **50%** | 874 | 576 |
| 6 | Llama-3.3-70B-NVFP4 | 0.509 | 24% | 1031 | 638 |
| 7 | DSV2-Lite | 0.405 | 4% | 104 | 179 |

## Re-stated head-to-head: Cell F vs Qwen3.5-35B-A3B (corrected)

| Dimension | Qwen3.5-35B-A3B | Cell F NVFP4 | Δ |
|---|---|---|---|
| GI avg_sim | **0.618** | 0.595 | Qwen3.5 +3.9% |
| KG topic cov | **50%** | 45% | Qwen3.5 +5pp |
| GI + KG s/run | 526 | **440** | **Cell F -16%** ⚡ |
| Weight footprint | 67 GB bf16 | **18 GB NVFP4** | **Cell F -73%** |
| Boot time | ~6 min | **~2 min** | **Cell F -67%** |

## Why the picture changed

The old `summary_bullets` source routed KG/GI extraction through
summary text generated by a prompt with an explicit "no speaker
names" rule. That suppressed the inputs that would have separated
candidates. With `provider` source, candidates read the transcript
directly:

- Qwen3.5-35B-A3B's stronger prompt adherence produced cleaner
  topic boundaries (its 50% KG topic cov beats Cell F's 45%).
- Cell F NVFP4 still extracts well but the bf16-vs-NVFP4 quality
  gap is now visible (~0.02 GI, ~5pp KG topic).
- All candidates improved on KG topic coverage (+1pp to +4pp)
  under the corrected pipeline.

## Entity coverage — all 7 candidates remain at 0%

The tightened entity-vs-topic prompt (4 worked correct examples,
6 worked common-mistake examples, quantity guidance rule) reduced
false-positive emission (Cell F p02_e02: 7 false orgs → 3) but
no candidate finds the named persons/orgs the silver expects.

**This is a fundamental LLM extraction gap, not a per-model
capability dimension.** Pattern B confirmed across the strongest
models in the cohort. Filed as follow-up #1035 — add NER pre-pass
before LLM classification. Until #1035 lands, entity coverage is
not a useful cohort-ranking signal.

## Daily-driver decision — unchanged

Cell F NVFP4 remains the autoresearch single-model daily driver.
The rationale is updated from "Cell F wins GI outright + is fastest"
to:

- Cell F is **#2 on both GI and KG topic coverage** under the
  corrected pipeline (within bounded quality gap to Qwen3.5-35B-A3B:
  -3.9% GI, -5pp KG topic).
- Cell F is **16% faster end-to-end** on the corrected pipeline
  (GI+KG s/run: 440 vs 526).
- Cell F is **73% smaller weight footprint** and **~3× faster boot**.
- For highest-stakes one-shot evals, operator can manually swap
  the compose to Qwen3.5-35B-A3B — that note remains in the
  `prod_dgx_full_with_fallback` registry docstring.

The registry headline_metric for Cell F is updated to drop the
"GI cohort #1 WINNER" claim; the rest of the registry entry stays.
