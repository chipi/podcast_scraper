...
| - | --- | --- | --- | --- |
| 1 | RSS feed downloads | `rss/downloader.py` — was `requests.Session` + urllib3 `Retry` | `fetch_rss_feed_url` uses `httpx.Client` via `create_client(subsystem="rss_feed", transport_wrapper=RetryTransport(...))`; urllib3 `Retry` replaced by `rss/http_retry.py::RetryTransport` with the same 408/429/5xx forcelist, GET/HEAD/OPTIONS allowed methods, exponential backoff capped at 120s, and `Retry-After` handling. `http_retry_events` counter preserved (both `http_retry_events` and DEPRECATED `http_urllib3_retry_events` aliases). | ✅ shipped (#1194) |
| 2 | Media/transcript downloads | same file (`fetch_url`, `http_head`, `http_get`, `http_download_to_file`) | Same client, `subsystem="rss_generic"`. Streaming path swapped from `iter_content` to `iter_bytes`; `requests.RequestException` → `httpx.HTTPError`. Response return type is now `httpx.Response` (URL is now an `httpx.URL` object — callers using `resp.url` for `urljoin` cast via `str()`; two such call sites in `rss/parser.py` + `workflow/stages/scraping.py` updated). Two seam tests in `tests/integration/net/test_migrated_callsites.py` prove both `fetch_url` (rss_generic) and `fetch_rss_feed_url` (rss_feed) route through the outbound proxy. | ✅ shipped (#1194) |
