...
| **G (multi-feed, fixtures)** | **Eight** E2E mock RSS feeds on **`127.0.0.1`**: primary **`podcast1`–`podcast5`** plus long-form **`podcast7_sustainability`**, **`podcast8_solar`**, **`podcast9_solo`** (p07–p09; **p06** edge-case feed omitted); local Whisper + spaCy; **no** GI/KG in the preset | **`make serve-e2e-mock`** then `config/examples/manual_e2e_mock_five_podcasts.yaml` (or a copy under `config/manual/`) | `.test_outputs/manual/e2e_mock_fixture_multi_feed` |
