# Consumer Player — Golden Walkthrough v3 (entity map + every enrichment surface)
...
**What's new in v3 (2026-07-09).** The consumer person card now has a **"Where they agree"** row —
`topic_consensus` is no longer operator-only. §2.3, §3, §4, §5 and the appendix are updated, and the
person-card screenshot is a fresh capture showing the row live. Cross-reference: the **operator** viewer
(`gi-kg-viewer`) also gained a show-landing **Signals** band the same day (top topics / key people, from
`GET /api/corpus/feed-signals`) — that's an operator surface, not part of this player walk; see
RFC-104 / UXS-015 Phase 2.

**v1** was `PROD-V2-GOLDEN-WALKTHROUGH.md` (operator + consumer, prod-v2). **v2** was player-only and
exhaustive. **This v3** folds in the consumer consensus row. Captured against the **prod-v2** corpus
(10 shows / 209 episodes, re-enriched), signed in as the mock user "Ada Admin".
...
The **"Where they agree"** row shows on any person who is in a `topic_consensus` pair — e.g. search
**Nic Harrigan** → open the person card (the §2.3 capture).
...
| Person card | Person | "Often appears with", **"Where they agree"** counterpart, "Related people" chips |
...
![Home — discovery](assets/player-walkthrough-v3/01-home-discovery.png)
...
![Topic card — foreign policy](assets/player-walkthrough-v3/04-topic-card-foreign-policy.png)
...
![Topic card — conversation arc + perspectives](assets/player-walkthrough-v3/05-topic-conversation-arc-perspectives.png)
...
### 2.3 — Follow a person → the person entity card (now with "Where they agree")

From the topic card's "Related people", or by searching a name, open a **person card** (note the ‹ Back —
the graph traversal is remembered). Here it's **Nic Harrigan**, a quantum-computing guest:

![Person card — grounding + co-appears + consensus](assets/player-walkthrough-v3/06-person-card-grounding-coappears-consensus.png)

- **Grounding** — "29 of 29 claims backed by quotes (100%)" → **`grounding_rate`** (per person, the
  fraction of their insights that have a supporting quote; a credibility signal).
- **Often appears with** — Noah Kravitz · 1 → **`guest_coappearance`** (people who share episodes, ranked
  by shared-episode count; each chip pivots to that person's card).
- **Where they agree** *(new in v3)* — **`topic_consensus`**: other people who make **the same point** as
  this person on a topic. Each row is the **counterpart** (a button → their card) + "on {topic}", then
  **this person's claim** and the **counterpart's claim** verbatim, so you can read the corroboration.
  Here: *Philipp Herzig on quantum computing* and *Noah Kravitz on quantum computing* both echo Nic's
  claims. Oriented to the focused person (their claim is always the "self" line); capped at 8; the whole
  row hides when the person is in no consensus pair. Component: `EntitySignals.vue` (`es-consensus`).
...
![Episode player](assets/player-walkthrough-v3/08-episode-player.png)
...
![Insight-density strip](assets/player-walkthrough-v3/09-episode-density-strip.png)
...
![Knowledge Panel](assets/player-walkthrough-v3/10-knowledge-panel.png)
...
![Library](assets/player-walkthrough-v3/11-library.png)
...
![Search — grounded + recall](assets/player-walkthrough-v3/12-search-grounded-recall.png)

Corpus-wide grounded search — "12 passages across 5 episodes", each hit tagged INSIGHT / TRANSCRIPT /
TOPIC / PASSAGE with a ▶ timestamp. It **resolves entities** (a name/topic query surfaces an "Open …"
entity result at the top → the entity card — this is the §2.3 path) and has the **Everything / My corpus**
toggle — "My corpus" (Recall) scopes results to what *you've* heard or captured (honest-empty for a fresh
user).
...
- **topic_consensus** (ML) — the ADR-108 reimagining of the retired contradiction detector: two people
  **corroborate** on a topic when their insights have high embedding cosine **and** low NLI contradiction
  (precision 0.91 on prod-v2). Each surviving pair carries both insight texts + a consensus score. The
  consumer person card renders these as **"Where they agree"** (`EntitySignals.vue`), oriented so the
  focused person's claim is the "self" line; the same pairs also feed the operator viewer's person
  landing. Diarization placeholders are filtered so a per-episode `SPEAKER_NN` never fakes a consensus.
...
- **Grounding is a real trust signal.** "Nic Harrigan 100% (29/29)" vs speakers with lower rates gives a
  fast read on who's quote-backed.
- **Consensus reads as corroboration, not agreement theatre.** On "quantum computing", Nic Harrigan,
  Philipp Herzig and Noah Kravitz genuinely restate the same substance in different words — the row shows
  both claims so you can judge it yourself, which is the honest framing (it's "these two said the same
  thing", not an LLM adjudicating "they agree"). This resolves the v2 open question — the consumer now
  has a consensus surface, and it sits naturally next to Perspectives (per-speaker takes).
...
| EntitySignals (grounding / co-appears / **consensus** / momentum / similar / alongside) | `EntitySignals.vue` | `EntitySignals.test.ts` (incl. the consensus-row case); e2e `entity-signals.spec.ts` |
...
+ the corpus/episode enrichment routes. The consumer corpus-enrichment surface
(`GET /api/app/corpus/enrichment`) is **discovery-based** — it globs every `<root>/enrichments/*.json`
envelope and passes each enricher's `data` through under its `enricher_id`, so `topic_consensus` reaches
the player automatically once the enricher has run (no hardcoded id list). The committed fixture behind the
e2e is `tests/fixtures/app-validation-corpus/v3`, guarded by
`tests/integration/test_app_validation_corpus_invariants.py`.
