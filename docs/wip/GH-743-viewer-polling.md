...
| `PipelineJobExplorePanel` | `GET /api/jobs/subprocess-log-tail?…` | Same jobs API + corpus path; job status **running** or **queued** | **Adaptive:** about **3s** while the log tail fingerprint is changing; about **10s** after **3** unchanged tails. **Paused** when the tab is hidden. Full load (tail + `corpus_run_summary` + runs summary + manifest) still runs once per job/corpus change. |
