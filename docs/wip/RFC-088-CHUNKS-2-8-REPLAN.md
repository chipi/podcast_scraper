...
## Deferrals recorded post-chunk-8 (2026-06-27)

The post-chunk-8 quality sweep surfaced three plan items that took a different shape than the original plan body specified. Recording here so the trail stays honest:

- **Catalog row enrichment-availability flags**: the chunk-6 plan body called for `CorpusEpisodeListItem` to gain `enrichments_available` flags (booleans for which enrichers have produced an envelope for that episode). **Status:** shipped in the chunk-8 follow-up commit — `CorpusEpisodeListItem.enrichments_available: dict[str, bool]` populated when the corpus has `metadata/enrichments/{stem}.{id}.json`.
- **`ProfilePreset.enrichments` field**: the chunk-7 plan body called for a new field on the ML-stage `ProfilePreset` dataclass (in `providers/ml/model_registry.py`). **Status:** **intentional architectural deviation** — the matrix lives in a separate `enrichment/profile_sets.py` module instead, decoupling the enrichment-layer concern from ML-stage compositions. The chunk-8 follow-up commit records this in the guide and adds an `enrichment:` block to each top-level `config/profiles/*.yaml` so the matrix is also discoverable from the profile YAML (the Python matrix remains the source of truth; the YAML blocks are advisory).
- **PRD-026 / PRD-027 status promotion**: the chunk-8 plan body called for PRD-026 → Implemented and PRD-027 → Partial. **Status:** shipped in the chunk-8 follow-up commit — both PRDs' status fields updated.
