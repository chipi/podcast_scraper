...
```
┌──────────────────────────────────────────────────────────────────┐
│  LAST RUN    ● Success · 47 episodes · 3m 24s · 2 hours ago  →  │
...
```
INDEX STATUS
Last rebuilt: 14 days ago  ⚠  Rebuild recommended
Indexed: 175 of 213 episodes (82%)
...
```
Tab order:  Coverage  |  Intelligence  |  Pipeline
Default:    Coverage
```
...
**Route:** `GET /api/corpus/coverage?path=`
...
**Route:** `GET /api/corpus/persons/top?path=&limit=5`
...
web/gi-kg-viewer/src/components/dashboard/CoverageByMonthChart.vue
web/gi-kg-viewer/src/components/dashboard/FeedCoverageTable.vue
...
web/gi-kg-viewer/src/components/dashboard/DashboardView.vue
  — new tab structure (Coverage | Intelligence | Pipeline)
  — default tab: Coverage
...
### New API modules:
```
web/gi-kg-viewer/src/api/corpusCoverageApi.ts
  — fetchCorpusCoverage()
...
### New server routes:
```
src/podcast_scraper/server/routes/corpus_coverage.py
...
### New server tests:
```
tests/unit/podcast_scraper/server/test_viewer_corpus_coverage.py
tests/unit/podcast_scraper/server/test_viewer_corpus_persons.py
```
...
web/gi-kg-viewer/e2e/E2E_SURFACE_MAP.md
  — briefing-last-run, briefing-corpus-health, briefing-action-items
  — briefing-all-clear
