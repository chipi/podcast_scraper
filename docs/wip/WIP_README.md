...
- **OPEN — airgapped/local ML grounder at 8% coverage** ([EVAL_GROUNDING_WHO_FINDS_THE_QUOTE_2026_07](https://evals.tail6d0ed4.ts.net/guides/eval-reports/EVAL_GROUNDING_WHO_FINDS_THE_QUOTE_2026_07/)) — the local extractive-QA + NLI grounder finds evidence for only **8%** of insights vs **82%** for the LLM (qwen) grounder. Two structural faults: no retrieval step, and the NLI verifier demands strict entailment that insights don't satisfy. Fix = add embedding retrieval **and** replace the verifier. Only affects the **local/offline (airgapped) profiles** — cloud/DGX profiles ground with the summarising LLM. This is the "tracked separately" item from the report; parked here (no GH issue opened) until scheduled.
