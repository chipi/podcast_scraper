...
| `HANDOVER-v2.5-phase.md` | **Cold-start handover for the fresh v2.5 session.** Part 1 = the ORIGINAL v2.5 arc (DGX-local LLM swap, Steps 1–5 with goal/metric/gate each; judge-panel-only gate; 2.5-first); includes the verified 2026-08-02 DGX entry-state (serving stack intact; Step 1 = bring up the autoresearch vLLM via `gpu-mode-swap research`). Part 2 = the NEW backlog items (host identity, corpus growth, quality tier) AFTER 2.5 freezes. | Active (handover) |
...
- **HANDOVER — [HANDOVER-2026-08-07-llm-cache-and-reprocess.md](HANDOVER-2026-08-07-llm-cache-and-reprocess.md)** — full brain-dump for a fresh session to (1) implement RFC-111 transcript-prefix caching (all providers cache except Gemini; move the transcript to the front of the system prompt → 69–99% hit, ~4–5× input-cost cut, huge for reprocessing), (2) kick off reprocessing, (3) re-derive the finale (opp#2). **Landmines captured:** the GI value gate was silently OFF for the whole finale (`_LLM_PROVIDERS` missing litellm/qwen — fixed) so its rankings + costs are invalid; real cost comes from `gateway_spend.py` SpendLogs, not the app (finale under-counted ~2–3×). Branch `feat/naming-arc-and-corpus-prep`, 67 unpushed commits, **do not push**. **Active**.
- **HANDOVER — [HANDOVER-2026-08-07-llm-cache-and-reprocess.md](HANDOVER-2026-08-07-llm-cache-and-reprocess.md)** — full brain-dump for a fresh session to (1) implement RFC-115 transcript-prefix caching (all providers cache except Gemini; move the transcript to the front of the system prompt → 69–99% hit, ~4–5× input-cost cut, huge for reprocessing), (2) kick off reprocessing, (3) re-derive the finale (opp#2). **Landmines captured:** the GI value gate was silently OFF for the whole finale (`_LLM_PROVIDERS` missing litellm/qwen — fixed) so its rankings + costs are invalid; real cost comes from `gateway_spend.py` SpendLogs, not the app (finale under-counted ~2–3×). Branch `feat/naming-arc-and-corpus-prep`, 67 unpushed commits, **do not push**. **Active**.
