...
| ---- | ----------- | ------ |
| `RFC097_FOLLOWUPS_HANDOFF.md` | Session-handoff for `feat/rfc097-followups` branch. Covers the 4 issues spawned by PR #1061 (#1060 registry preset promotion, #1048/#1049/#1050 viewer chunk-8 surfaces), suggested order, and the 3 open decisions on #1060 that need operator input before code (`ProfilePreset` redesign vs YAML-only first-class, `summllama` promote vs migrate, dev/airgapped benchmark data path). | Active |
