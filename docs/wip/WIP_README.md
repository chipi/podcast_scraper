...
| `player/1144-DISAGREEMENT-DETECTOR-FEASIBILITY.md` | #1144 decision (07-07): **BUILD a precise stance-level detector.** The spike's 0/40 on prod-v2 is a corpus property (small, curated, mostly agreeable interviews — 31/40 `no_shared_question`), not a feature verdict; the mechanism is proven (injected v3 panel detects + surfaces). Design: a `stance_disagreement` ML enricher gated on a **shared question** (the fix for `nli_contradiction`'s topic-adjacency over-firing), emitting only confident disagreements, accuracy-gated vs the Opus silver. | Active |
