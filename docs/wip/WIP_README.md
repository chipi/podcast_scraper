...
| ---- | ----------- | ------ |
| `SPEAKER-RESOLUTION-ROADMAP.md` | Roadmap to reduce unknown diarized speakers, with **before/after measurement per step** on the real prod-v2 corpus (90 diarized eps / 579 voices). Shipped: #1a own-turn self-intro (**+41 voices named, 36.6%→43.7%, 0 regressions**), #1b episode-scoped ids (eliminates 7 cross-show + 9 cross-episode phantom-person merges; `person:speaker-00` was fusing 16 eps across 6 shows). Remaining reducers ordered B (per-feed `known_hosts`) → D (NER intro guest detection) → C (reconcile-hosts precursor — measured 0 fuel today). | Active (plan + measured) |
