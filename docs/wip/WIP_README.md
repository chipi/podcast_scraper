...
| ---- | ----------- | ------ |
| `STORYLINE-INDEX-PROD-MIGRATION-2026-09-17.md` | Post-deploy procedure for the prod operator now that storylines are real index rows (`doc_type="storyline"`, one row per theme cluster). ONE additive `cli index --output-dir /app/output` via `run --rm --no-deps api` — not a rebuild, no schema migration, no api restart, app never started. Records why `index-two-tier` / `--rebuild` must NOT be used (`drop_existing=True` + divergent chunk params: 32/36 re-embedded, segments 131→115), the root-ownership trap that silently turns search dark (`index_unreadable` → `no_index`), the `topic_theme_clusters.json` precondition, verification curls, and an equal-weight NOT-verified section (nothing run against prod; prod runtime and storyline count unmeasured; ranking untuned). | Active (runbook) |
| `HANDOVER-2026-09-15-player-offline-profile.md` | Analysis handover for the player offline / profile-missing / can't-reach-server cluster. Maps each symptom to the governing code, records what tonight's `5c21ec20a`+`701f0fec7` fix changed (read-gate split, native connectivity signal, boot ordering, guard token fallback), the build/deploy state (dist + iOS bundle at sha `701f0fec76c2`, uncommitted Android push gradle), and a NOT-VERIFIED/suspects section — chiefly that the masthead signed-in gate (`App.vue`) and the router guard use two different definitions of signed-in, and that nothing is confirmed on device. | Active (handover) |
...
| `2026-09-02-enricher-reenable-prep.md` | Prepared runbook (#1921) for turning the three DETERMINISTIC corpus enrichers back on (`grounding_rate`, `topic_cooccurrence_corpus`, `topic_theme_clusters`) after the Batch A ingestion lands — the two ML ones (`topic_similarity`, `topic_consensus`) stay off by operator decision. Carries the full enabled/disabled inventory, the `PUT /api/enrichment/config` whole-block-replace hazard, a schema-validated payload, and a testable hypothesis for WHY they were disabled (30s manifest cap vs the 300s override the two enabled corpus-scope enrichers carry). Also lists what to measure, including whether populated co-occurrence reframes #1918. | Active |
...
- `BAKEOFF-18EP-RESULTS.md` (moved to the private eval repo) (moved to the private eval repo) — final medium-tier multi-provider bake-off on the 18-episode prod-v3 corpus, 8 clean arms scored with grok-4.3 as the single vendor-disjoint judge (gemini-2.5-flash-lite leads on surf/ep; sonnet-5 and gpt-5.5 dropped — empty-content failures on bundled evidence calls, the latter also triggering the entailment-fallback money-guardrail fix).
...
- [2026-09-11-share-card-design.md](2026-09-11-share-card-design.md) — design SSOT for the shareable "collectible" card (#2036): the rejected glossy Higgsfield frame (too gamified / off-brand) → the chosen **design-system-native quote-led editorial minimal** (mono + one cyan accent, Georgia serif, square, lots of air), with exact tokens + layout/slots, client-canvas render (no dep, bridge-only), and the Share menu (card / link / text). Built v1 = entity card + episode; remaining = show/storyline surfaces, per-kind accent, a signature quote on the entity card, and the server OG-image for link unfurl. **Active.**
