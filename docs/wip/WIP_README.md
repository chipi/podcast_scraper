...
| ---- | ----------- | ------ |
| `HANDOVER-2026-08-03-tailscale-oauth-cleanup.md` | Session close-out for the `TS_API_KEY` → OAuth migration ([[ADR-143]]). Functional work DONE + on `main` (migration #1388, teardown-exchange fix, ACL tag self-ownership `afab42bb`, docs, DR drill green). Records the 3 keeper OAuth clients (`ka9…` infra / `knKTVm…` ACL / `kZTri…` join) and the operator-only TODO the API can't reach: console credential sweep (API-tokens + Auth-keys tabs), confirm `TS_OAUTH_CLIENT_ID`, deferred `tag:infra-minter`. Also the "if it 400s in 3 months, fix `tagOwners` — do NOT recreate the client" rule. | Active (handover) |
...
| `PLAN-v2.5-corpus-and-naming-arc-handover.md` | Cold-start handover after #1355 merged (v2.4 → main `b24608fa`). North star = the **2.5 corpus**: v2.4 pipeline/quality with the **Gemini** naming/summarization LLM swapped for a **DGX-local** model at judge-panel parity. Ordered plan B (close #1360 test gap) → A (naming/labeling arc #1226/#1228/#1169/#1192/#1286/#1170/#1285) → C (docs-hygiene + go-live) → **D autoresearch prompt-tuning loop** (per-provider, bake-off picks the model, gate = disjoint-vendor scalar judge-panel parity vs Gemini) → E provider swap → F freeze 2.5. Locked: model decided by the loop; gate = judge-panel parity; tuning precedes the swap. | Active (handover) |
...
| `1000-EPISODES-REPROCESS-PLAN.md` | Umbrella sequencing plan for the `feat/1000-episodes` arc: reprocess v2→v3 with other LLMs + expand to 500–1000 eps / 20–30 podcasts. Organizing principle = **reprocess-once economics** (lock artifact-shape / input-text / model changes before the run or rebuild the whole corpus twice). Key call: v4 fixtures (#1189) built as a **growing harness** — scaffold the container now, B and A author the rows, freeze the full ladder just before the reprocess gate. Phases 0–4, Bucket A (lock: #1189, #1178/#1179, #1191, #1220, #1188) / B (decide: #630, #102, #1192, #972) / C (out: LoRA, go-live, viewer, housekeeping). #1219 flagged as first item of the **post-reprocess viewer-perf pass** (scales badly at 1000 eps). Open decisions: scaffold thickness, cut scope, expansion feeds. | Active (plan) |
...
| `SEARCH-V3-IMPLEMENTATION-PLAN.md` | 8 shipping slices (S1–S8) + S0 baseline for PRD-045 / RFC-107. Promotes search from 288 px sidebar to 5th main tab (Query Workspace) + Cmd-K palette, merges Search+Explore, adds result-set operators (cluster/timeline/graph/compare/contradictions), enriched-answer hero, rail-launch handoffs, in-episode search, saved queries. Adopts ADR-095 three-tier pyramid + labelled quality-eval harness + CDP perf capture mirroring `capture-graph-lcp`. Encodes #1205 SIGSEGV guardrails (never re-enable LanceDB `_combine_hybrid_results` — Python-side `search_bm25 + search_vector + rrf_fuse` only) as first-class review gates. | Active (plan) |
...
- [HANDOVER-next-arc-rfcs-2026-08-06.md](HANDOVER-next-arc-rfcs-2026-08-06.md) — next-arc RFCs 112 (remote MCP + OAuth, slices 1–4 + hardening), 113 (incremental graph-aware Obsidian export + button), 114 (personal corpus Phase 1+2) built on `feat/next-arc-rfcs` (unpushed). All local gates green; fable-5 review findings implemented (H1–H4, M1–M8); pre-public deploy (Caddy MCP vhost + gate exemptions) and a few residue items are your call.
