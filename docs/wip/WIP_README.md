...
| `1046-WHISPER-MULTI-MODEL-DESIGN.md` | Design doc for #1046 (Whisper sniff-pass gate) — cost model, break-even analysis, measurement methodology for (r, T_small, T_large, gate accuracy), GPU budget under autoresearch contention, per-request model selection design, acceptance milestones. Gates on operator-attended ~1h measurement pass on DGX before service code lands. | Active |
