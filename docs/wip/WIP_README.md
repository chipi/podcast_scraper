...
| `ADR-108-REAL-CORPUS-EVAL-2026-07.md` | Honest real-DeBERTa eval of the reimagined `topic_consensus` + `stance_timeline` (ADR-108) over prod-v2 (99 bundles). **Neither clears a meaningful precision bar → both correctly stay dark.** topic_consensus emits 1 pair over 2 903 scored (symmetric entailment ≥ 0.6 is very strict; recall ≈ 0). stance_timeline's stance signal is ~0 (factual insights don't entail evaluative anchors); a `sign_flips`-on-noise bug flagged 11/12 flat trajectories as shifted — **fixed with a deadzone** (`move_threshold/2`), post-fix 1/13. No `gate_metrics.json` fabricated. UI still demoable via the fixture corpus; legitimate activation is an autoresearch-style tuning follow-up. | Findings |
