...
| `ISSUE-382-TRANSFORMERS-V5-EXECUTION-PLAN.md` | Step-by-step execution manual on branch `feat/upgrade-transformers-v5`: 10 phases (pre-baseline → deps → cache → QA → hybrid → summarizer → tests → parity gate → docs → CVE cleanup), per-phase test-harness updates + coverage, data/eval baseline capture + `run-compare` parity, docs edits (ADR-068/069/071 + CHANGELOG + README + issue #382), rollback matrix, push-gate checklist | Active |
