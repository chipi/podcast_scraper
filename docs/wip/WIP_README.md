...
| `RFC-088-real-corpus-validation-findings.md` | Post-chunks-0-9 real-corpus validation (item #8) against `.test_outputs/manual/prod-v2/corpus` (209 episodes, 99 latest-run bundles). Closes Bug 1 (CLI was a no-op — registry never populated, episode_bundles=[]; this branch wires `register_deterministic_enrichers` + `discover_episode_bundles` + force-include semantics for `--only` on an empty base). Surfaces Bugs 2–5 as follow-ups: temporal_velocity.velocity_last_over_6mo always 0, guest_coappearance polluted by SPEAKER_NN placeholders, metadata duration_seconds=0, run summary records_written always 0. | Active (Bug 1 fixed; 2–5 open) |
