...
| ---- | ----------- | ------ |
| `RFC097_FOLLOWUPS_HANDOFF.md` | Session-handoff for `feat/rfc097-followups` branch. **2026-06-23 update:** #1060 + #1048 closed in this branch — #1060 (commit `36ed9274`): D1=B doc-only + D2=A summllama_3_2_3b_paragraph StageOption + D3=A tiny.en/medium.en/bart-small+long-fast StageOptions; 3 ProfilePresets promoted (airgapped/airgapped_thin/dev); benchmark at `docs/guides/eval-reports/EVAL_DEV_TIER_REGISTRY_2026_06_23.md`. #1048 (commit `ceeb0485`): Person Landing shared shell — "Person Profile" + "Position Tracker" tab pair, identity-header role/episode-count/org-chips, ABOUT∩MENTIONS_PERSON ranked-topic overview; vitest 2227/2227 + viewer vue-tsc strict build green. Viewer chain remainder (#1049 Position Tracker timeline, #1050 Person Profile aggregate) still active. | Active |
