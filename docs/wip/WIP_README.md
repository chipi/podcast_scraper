...
| `HANDOVER-2026-08-29-batch-a-remainder.md` | Executable runbook for the next batch: ingest **10 episodes each** for the **10 remaining §5f Batch A feeds** (Conversations with Tyler, In Our Time, The Rest Is History, Empire, ChinaTalk, Sinica, Odd Lots, EconTalk, Ground Truths, The Long Run). Carries the `PUT /api/feeds` merge (**it replaces the whole list — never append-by-overwrite**), the per-feed `POST /api/jobs` call, and the prod-proven measurement recipe behind §5j's table. All ten RSS re-verified 2026-08-29 (counts match §5f + 2 weeks of new episodes). Open: per-episode cost has never been measured. | Active |
