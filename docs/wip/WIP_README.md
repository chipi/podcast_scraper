...
| `player/1144-DISAGREEMENT-DETECTOR-FEASIBILITY.md` | #1144 decision (07-07): **do NOT build** the stance-level disagreement detector. The feasibility spike (Opus over 40 cross-speaker stance pairs on prod-v2) found **0 disagreements** (31 `no_shared_question`, 9 `agree`) — real cross-person disagreement is near-absent, so a detector has nothing to detect. Perspectives (#1146) already ships the multi-speaker surface. Reopen only if debate-format corpora enter scope. | Active |
