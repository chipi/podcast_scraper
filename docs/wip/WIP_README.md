...
| ---- | ----------- | ------ |
| `RFC-088-ENRICHMENT-LAYER-IMPLEMENTATION-PLAN.md` | 7-chunk plan to actually implement RFC-088 Enrichment Layer (protocol + registry + executor + 6 deterministic enrichers + topic_similarity + nli_contradiction + QueryEnricher + viewer integration). Resolves the RFC-097/RFC-088 boundary via new ADR-104. Open decisions for operator listed at the bottom. | Active (plan) |
