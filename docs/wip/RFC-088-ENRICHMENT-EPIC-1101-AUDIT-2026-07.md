...
| 3 — topic_similarity (embedding) | #1105 | **CLOSED** (07-06) | enricher + eval + sweep | eval ran on prod-v2 (24-topic Opus silver) → **recall@10 99%, precision@10 71%** (measured locally, not persisted as a gate metric — topic_similarity declares no `accuracy_gate`, so the shipped outcome is the tuned `top_k` default); default top_k retuned 10->7 (80/80 @7) | ✅ validated — useful signal, tuned; closed |
