...
- **GI / KG Viewer (v2)** — Optional browser UI: graph visualization, dashboard, semantic search, explore, **Corpus Library** (feed/episode browser, [RFC-067](docs/rfc/RFC-067-corpus-library-api-viewer.md)), and **Corpus Digest** (time-windowed highlights, [RFC-068](docs/rfc/RFC-068-corpus-digest-api-viewer.md)) — all against a pipeline output folder ([RFC-062](docs/rfc/RFC-062-gi-kg-viewer-v2.md); see below). Cross-surface navigation is owned by a single **graph handoff FSM** ([ADR-094](docs/adr/ADR-094-graph-handoff-orchestrator-fsm.md), [RFC-085](docs/rfc/RFC-085-graph-handoff-orchestrator-retrospective.md)) with envelope contract, generation-token concurrency, stuck detection, and visible error UX.
- **Generic MCP server** — Optional agent-facing tool surface: expose the platform's read capabilities (hybrid search, the relational-query layer, CIL intelligence, catalog) as composable, read-only **[MCP](https://modelcontextprotocol.io) tools** so agentic clients (Claude Desktop/Code, Cursor) use a corpus as a tool — resolve an entity, search for grounded evidence, ask "who said what across shows", pull a person's positions or position-arc, all provenance-bearing ([PRD-034](docs/prd/PRD-034-generic-mcp-server.md), [RFC-095](docs/rfc/RFC-095-generic-mcp-server.md)). **Two transports:** **stdio** for local agents (`podcast mcp --corpus <dir> --transport stdio`) and **Streamable HTTP** for remote, bring-your-own-model agents over the public edge — a hosted **OAuth 2.1 authorization server + personal-access tokens** + the `mcp_access` entitlement, connectable from **claude.ai / Claude Code / Cursor** ([RFC-112](docs/rfc/RFC-112-remote-mcp-transport-and-auth.md)). Ships in `[dev]`; needs `[search]` for retrieval. See the [Server Guide — MCP server](docs/guides/SERVER_GUIDE.md#mcp-server-agent-tools) + Player launch — MCP.
- **Personal corpus + knowledge export** — a read-time definition of what a user has engaged with (`experienced` = heard ∪ captured; `saved` = save-for-later), versioned by a per-user **revision counter + change log** ([RFC-114](docs/rfc/RFC-114-personal-corpus.md)), plus a graph-aware **Obsidian vault export** (wikilinked, incremental, bridge-only) via `/api/app/export` ([RFC-113](docs/rfc/RFC-113-graph-aware-pkm-export.md)).
- **Operator playbook** — Always-on Hetzner VPS hosting design with Tailscale-only ingress ([RFC-082](docs/rfc/RFC-082-always-on-pre-prod-and-prod-hosting.md), PROD_RUNBOOK), version-aware **corpus snapshot backup + restore** ([RFC-084](docs/rfc/RFC-084-corpus-backup-manifest-and-version-aware-restore.md), [ADR-092](docs/adr/ADR-092-corpus-snapshot-backup-manifest-and-newest-compatible-restore.md), `make restore-corpus` / `restore-corpus-prod`), **instance-to-instance corpus portability** without going through the backup repo (`make export-corpus` / `make import-corpus`; laptop ↔ VPS, prod ↔ codespace, airgapped — Corpus airgap runbook), a **ledgered corpus-upgrade framework** ([#862](https://github.com/chipi/podcast_scraper/issues/862), [CORPUS_UPGRADE](docs/guides/CORPUS_UPGRADE.md), `make upgrade-check` / `upgrade-corpus`) that carries deployed corpora across release boundaries and is auto-run on prod restore ([#1176](https://github.com/chipi/podcast_scraper/issues/1176)), and a **canonical stack contract** ([ADR-093](docs/adr/ADR-093-canonical-stack-contract-and-environment-adapters.md)) covering dev / codespace / pre-prod / prod with one health surface and one `stack-test` discipline.
- **Three-tier viewer test pyramid** — Tier-1 mocks, Tier-2 production-shaped fixtures, Tier-3 real corpus via `make ci-ui-validation CORPUS=…` ([ADR-095](docs/adr/ADR-095-viewer-test-pyramid.md), [RFC-086](docs/rfc/RFC-086-viewer-test-pyramid-and-production-shaped-fixtures.md)). Real-bug-first matrix rule keeps coverage honest.
...
> **The default diarization model (`community-1`, v4) is non-gated — no Hugging Face
> token needed**, and the ML image bakes it in. A token is only required if you switch
> to the gated `pyannote/speaker-diarization-3.1` fallback (whose `segmentation-3.0` is
> also gated and cannot be redistributed in a published image). For that fallback,
> accept the terms on Hugging Face, then pass your token and a persistent cache so the
...
| Production Runbook | Always-on VPS operator playbook (RFC-082) |
| VPS Multi-App Onboarding | Hosting additional compose stacks on the same VPS (RFC-087) |
| [Viewer Graph Spec](docs/architecture/VIEWER_GRAPH_SPEC.md) | Graph handoff orchestrator + canvas contract (ADR-094) |
| [AGENTS.md](AGENTS.md) | Canonical contributor + AI-assistant guide (single source of truth) |
