...
- **Operator playbook** — Always-on Hetzner VPS hosting design with Tailscale-only ingress ([RFC-082](docs/rfc/RFC-082-always-on-pre-prod-and-prod-hosting.md), [PROD_RUNBOOK](docs/guides/PROD_RUNBOOK.md)), version-aware **corpus snapshot backup + restore** ([RFC-084](docs/rfc/RFC-084-corpus-backup-manifest-and-version-aware-restore.md), [ADR-092](docs/adr/ADR-092-corpus-backup-manifest-defaults-and-restore-trust.md), `make restore-corpus` / `restore-corpus-prod`), **instance-to-instance corpus portability** without going through the backup repo (`make export-corpus` / `make import-corpus`; laptop ↔ VPS, prod ↔ codespace, airgapped — [Corpus airgap runbook](docs/guides/CORPUS_AIRGAP_RUNBOOK.md)), a **ledgered corpus-upgrade framework** ([#862](https://github.com/chipi/podcast_scraper/issues/862), [CORPUS_UPGRADE](docs/guides/CORPUS_UPGRADE.md), `make upgrade-check` / `upgrade-corpus`) that carries deployed corpora across release boundaries and is auto-run on prod restore ([#1176](https://github.com/chipi/podcast_scraper/issues/1176)), and a **canonical stack contract** ([ADR-093](docs/adr/ADR-093-canonical-stack-contract-and-environment-adapters.md)) covering dev / codespace / pre-prod / prod with one health surface and one `stack-test` discipline.
