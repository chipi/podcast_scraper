...
- **Generic MCP server** — Optional agent-facing tool surface: expose the platform's read capabilities (hybrid search, the relational-query layer, CIL intelligence, catalog) as composable, read-only **[MCP](https://modelcontextprotocol.io) tools** so agentic clients (Claude Desktop/Code, Cursor) use a corpus as a tool — resolve an entity, search for grounded evidence, ask "who said what across shows", pull a person's positions or position-arc, all provenance-bearing ([PRD-034](docs/prd/PRD-034-generic-mcp-server.md), [RFC-095](docs/rfc/RFC-095-generic-mcp-server.md)). Stdio; `podcast mcp --corpus <dir>` (ships in `[dev]`; needs `[search]` for retrieval). See the [Server Guide — MCP server](docs/guides/SERVER_GUIDE.md#mcp-server-agent-tools).
